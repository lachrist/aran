...
import { slice, map, join, hasOwn } from "../util/index.mjs";
import { isIntrinsic, isParameter } from "../lang.mjs";
...
export const convertLink = (node) => {
  if (node.type === "ImportDeclaration") {
    if (node.specifiers.length === 0) {
      return {
        type: "ImportLink",
        source: /** @type {convert.Source} */ (node.source.value),
        import: null,
        tag: locate(node.loc),
      };
    } else {
...
export const convertStatement = (node) => {
  if (node.type === "BlockStatement") {
    ...
    return {
      ...
      kind: node.kind,
      variable: /** @type {convert.GlobalVariable} */ (
...
export const convertExpression = (node) => {
  if (node.type === "Literal") {
    if (hasOwn(node, "bigint")) {
      ...
      return {
        type: "PrimitiveExpression",
        primitive: /** @type {estree.SimpleLiteral} */ (node).value,
        tag: locate(node.loc),
      };
    }
    ...
    return {
      ...
      variable: /** @type {convert.GlobalVariable} */ (node.elements[0].name),
      tag: locate(node.loc),
    };
  } else if (node.type === "Identifier") {
    switch (node.name) {
    ...
    return {
      ...
      head: map(
        slice(node.expressions, 1, node.expressions.length),
        convertEffect,
      ),
      tail: convertExpression(node.expressions[node.expressions.length - 1]),
      tag: locate(node.loc),
    };
