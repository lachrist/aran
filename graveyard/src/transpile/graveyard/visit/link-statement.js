...
exports.visitLinkStatement = (scope, node, context) => /* [Prelude] */ (
...
visitors.ImportDeclaration = (scope, node, context, _index) => (
  ...
  ArrayLite.map(
    node.specifiers,
    (specifier) => Visit.visitImportSpecifier(scope, specifier, {source:node.source.value})));

exports.visitImportSpecifier = (scope, node, context) => /* Prelude */ (
  Throw.assert(
    (
...
visitors.ExportNamedDeclaration = (scope, node, context) => (
  ...
  ArrayLite.map(
    node.specifiers,
    (specifier) => Visit.visitExportSpecifier(
      scope,
      specifier,
  ...
  (
    ...
    ArrayLite.map(
      ArrayLite.flatMap(
        ...
        (node) => Query.getPatternIdentifierArray(node.id)),
      (identifier) => Tree.ExportLink(identifier))));

exports.visitExportSpecifier = (scope, node, context) => /* Prelude */ (
  Throw.assert(
