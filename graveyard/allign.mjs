 ...
 * @param {import("./allign").Result<A1, A2>[]} results
 * @param {string} path
 * @param {[aran.Node<A1>, aran.Node<A2>]} parents
 * @returns {import("./allign").Result<A1, A2>}
 */
const combineAllResult = (results, path, parents) => {
  ...
  const labels = flatMap(
    /** @type {(import("./allign").Result<A1, A2> & { type: "success" })[]} */ (
      results
    ),
    listLabel,
  );
...
/**
 * @type {<A1 extends aran.Atom, A2 extends aran.Atom>(
 *   result: import("./allign").Result<A1, A2>,
  ...
  if (node1.type === "ScriptProgram" && node2.type === "ScriptProgram") {
    ...
    return [
      ...
      ...flatMap(
        ...
        (index) =>
          /** @type {import("./allign").Result<A1, A2>[]} */ (
            allignNode(
    ...
    return [
      allignPrimitive(
        [node1.head.length, node2.head.length],
        `${path}.head.length`,
        pair,
      ),
      ...flatMap(
        enumerate(min(node1.head.length, node2.head.length)),
        (index) =>
          allignNode(
            [node1.head[index], node2.head[index]],
            `${path}.head[${index}]`,
          ),
      ),
 ...
 * @returns {import("./allign").Result<A1, A2> | null}
 */
