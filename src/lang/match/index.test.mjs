...
const success = (type, code1, code2, mapping1, mapping2) =>
  Assert.deepEqual(
    Match.match(
...
const failure = (type, code1, code2, regexp) =>
  Assert.deepEqual(
    Match.match(
      Parse[`parse${type}`](code1),
      Parse[`parse${type}`](code2),
...
// Export //
success("Link", `export foo;`, `export foo;`, Mapping.Empty(), Mapping.Empty());
failure(
  "Link",
  `export foo;`,
  `export bar;`,
...
Assert.deepEqual(
  Match.match(
    Tree.ListStatement([
      Tree.ListStatement([Tree.BreakStatement("k1")]),
      Tree.BreakStatement("l1"),
    ]),
    Tree.ListStatement([Tree.BreakStatement("k2"), Tree.BreakStatement("l2")]),
    (success, result) => (
      Assert.deepEqual(success, true),
      Assert.deepEqual(result, {
...
// primitive //
success("Expression", `123`, `123`, Mapping.Empty(), Mapping.Empty());
failure("Expression", `123`, `456`, /^Structural mismatch .* 123 and 456\n/);
// intrinsic //
success("Expression", `#eval`, `#eval`, Mapping.Empty(), Mapping.Empty());
failure(
  "Expression",
  `#Reflect.get`,
  `#Reflect.set`,
...
// read //
success("Expression", `x1`, `x2`, Mapping.Single("x1", "x2"), Mapping.Empty());
success("Expression", `input`, `input`, Mapping.Empty(), Mapping.Empty());
// Consumers //
// export //
success(
  "Expression",
...
success(
  ...
  `(x1 = y1, 123)`,
  `(x2 = y2, 123)`,
  Mapping.combine(null, Mapping.Single("x1", "x2"), Mapping.Single("y1", "y2")),
  Mapping.Empty(),
);
success(
  "Expression",
...
success(
  ...
  `enclave super[x] = z`,
  `enclave super[y] = t`,
  Mapping.combine(null, Mapping.Single("x", "y"), Mapping.Single("z", "t")),
  Mapping.Empty(),
);
// sequence //
success(
