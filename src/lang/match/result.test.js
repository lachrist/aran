...
Assert.deepEqual(Result.SingleIdentifier("path", "input", "input"), {
  identifier: Mapping.Empty(),
  label: Mapping.Empty(),
  structural: null,
});

Assert.deepEqual(Result.SingleIdentifier("path", "foo", "input"), {
  identifier: `Input missmatch at path between foo and input`,
  label: Mapping.Empty(),
  structural: null,
});

Assert.deepEqual(Result.SingleIdentifier("path", "input", "foo"), {
  identifier: `Input missmatch at path between input and foo`,
  label: Mapping.Empty(),
  structural: null,
});

Assert.deepEqual(Result.SingleIdentifier("path", "foo", "bar"), {
...
Assert.deepEqual(Result.SingleLabel("path", "foo", "bar"), {
  identifier: Mapping.Empty(),
...
Assert.deepEqual(
  Result.check("path", 123, 456, () => Assert.fail()),
  {
    identifier: Mapping.Empty(),
...
Assert.deepEqual(
  Result.check("path", "foo", "bar", () => Assert.fail()),
  {
    identifier: Mapping.Empty(),
    label: {__proto__: null},
...
Assert.deepEqual(
  Result.combine(
    "path1",
    Result.check("path2", 123, 456, () => Assert.fail()),
    Result.Empty(),
  ),
  {
    identifier: Mapping.Empty(),
...
Assert.deepEqual(
  Result.bindIdentifier("path1", "input", "bar", Result.Empty()),
  {
