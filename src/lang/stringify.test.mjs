...
import {parseProgram, parseExpression} from "./parse.mjs";
import {stringifyReadable} from "./stringify.mjs";

const test = (parse, code) => {
  assertEqual(stringifyReadable(parse(code)), code);
};

test(parseExpression, "123");
test(parseExpression, "! 123");
test(parseExpression, "(\n123 +\n456)");
test(parseExpression, "_foo");
test(parseExpression, "$foo");
test(parseEffect, "_foo = 123");
test(parseEffect, "$foo = 123");
