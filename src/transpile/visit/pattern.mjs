...
const visitPattern = generateVisit1({
  CallExpression: (node, serial, scope, state, meta) =>
    makeSequenceEffect(
      makeExpressionEffect(visitExpression(node, scope, state, true)),
      makeExpressionEffect(
        makeThrowReferenceErrorExpression("Cannot assign to call expression"),
      ),
      serial,
    ),
  MemberExpression: (node, serial, scope, meta) =>
    makeSetExpression(
      isStrictScope(scope),
...
visitors.RestElement = (scope, node, context) =>
  Visit.visitPattern(scope, node.argument, context);

visitors.Property = (scope, node, context) =>
  Visit.visitPattern(scope, node.value, context);
...
visitors.CallExpression = (scope, node, context) =>
  sequence(
    ...
    sequence(
      context.surrounding,
      lift(context.surrounding, Visit.visitExpression(scope, node, null)),
      lift(
        context.surrounding,
...
visitors.MemberExpression = (scope, node, context) =>
  boxify(
    context.surrounding,
    scope,
    false,
        ...
        : boxify(
            ...
            (object_box) =>
              boxify(
                ...
                false,
                "AssignMemberProperty",
                Visit.visitKey(scope, node.property, {
...
visitors.Identifier = (scope, node, context) =>
  context.kind === null
    ...
    : (Throw.assert(
        ...
        null,
        `Cannot initialize variable in an expression context`,
      ),
...
visitors.AssignmentPattern = (scope, node, context) =>
  boxify(
    context.surrounding,
    scope,
    false,
    ...
    context.expression,
    (box) =>
      Visit.visitPattern(scope, node.left, {
        kind: context.kind,
...
visitors.ObjectPattern = (scope, node, context) =>
  boxify(
    context.surrounding,
    scope,
    false,
          ...
          node.properties[node.properties.length - 1].type === "RestElement"
            ? ArrayLite.mapReduce(
                ...
                (next, property) =>
                  boxify(
                    ...
                    false,
                    "AssignObjectRestKey",
                    Visit.visitKey(scope, property.key, {
                      computed: property.computed,
                    }),
                    (key_box) =>
                      sequence(
                        context.surrounding,
                        Visit.visitPattern(scope, property.value, {
                          kind: context.kind,
                          surrounding: context.surrounding,
            ...
            : ArrayLite.mapReduce(
                ...
                (next, property) =>
                  sequence(
                    context.surrounding,
                    Visit.visitPattern(scope, property, {
                      kind: context.kind,
                      surrounding: context.surrounding,
                      expression: Intrinsic.makeGetExpression(
                        Scope.makeOpenExpression(scope, box),
...
visitors.ArrayPattern = (scope, node, context) =>
  boxify(
    context.surrounding,
    scope,
    false,
