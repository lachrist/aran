...
const INIT_TREE_SIZE = /** @type {TreeSize} */ (0);
 ...
 *     ) => ExternalValue,
 *   },
 *   registery: WeakMap<InternalPrimitive, TreeSize>,
 ...
 * }>}
 */
const compileAdvice = ({ apply, construct }, registery) => ({
  ...
  "test@before": (_state, kind, value, _tag) => {
    console.log(`${kind} >> ${getTreeSize(value, registery)}`);
  ...
  "apply@around": (_state, callee, that, input, _tag) =>
    enterValue(
      ...
      reduce(
        input,
        (size, value) =>
          /** @type {TreeSize} */ (size + getTreeSize(value, registery)),
        /** @type {TreeSize} */ (
          getTreeSize(callee, registery) + getTreeSize(that, registery)
        ),
      ),
      registery,
    ),
