...
module.exports = (aran, join) => {
  ...
  const sanitize = ((() => {
    const substitutes = SafeWeakMap();
    substitutes.set(Function, function Function () {
    ...
    substitutes.set(eval, function eval (script) {
      return $eval(join(script, null));
    });
    substitutes.set(Proxy, function Proxy (target, handlers) {
       // https://github.com/jsdom/webidl2js/issues/78
    ...
    return (value) => substitutes.has(value) ? substitutes.get(value) : value;
  }) ());
  ...
  traps.read = (identifier, value, serial) => {
    ...
    return produce(sanitize(result), serial);
  };
  ...
  traps.get = (value1, value2, serial) => {
    consume(value2, serial);
    consume(value1, serial);
    return produce(sanitize(value1[value2]), serial);
  };
