...

{
  ...
  const keywords2 = [
    ...
    "method",
    "constructor",
    Tree._builtin_identifier,
    Tree._eval_check_identifier
  ];
  const parameters = Object.values(Tree._parameters);
}
...
Label
  = s:IdentifierName
  & { return !keywords1.includes(s) }
  { return s }

Identifier
  = s:IdentifierName
  & { return !keywords1.includes(s) && !keywords2.includes(s) }
  { return s }

Declarable
  = s:IdentifierName
  & { return !keywords1.includes(s) && !keywords2.includes(s) && !parameters.includes(s) }
  { return s }
