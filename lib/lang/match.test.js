...
/////////////
{
  const success = (code1, code2, mapping1) => Assert.deepEqual(
    Match._match(
      Parse._parse_program(code1),
      Parse._parse_program(code2),
      (success, mapping2) => (
        Assert.ok(success),
...
/////////////
{
  const success = (code1, code2, mapping1) => Assert.deepEqual(
    Match._match(
      Parse._parse_prelude(code1),
      Parse._parse_prelude(code2),
      (success, mapping2) => (
        Assert.ok(success),
        Assert.deepEqual(mapping1, mapping2),
        "foo")),
...
///////////
{
  const success = (code1, code2, mapping1) => Assert.deepEqual(
    Match._match(
      Parse.PARSE_BLOCK(code1),
      Parse.PARSE_BLOCK(code2),
      (success, mapping2) => (
        Assert.ok(success),
        Assert.deepEqual(mapping1, mapping2),
        "foo")),
...
///////////////
{
  const success = (code1, code2, mapping1) => Assert.deepEqual(
    Match._match(
      Parse.ParseStatement(code1),
      Parse.ParseStatement(code2),
      (success, mapping2) => (
        Assert.ok(success),
        Assert.deepEqual(mapping1, mapping2),
        "foo")),
...
////////////////
{
  ...
  success(`enclave super[x1](...x2)`, `enclave super[y1](...y2)`, {__proto__:null, x1:"y1", x2:"y2"});
  // sequence //
  success(`(x1, y1)`, `(x2, y2)`, {__proto__:null, x1:"x2", y1:"y2"});
  // conditional //
