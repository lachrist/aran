...
const generate_program = (indent, program) => Tree._dispatch_program(program_callback_object, indent, program);

const program_callback_object = {
  __proto__: null,
  _program: (indent, node, preludes, block) => (
    ArrayLite.join(
      ArrayLite.map(
        preludes,
        (prelude) => generate_prelude(indent, prelude)),
      "") +
...
const generate_prelude = (indent, prelude) => Tree._dispatch_prelude(prelude_callback_object, indent, prelude);

const prelude_callback_object = {
  __proto__: null,
  _import: (indent, node, specifier, source) => (
    "import " +
  ...
  _export: (indent, node, specifier) => (
    ...
    ";" +
    ("\n" + indent)),
  _aggregate: (indent, node, specifier1, source, specifier2) => (
    "aggregate " +
    (
      specifier1 === null ?
      "*" :
  ...
  construct: (indent, expression, callee_expression, argument_expression_array) => (
    ...
    (
      ...
      (
        ...
        generate_construct_argument_part(indent, argument_expression_array) +
        ")"))),
  import: (indent, node, specifier, source) => (
    "import " +
    (specifier === null ? "*" : specifier) +
    " from " +
    global_JSON_stringify(source)),
  export: (indent, node, specifier, expression) => (
    "export " +
    (
    ...
    " " +
    generate_expression(indent, expression)),
  require: (indent, expression, source_expression) => (
    "require " +
    generate_expression(indent, source_expression)),
...
exports._program = (program) => generate_program("", program);

exports._prelude = (prelude) => generate_prelude("", prelude);
...
exports._indented_program = generate_program;

exports._indented_prelude = generate_prelude;
