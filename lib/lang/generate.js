...
const statement_callback_object = {
  Import: (indent, statement, identifier, source) => (
    ("\n" + indent) +
    ("import * as " + identifier + " from ") +
    global_JSON_stringify(source) +
    ";"),
  Export: (indent, statement, key, expression) => (
    ("\n" + indent) +
    ("export " + key + ": ") +
    generate_expression(indent, expression) +
    ";"),
  Aggregate: (indent, statement, source) => (
    ("\n" + indent) +
    "export * from " +
    global_JSON_stringify(source) +
    ";"),
  ...
  construct: (indent, expression, callee_expression, argument_expression_array) => (
    ...
    (
      ...
      (
        ...
        generate_construct_argument_part(indent, argument_expression_array) +
        ")"))),
  import: (indent, expression, source_expression) => (
    "import " +
