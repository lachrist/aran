...
const callbacks = {
  ...
  arrow: (newline, node, block) => `() =>${generate(newline, block)}`,
  ...
  enclave_write: (newline, node, strict, identifier, expression) => `enclave${strict ? `!` : `?`} ${identifier} =${generate(newline, expression)}`,
