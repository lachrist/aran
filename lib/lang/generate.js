...
const callbacks = {
  ...
  // Block //
  BLOCK: (newline, node, identifiers, statements) => (
    `{` +
    (
  ...
  Continue: (newline, node, label) => `continue ${label};`,
  ...
  Bundle: (newline, node, statements) => (
    ...
    ArrayLite.join(
      ArrayLite.map(
        ...
        (statement) => generate(newline, statement)),
      "")),
  Lone: (newline, node, labels, block) => `${generate_label_array(labels)}/* lone */${generate(newline, block)}`,
  If: (newline, node, labels, expression, block1, block2) => (
    `${generate_label_array(labels)}if (` +
    generate(newline, expression) +
    `)` +
    generate(newline, block1) +
    ` else` +
    generate(newline, block2)),
  While: (newline, node, labels, expression, block) => (
    `${generate_label_array(labels)}while (` +
    generate(newline, expression) +
    `)` +
    generate(newline, block)),
  Try: (newline, node, labels, block1, block2, block3) => (
    `${generate_label_array(labels)}try` +
    generate(newline, block1) +
    ` catch` +
    generate(newline, block2) +
    ` finally` +
  ...
  arrow: (newline, node, block) => `arrow ()${generate(newline, block)}`,
  function: (newline, node, block) => `function ()${generate(newline, block)}`,
  method: (newline, node, block) => `method ()${generate(newline, block)}`,
  constructor: (newline, node, block) => `constructor ()${generate(newline, block)}`,
  ...
  enclave_super_invoke: (newline, node, expression1, expression2) => `enclave super[${generate(newline, expression1)}](...${generate(newline, expression2)})`,
