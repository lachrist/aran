...
const statement_callback_object = {
  ...
  Aggregate: (path, statement1, statement2, source1, source2) => check(
    path + ".aggregate-source",
    source1,
    source2,
    null,
    EMPTY),
  Export: (path, statement1, statement2, key1, expression1, key2, expression2) => check(
    path + ".export-key",
    key1,
    key2,
    null,
    () => Tree._allign_expression(
      expression_callback_object,
      path + ".export-expression",
      expression1,
      expression2)),
  Import: (path, statement1, statement2, identifier1, source1, identifier2, source2) => check(
    path + ".import-source",
    source1,
    source2,
    null,
      ...
      ArrayLite.concat(
        ...
        ArrayLite.map(
          ...
          (_, index) => Tree._allign_expression(
            ...
            expressions1[index],
            expressions2[index]))))),
  import: (path, expression1, expression2, expression11, expression21) => Tree._allign_expression(
    expression_callback_object,
    path + ".import-source",
    expression11,
    expression21),
