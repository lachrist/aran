...
import { intrinsics } from "../syntax.mjs";
...
/** @type {(node: EstreeNode, labels: Label[]) => Block<Location>} */
const convertBlockInner = (node, labels) => {
  if (node.type === "LabeledStatement") {
    return convertBlockInner(node.body, concat$_(labels, node.label.name));
...
/** @type {(node: EstreeNode) => Block<Location>} */
export const convertBlock = (node) => convertBlockInner(node, []);
...
export const convertIntrinsic = (node) => {
  if (node.type === "Identifier") {
    ...
    return concat$_(convertIntrinsic(node.object), node.property.name);
  } else {
    throw makeSyntaxError(node);
  }
};
  ...
  if (node.type === "Literal") {
    if (hasOwn(node, "bigint")) {
      return {
        ...
        primitive: {
          bigint: /** @type {EstreeBigIntLiteral} */ (node).bigint,
        },
        tag: locate(node.loc),
      };
      ...
      return {
        type: "PrimitiveExpression",
        primitive: /** @type {EstreeSimpleLiteral} */ (node).value,
        tag: locate(node.loc),
      };
    }
