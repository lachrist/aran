...
exports.scopeProgram = (node, source, _variables) => (
  ...
  _variables = Hoisting.hoistProgram(
    node,
    {
      type: Source.getType(source),
      strict: Source.isStrict(source),
      hoistings: hoistings}),
  set(
    evals,
    node,
...
const visitMethodDefinition = (node, source, derived) => or(
  ...
  visitClosure(
    ...
    (
      ...
      (
        derived ?
        "extendDerivedConstructor" :
        "extendConstructor") :
      "extendMethod")));

const visitClosure = (node, source, name) => (
  Throw.assert(
    (
    ...
    isEmpty(
      Hoisting.hoistClosure(
        node,
        {
          ...
          strict: Source.isStrict(source)})),
    null,
    `Variable escaped from Function`),
  source = Source[name](source),
  source = Source.extendHoisting(
    source,
    get(hoistings, node)),
  set(
    evals,
...
const visitors = {
  ...
  ArrowFunctionExpression: (node, source) => visitClosure(node, source, "extendArrow"),
  FunctionExpression: (node, source) => visitClosure(node, source, "extendFunction"),
  ...
  Property: (node, source) => or(
    ...
    (
      ...
      visitClosure(node.value, source, "extendMethod") :
