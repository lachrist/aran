...
const setAnnotation = (setter, node, value, _annotation) => (
  ...
  (
    ...
    (
      ...
      global_Reflect_apply(global_Map_prototype_set, annotations, [node, _annotation])) :
    setter(_annotation, value)));
...
const setHoisting = (node, variables) => setAnnotation(Annotation.setHoisting, node, variables);
  ...
  CallExpression: (node, source) => or(
    ...
    ArrayLite.reduce(
      ...
      (
        ...
        (
          setAnnotation(Annotation.setSource, node, source),
          true) :
        false))),
  // Other //
