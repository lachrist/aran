...
exports.assign1 = (scope, boolean, pattern, cache) => visit(scope, boolean, pattern)(cache);

exports.assign2 = (scope, boolean, pattern, expression) => Scope.cache(
  scope,
  "PatternAssign2Right",
  expression,
  visit(scope, boolean, pattern));

exports.assign3 = (scope, boolean, pattern, node) => Scope.cache(
  scope,
  "PatternAssign3Right",
  (
    node === null ?
...
const visit = (scope, boolean, pattern) => visitors[pattern.type](scope, boolean, pattern);
...
visitors.MemberExpression = (scope, boolean, pattern) => (cache1) => (
...
visitors.Identifier = (scope, boolean, pattern) => (cache) => Scope[boolean ? "initialize" : "write"](
  scope,
  pattern.name,
  Scope.get(scope, cache));

visitors.AssignmentPattern = (scope, boolean, pattern) => (cache) => Scope.cache(
  scope,
  ...
  Build.conditional(
    ...
    Scope.get(scope, cache)),
  visit(scope, boolean, pattern.left));
...
// > thisfoo === thisbar
// false
visitors.ObjectPattern = (scope, boolean, pattern) => (cache1) => (
  (
            ...
            (
              function self (list) { return (
                ...
                Build.sequence(
                  ...
                  self(list.cdr)) :
                visit(scope, boolean, pattern.properties[index].argument)(cache2))})
            (list))) :
        Scope.cache(
          scope,
            ...
            Scope.cache(
              ...
              Object.get(
                Object.objectify(
                  () => Scope.get(scope, cache1)),
                Scope.get(scope, cache2)),
              visit(scope, boolean, pattern.properties[index].value)),
            self(
              index + 1,
              {
                __proto__: null,
      ...
      function self (index) { return (
        ...
        Build.sequence(
          Scope.cache(
            ...
            visit(scope, pattern.properties[index].value, boolean)),
          self(index + 1)))})
    (0)));
...
visitors.ArrayPattern = (scope, boolean, pattern) => (cache1) => Scope.cache(
  scope,
            ...
            Scope.cache(
              ...
              (cache3) => Build.sequence(
                Build.apply(
                  ...
                  Build.primitive(void 0),
                  []),
                visit(scope, boolean, pattern.elements[index].argument)(cache3))) :
            Build.sequence(
              Scope.cache(
                scope,
                "PatternArrayChild",
                Object.get(
                  Build.apply(
                    ...
                    []),
                  Build.primitive("value")),
                visit(scope, boolean, pattern.elements[index])),
              self(index + 1))))})
      (0))));
