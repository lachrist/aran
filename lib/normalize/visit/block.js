            ...
            [
              ...
              Tree.Bundle(
                ArrayLite.map(
                  ...
                  (node) => Visit.Statement(scope, null, Completion._make_empty())))]) :
          Tree.Return(
...
exports.CATCH = (scope, node, context) => (
  ...
  (
    node.param === null ?
    Visit.BLOCK(scope, node.body, null) :
    Scope.BLOCK(
      scope,
      null,
      ...
      (scope) => Tree.Bundle(
        [
          ...
          Visit.Statement(scope, node.body, null)]))));
...
exports.PROGRAM = (scope, node, context, _completion) => (
  Throw.assert(node.type === "Program", null, `Invalid PROGRAM node`),
  context = global_Object_assign(
    {
      source: "script"},
    context),
  _completion = node.body[node.body.length - 1].type === "ExpressionStatement",
  scope = (
    (
