...
exports.Visit = (node, scope, completion, labels) => State._visit(node, [node, scope, completion, labels], visitors[node.type]);

const visitors = {__proto__:null};

visitors.ExpressionStatement = (node, scope, completion, labels) => Tree.Lift(
  (
...
visitors.EmptyStatement = (node, scope, lexic, labels) => Tree.Bundle([]);

// type Node = BlockStatement
visitors.BlockStatement = (node, scope, lexic, labels) => Tree.Lone(
  labels,
...
visitors.SwitchCase = (node, scope, lexic, labels) => Tree.If(
  labels,
  Tree.conditional(
    Scope.get(
      scope,
...
visitors.IfStatement = (node, scope, lexic, labels) => Tree.If(
  labels,
  Visit.node(node.test, scope, false, null),
  block(
    (
...
visitors.ReturnStatement = (node, scope, lexic, labels) => Tree.Return(
  (
...
visitors.ThrowStatement = (node, scope, lexic, labels) => Tree.Expression(
  Tree.throw(
    Visit.node(node.argument, scope, false, null)));

visitors.TryStatement = (node, scope, lexic, labels) => Tree.Try(
  labels,
  Scope.BLOCK(
    scope,
    false,
...
visitors.DebuggerStatement = (node, scope, lexic, labels) => Tree.Debugger();
...
visitors.FunctionDeclaration = (node, scope, lexic, labels) => Tree.Lift(
  Scope.write(
    scope,
    node.id.name,
    Closure.function(node, scope, false, null)));

visitors.ClassDeclaration = (node, scope, lexic, labels) => Tree.Lift(
  Scope.write(
    scope,
    node.id.name,
