...
exports._hoisted_context = {
  __proto__: exports._default_context,
  hoisted: true};
...
// const normal = (scope, nullable_node, completion, _nodes) => (
//   _nodes = (
//     node === null ?
//     [] :
//     extract(nullable_node)),
//   Scope.NORMAL(
//     scope,
...
exports.Visit = (scope, node, context) => State._visit(
  (
    (
      context.visited ||
      context.hoisted) ?
...
visitors.DebuggerStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  Tree.Debugger());
...
visitors.ThrowStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  Tree.Lift(
    Tree.throw(
...
visitors.ReturnStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  Tree.Return(
    (
...
const variable_declarator_visitor = (scope, node, kind) = Tree.Lift(
  Common.assign(
    scope,
    kind,
    node.id,
...
visitors.VariableDeclaration = (scope, node1, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  Tree.Bundle(
    ArrayLite.map(
      node1.declarations,
      (node2) => State._visit(node2, variable_declarator_visitor, [scope, node2, node1.kind]))));
...
visitors.ClassDeclaration = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  Tree.Lift(
    Scope.initialize(
    ...
    () => (
      ...
      State._visit(
        ...
        () => Scope.ImportInitialize(
          ...
          node.specifiers[0].source.value)) :
      Scope.ImportBox(
        scope,
        node.source.value,
        (box) => Tree.Bundle(
...
visitors.ExportAllDeclaration = (scope, node, context) => (
  context === Statment._hoisted_context ?
  Tree.Bundle([]) :
  Tree.Aggregate(scope.source));

visitors.ExportDefaultDeclaration = (scope, node, context) => (
  context === Statemente._hoisted_context ?
  (
    (
      (
    ...
    Statement.Visit(scope, node, Statement._hoisted_context) :
    Tree.Bundle([])) :
  Tree.Bundle(
    [
      (
        (
          node.declaration.type === "ClassDeclaration" &&
          node.declaration.id !== null) ?
        Statement.Visit(scope, node.declaration, Statement._default_context) :
        Tree.Bundle([])),
      Scope.Box(
        scope,
        false,
                ...
                (
                  ...
                  Common.class(
                    ...
                    node.declaration,
                    {
                      __proto__: Expression._default_context,
                      name: box}) :
                  Sceop.read(scope, node.declaration.id.name)) :
                Expression.visit(
                  scope,
                  node.declaration,
...
visitors.ExportNamedDeclaration = (scope, node, context) => (
  ...
  (
    ...
    Tree.Bundle([]) :
    Statement.Visit(scope, node.declaration, Statement._hoisted_context)) :
  (
    node.declaration !== null ?
...
visitors.BlockStatement = (scope, node, context) => (
  ...
  Tree.Lone(
    ...
    Scope.BLOCK(
      ...
      (scope) => Common.Body(scope, node.body, context.completion))));

visitors.LabeledStatement = (scope, node, context) => (
  context.hoisted ?
  Statement.Visit(scope, node.body, Statement._hoisted_context) :
  Statement.Visit(
    scope,
    node.body,
    {
...
visitors.IfStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
    ...
    Tree.If(
      ...
      State._visit(
        node.consequent.type === "BlockStatement" ? node.consequent : null,
        Scope.BLOCK,
        [
          scope,
          null,
      ...
      (
        ...
        Scope.BLOCK(
          ...
          (scope) => []) :
        State._visit(
          node.alternate.type === "BlockStatement" ? node.alternate : null,
          Scope.BLOCK,
          [
            scope,
            null,
...
visitors.TryStatement = (scope, node, context) => (
  context === Statement._hoisted_context ?
  Tree.Bundle([]) :
  complete(
    scope,
    context.completion,
    Tree.Try(
      ...
      State._visit(
        ...
        [
          scope,
          null,
          Query._get_block_hoisting(node.block.body),
          (scope) => Common.Body(scope, node.block.body, context.completion)]),
            ...
            (
              (scope) => State._visit(
                ...
                [
                  ...
                  (scope) => complete(
                    Common.Body(scope, node.handler.body.body, context.completion))]))))),
      (
        node.finalizer === null ?
        Scope.BLOCK(
          scope,
        ...
        State._visit(
          ...
          [
            scope,
            null,
            Query._get_block_hoisting(node.finalizer.body),
            (scope) => Common.Body(
              scope,
              node.finalizer,
              Completion._set_last(context.completion, false))])))));
...
// set 123
// 129319
visitors.WithStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
...
visitors.WhileStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
...
visitors.DoWhileStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
...
visitors.ForStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
        ...
        (closure) => (
          ...
          (
            ...
            (
              ...
              Tree.Bundle(
                [
                  Statement.Visit(scope, node.init, Statement._default_context),
                  closure(scope)]) :
              Tree.Lone(
                [],
                Scope.BLOCK(
      ...
      (
        ...
        (scope) => Tree.While(
          ...
          (
            ...
            (
              (scope) => State._visit(
                node.type === "BlockStatement" ? node : null,
                Scope.BLOCK,
                [
                  scope,
                  null,
...
visitors.ForInStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
                    ...
                    (scope) => Tree.Bundle(
                      [
                        ...
                        Tree.Lone(
                          [],
                          State._visit(
                            node.body === "BlockStatement" ? node.body : null,
                            Scope.BLOCK,
                            [
                              scope,
                              null,
...
visitors.SwitchStatement = (scope, node, context) => (
  context.hoisted ?
  Tree.Bundle([]) :
  complete(
    scope,
        ...
        (matched_box) => Tree.Lone(
          ...
          Scope.BLOCK(
            ...
            (scope) => Common.Switch(
              ...
              node.cases,
              context.completion,
              (scope, node, callback) => State._visit(
                node,
                () => (
                  node.test === null ?
                  Tree.Bundle(
