        ...
        (
          ...
          (
            ...
            Scope.box(
              ...
              "name",
              false,
              Tree.primitive(declaration.id.name),
              (box1) => Scope.box(
                scope,
                "super",
                false,
                ...
                (box2) => (
                  node.type ===
                State._visit(declaration.init, [scope, declaration.init, ]Expression.visit(scope, declaration.init, false, box)) :
            Expression.visit(scope, declaration.init, false, null))),
        node.kind !== "var"))));
    ...
    Scope.Box(
      ...
      (box2) => Tree.Lift(
        Scope.write(
          scope,
          node.id.name,
          Closure.function(scoe, node, box1, box2)))))) :
State._visit(node, [scope, node, completion, labels], visitors[node.type]));
...
visitors.ClassDeclaration = (scope, node, completion, labels) => Tree.Lift(
  Scope.write(
    ...
    Scope.box(
      ...
      (name_box) => Class.class(node, scope, {
        __proto__: null,
        kind: null,
        name: name_box,
        super: null})
