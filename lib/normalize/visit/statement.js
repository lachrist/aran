...
const block = (statement, scope, lexic) => ArrayLite.flatMap(Query._rearrange(statement), (statement, index, statements) => {
  const labels = [];
  while (statement.type === "LabeledStatement") {
    labels[labels.length] = statement.label.name;
    statement = statement.body;
  }
  return State.visit(statement, () => visitors[statement.type](statement, scope, Lexic.child(lexic, index, statements, labels), labels));
});
...
visitors.BlockStatement = (node, scope, lexic, labels) => Build.Lone(
  ...
  Scope.EXTEND_STATIC(
    scope,
    Query._shallow_hoisting(node.body),
    (scope) => ArrayLite.flatMap(
      Query._sort(node.body),
...
visitors.LabeledStatement = ({label:{name:label}, body:statement}, scope, lexic, labels) => Visit.Node(
  statement,
  scope,
  Lexic.label(lexic, label),
  ArrayLite.add(labels, label));
  (
