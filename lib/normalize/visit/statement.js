...
const block = (statements, scope, lexic) => Scope.EXTEND_STATIC(
  scope,
  Query._shallow_hoisting(statements),
  (scope) => ArrayLite.flatMap(
    ...
    (statement, index, statements) => Statement.Visit(
      statement,
      scope,
      Lexic.child(lexic, index, statements),
      [])));

const switch_block = (switch_cases, scope, lexic, box1, box2) => Scope.EXTEND_STATIC(
  scope,
  Query._shallow_hoisting(
    ArrayLite.flatMap(
      switch_cases,
      (switch_case) => switch_case.consequent)),
  (scope) => ArrayLite.concat(
    ArrayLite.flatMap(
      ArrayLite.flatMap(
        switch_cases,
        (switch_case) => ArrayLite.filter(switch_case.consequent, Query._is_function_declaration)),
    ...
    ArrayLite.flatMap(
      ...
      (switch_case) => (
        ...
        ArrayLite.concat(
          ...
          ArrayLite.flatMap(
            ArrayLite.filter(switch_case.consequent, Query, Query._is_not_function_declaration),
...
visitors.BlockStatement = (node, scope, lexic, labels) => Build.Lone(
  labels,
  block(node.body, scope, lexic));

visitors.ExpressionStatement = (node, scope, lexic, labels) => Build.Lift(
...
visitors.IfStatement = (node, scope, lexic, labels) => Build.If(
  labels,
  Visit.node(node.test, scope, false, null),
  block(
    (
...
visitors.FunctionDeclaration = (node, scope, lexic, labels) => Build.Lift(
  Scope.write(
    scope,
    node.id.name,
    Closure.function(node, scope, false, null)));

visitors.ClassDeclaration = (node, scope, lexic, labels) => Build.Lift(
  Scope.write(
    scope,
    node.id.name,
