...
exports.Visit = (scope, node, context) => (
...
visitors.ContinueStatement = (scope, node, context, _label) => (
  _label = (
    ...
    null :
    node.label.name),
  Throw.assert(!ArrayLite.has(context.labels, _label), `Break label used as continue label`),
  Tree.Continue(
    Stratum._base(_label)));
...
visitors.VariableDeclaration = (scope, node1, context) => Tree.Bundle(
  ArrayLite.map(
    node.declarations,
    (node1) => Visit.Declarator(
      scope,
      node1,
      {kind:node1.kind})));
