...
exports.ThisExpression = (node, scope, dropped, cache) => Scope.read(scope, "this");

exports.ArrayExpression = (node, scope, dropped, cache) => (
  ArrayLite.every(
    node.elements,
    (node) => (
      node !== null &&
...
exports.ObjectExpression = (node, scope, dropped, cache) => (
  (
    (closure) => (
      ArrayLite.every(
        node.properties,
...
// > o.x.name
// ''
exports.SequenceExpression = (node, scope, dropped, cache) => (
  (
    function self (index) { return (
      index === (node.expressions.length - 1) ?
      Visit.node(node.expressions[index], scope, dropped, null) :
      Build.sequence(
        Visit.node(node.expressions[index], scope, true, null),
        self(index + 1)))})
  (0));

exports.UnaryExpression = (node, scope, dropped, cache) => (
  (
...
exports.BinaryExpression = (node, scope, dropped, cache) => Build.binary(
  node.operator,
...
//     at f (repl:1:52)
exports.AssignmentExpression = (node, scope, dropped, cache) => (
...
// 'yolo1'
// > 
exports.UpdateExpression = (node, scope, dropped, cache) => (
  node.argument.type === "MemberExpression" ?
  Scope.cache(
    scope,
...
exports.LogicalExpression = (node, scope, dropped, cache) => Scope.cache(
  scope,
  ...
  (cache) => Build.conditional(
    ...
    (
      node.operator === "&&" ?
      Visit.node(node.right, scope, dropped, null) :
      Scope.get(scope, cache)),
    (
      node.operator === "||" ?
      Visit.node(node.right, scope, dropped, null) :
...
exports.ConditionalExpression = (node, scope, dropped, cache) => Build.conditional(
  Visit.node(node.test, scope, false, null),
  Visit.node(node.consequent, scope, dropped, cache),
  Visit.node(node.alternate, scope, dropped, cache));

exports.NewExpression = (node, scope, dropped, cache) => (
  ArrayLite.every(
    node.arguments,
...
// > f()
// 'foo'
exports.CallExpression = (node, scope, dropped, cache) => (
  ArrayLite.every(
    node.arguments,
    (argument) => argument.type !== "SpreadElement") ?
  (
...
exports.MemberExpression = (node, scope, dropped, cache) => Scope.cache(
  scope,
...
exports.MetaProperty = (node, scope, dropped, cache) => Scope.read(scope, "new.target");

exports.Identifier = (node, scope, dropped, cache) => Scope.read(scope, node.name);

exports.Literal = (node, scope, dropped, cache) => (
  node.regex ?
  Build.construct(
...
exports.TemplateLiteral = (node, scope, dropped, cache) => (
  (
    function self (index) { return (
...
exports.TaggedTemplateExpression = (node, scope, dropped, cache) => Build.apply(
...
exports.ArrowFunctionExpression = (node, scope, dropped, cache) => Closure.arrow(node, scope, cache);

exports.FunctionExpression = (node, scope, dropped, cache) => Closure.function(node, scope, cache);
