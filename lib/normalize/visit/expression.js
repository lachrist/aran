...
// 'yolo1'
// > 
exports.UpdateExpression = ({operator:operator, arguments:expression}, scope, dropped, cache) => (
  expression.type === "MemberExpression" ?
          ...
          Scope.write(
            ...
            (
              ...
              Build.binary(
                ...
                Build.primitive(1)))),
          Scope.get(scope, cache)))) :
    Throw("Invalid left-hand side update")));
...
exports.MetaProperty = ({meta:{name:identifier1}, property:{name:identifier2}}, scope, dropped, cache) => (
  (
    identifier1 !== "new" ||
    identifier2 !== "target")
  Throw("According to the ESTree specification, only new.target is currently used for meta properties") :
...
exports.ArrowFunctionExpression = ({params:patterns, body:closure_body, generator:is_generator, async:is_async, expression:is_expression}, scope, dropped, cache, _names) => (
  is_generator ?
  Throw("Unfortunately, Aran does not support generator arrows (yet)...") :
  (
    is_async ?
    Throw("Unfortunately, Aran does not support asynchronous arrows (yet)...") :
    (
      _names = (
        is_expression ?
        [] :
...
exports.FunctionExpression = ({id:{name:nullable_identifier}={name:null}, params:patterns, body:body, generator:is_generator, async:is_async, expression:is_expression}, scope, dropped, cache) => (
  is_generator ?
  Throw("Unfortunately, Aran does not support generator functions (yet)...") :
  (
    Throw("Unfortunately, Aran does not support asynchronous functions (yet)...") :
    (
...
exports.YieldExpression = ({argument:nullable_expression, delegate:is_delegate}, scope, dropped, cache) => Throw("Unfortunately, Aran does not support yield expressions (yet)...");

exports.AwaitExpression = ({argument:expression}, scope, dropped, cache) => Throw("Unfortunately, Aran does support await expressions (yet)...");

exports.ClassExpression = ({id:{name:nullable_identifier}={name:null}, superClass:nullable_expression, body:class_body}, scope, dropped, cache) => Throw("Unfortunately, Aran does not support class expressions (yet)...");
