...
visitors.TaggedTemplateExpression = (scope, node, context) => (
  (
    (callback) => (
      ...
      Visit.member(scop, node.tag, {callback}) :
      (
        (
          node.tag.type === "ChainExpression" &&
          node.tag.expression.type === "MemberExpression") ?
        Visit.member(scope, node.tag.expression, {callback}) :
        callback(
  ...
  Builtin.concat(
    ArrayLite.map(
      ...
      (element) => (
        ...
        (
          element.type === "SpreadElement" ?
          Visit.expression(scope, element.argument, null) :
          Builtin.construct_array(
            [
              Visit.expression(scope, element, null)]))))));
    ...
    _closure2 = (node, _key_box) => (// console.assert(is_not_proto_property(property) && is_not_spread_element_node(property))
      ...
      [
        Scope.box(
          ...
          (
            node.computed ?
            Visit.expression(scope, node.key, null) :
            Tree.primitive(
              (
      ...
      ArrayLite.reduce(
        ...
        (expression, property, _expression, _descriptor) => (
          ...
          Builtin.assign(
            expression,
            [
              Visit.expression(scope, property.argument, null)],
            true,
            Builtin._target_result) :
          (
  ...
  (box) => (
    ...
    Builtin.fork_nullish(
      ...
      Builtin.get(
        ...
        (
          node.computed ?
          Visit.expression(scope, node.property, null) :
