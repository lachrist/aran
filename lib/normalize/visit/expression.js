...
exports.visit = (scope, node, context) => (console.log(node), State._visit(node, [scope, node, context], visitors[node.type]));
    ...
    Scope.box(
      ...
      (object_box) => (
        (
          node.operator === "=" &&
          context.dropped &&
          !Scope._is_strict(scope)) ? // this could go away but kept for readability
        // Special Case #2:
        Mop.set(
          Mop.convert(
          ...
          (
            node.left.computed ?
            Expression.visit(scope, node.left.property, Expression._default_context) :
            Tree.primitive(scope, node.left.property.name)),
          Expression.visit(scope, node.right, Expression._default_context),
          null,
          Mop._success_result,
          false) :
          ...
          (key_box) => (
            _expression = (
              ...
              Tree.binary(
                ...
                Visit.expression(scope, node.right, Expression._default_context))),
            (
              context.dropped ?
              // Special Case #3
              Meta.set(
        ...
        Scope.box(
          ...
          (value_box) => Tree.sequence(
            Scope.write(
              ...
              Scope.get(scope, value_box)),
            Scope.get(scope, value_box))))) :
    Common.assign(scope, node.left, node.right, false)));
//
