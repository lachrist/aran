...
Pattern._resolve_circular_dependencies({
      ...
      (scope) => Tree.Bundle(
        [
          ...
          Tree.Lift(
            Pattern.assign(
              scope,
              Acorn.parse(`x = null;`).body[0].expression.left,
  ...
  Assert.throws(
    () => Scope.EXTEND_STATIC(
      ...
      (scope) => Tree.Lift(
        Pattern.assign(
          scope,
          Acorn.parse(`(123).foo = null;`).body[0].expression.left,
          Lang.parse_expression(`123`),
          true))),
      ...
      Lang._match_block(
        Scope.EXTEND_STATIC(
          ...
          (scope) => Tree.Lift(
            Pattern.assign(
              scope,
              Acorn.parse(`(123)${property} = null;`).body[0].expression.left,
              Lang.parse_expression(`123`),
              false))),
        Lang.PARSE_BLOCK(
          (
  ...
  const test = (hoisting, code1, code2, code3) => Lang._match_block(
    Scope.EXTEND_STATIC(
      ...
      (scope) => Tree.Lift(
        Pattern.assign(
          scope,
          Acorn.parse(`(${code1} = null);`).body[0].expression.left,
