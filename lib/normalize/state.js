...
// type Result = *
exports._run_session = (state, callback, array) => {
  if (session !== null) {
...
// type Callback = () => Result
// type Result = *
exports._visit = (nullable_estree_node, callback, array) => {
  if (nullable_estree_node === null) {
    return global_Reflect_apply(callback, void 0, array);
  }
  const serial = session.serial;
  session.serial = session.state.nodes.length;
  session.state.nodes[session.state.nodes.length] = nullable_estree_node;
  const result = global_Reflect_apply(callback, void 0, array);
...
exports._register_scope = (scope) => {
  session.state.scopes[session.serial] = scope;
};
