...
State._run_session({nodes:[], serials:new Map(), scopes:{__proto__:null}}, [], () => {
  // _extend_eval && _get_eval //
  Assert.deepEqual(Base._get_dynamic(Outer._make_root(), (box, nullable_box) => { Assert.fails() }), null);
  Assert.deepEqual(Outer.EXTEND_STATIC(Outer._make_root(), (scope1) => {
    return Meta.Box(scope1, "foo", false, Tree.primitive("object-box"), (box1) => {
      return Meta.Box(scope1, "bar", false, Tree.primitive("unscopables-box"), (box2) => {
        const scope2 = Base._extend_dynamic(scope1, box1, box2);
        Assert.deepEqual(Base._get_dynamic(scope2, (box, nullable_box) => {
          Assert.deepEqual(box, box1);
          Assert.deepEqual(nullable_box, box2);
          return true;
        }), [box1, box2]);
