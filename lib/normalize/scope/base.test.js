...
State._run_session({nodes:[], serials:new Map(), evals:{__proto__:null}}, [], () => {
  ...
  Assert.deepEqual(Outer.EXTEND_STATIC(null, (scope1) => {
    return Meta.Box(scope1, "object_box", true, Lang.primitive("object-box"), (box1) => {
      return Meta.Box(scope1, "unscopables_box", true, Lang.primitive("unsopables-box"), (box2) => {
  ...
  Assert.throws(() => Base.lookup(null, "this", null, {
    on_miss: () => Assert.fail(),
    on_dead_hit: () => Assert.fail(),
    on_dynamic_hit: () => Assert.fail(),
    on_live_hit: () => Assert.fail()
  }), new Error("Missing special identifier"));
  Assert.throws(() => Base.lookup(null, "new.target", null, {
    on_miss: () => Assert.fail(),
    on_dead_hit: () => Assert.fail(),
    on_dynamic_hit: () => Assert.fail(),
    on_live_hit: () => Assert.fail()
  ...
  Assert.deepEqual(Outer.EXTEND_STATIC(null, (scope1) => {
    ...
    return Lang.Bundle([
      ...
      Meta.Box(scope1, "object_box", true, Lang.primitive("object-box"), (box) => {
