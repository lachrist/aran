...
State._run_session({nodes:[], serials:new Map(), evals}, [], () => {
  // is_strict //
  Assert.deepEqual(Inner._is_strict(null), false);
  Assert.deepEqual(Inner._is_strict(Inner._extend_use_strict(null)), true);
  Assert.deepEqual(Inner._is_strict(Inner._extend_closure(Inner._extend_use_strict(null))), true);
  // get_depth //
  Assert.deepEqual(Inner.EXTEND_STATIC(null, (scope) => {
  ...
  // // uninitialized //
  Assert.throws(() => Inner.EXTEND_STATIC(null, (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), undefined);
    return Lang.Bundle([]);
  }), new Error("Uninitialized identifier"));
  // declare //
  Assert.throws(() => Inner._declare(null, "x", "tag"), new Error("Invalid scope for declaration"));
  Assert.throws(() => Inner._declare(Inner._extend_use_strict(null), "x", "tag"), new Error("Invalid scope for declaration"));
  Assert.deepEqual(Inner.EXTEND_STATIC(null, (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), undefined);
  ...
  Assert.throws(() => Inner._is_declared(null, "x"), new Error("Invalid scope for declaration query"));
  Assert.throws(() => Inner._is_declared(Inner._extend_use_strict(null), "x"), new Error("Invalid scope for declaration query"));
  Assert.deepEqual(Inner.EXTEND_STATIC(null, (scope) => {
  ...
  // get_dynamic_frame //
  Assert.deepEqual(Inner._get_dynamic_frame(Inner._extend_use_strict(Inner._extend_dynamic(null, "foo")), () => true), "foo");
  Assert.deepEqual(Inner._get_dynamic_frame(Inner._extend_use_strict(Inner._extend_dynamic(null, "foo")), () => false), null);
  // initialize //
  Assert.throws(() => Inner.initialize(null, "x"), new Error("Invalid scope for initialization"));
  Assert.throws(() => Inner.initialize(Inner._extend_use_strict(null), "x"), new Error("Invalid scope for initialization"));
  Assert.deepEqual(Inner.EXTEND_STATIC(null, (scope) => {
  ...
  // parameter //
  Assert.deepEqual(Inner.parameter(null, "this"), Parser.parse("THIS"));
  ...
  // eval //
  Assert.deepEqual(Inner.EXTEND_STATIC(Inner._extend_use_strict(Inner._extend_dynamic(Inner._extend_closure(null), "dyn")), (scope) => {
  ...
  // is_eval //
  Assert.deepEqual(Inner._is_eval(Inner._extend_use_strict(null)), false);
  Assert.deepEqual(Inner._is_eval(Inner._deserialize(evals.null)), true);
  // _deserialize //
  [evals.null, JSON.parse(JSON.stringify(evals.null))].forEach((frame_array) => {
    Assert.deepEqual(Inner.EXTEND_STATIC(Inner._deserialize(frame_array), (scope) => {
  ...
  // eval-escaped //
  Assert.deepEqual(Inner.EXTEND_STATIC(null, (scope1) => {
