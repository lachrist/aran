...
State._run_session({nodes:[], serials:new Map(), scopes}, [], () => {
  ...
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    ...
    return Tree.Bundle([
      Tree.Lone([], Inner.EXTEND_STATIC(scope, (scope) => {
        ...
        Assert.equal(Inner._get_depth(Inner._extend_binding(scope, "foo", "bar")), 2);
        return Tree.Bundle([]);
      }))
    ]);
  ...
  // _declare //
  Assert.throws(() => Inner._declare(Inner._make_root(), "x", "tag"), new Error("Missing binding scope frame for variable declaration"));
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), true);
    Assert.equal(Inner._declare(scope, "x", "tag"), false);
    Assert.equal(Inner._declare(Inner._extend_dynamic(scope, "dyn")), "dyn");
    // Assert.throws(() => Inner._declare(scope, "x", "tag"), new Error("Duplicate declaration"));
    return Tree.Lone([], Inner.EXTEND_EMPTY(scope, (scope) => {
      Assert.equal(Inner._declare(scope, "x", "tag"), false);
      Assert.equal(Inner._declare(Inner._extend_dynamic(scope, "dyn")), "dyn");
  ...
  //     })),
  //     Assert.throws(() => Inner._get_tag(scope, "x"), new Error("Missing tag"))
  //   ][0];
  // }), Lang.PARSE_BLOCK(`{123;}`), Assert);
  // initialize //
  Assert.throws(() => Inner.initialize(Inner._make_root(), "x"), new global.Error("Missing binding scope frame for variable initialization"));
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), true);
    Assert.equal(Inner._declare(scope, "y", "tag"), true);
    const aran_expression = Inner.initialize(Inner._extend_binding(scope, "foo", "bar"), "x", Tree.primitive(123));
    Assert.throws(() => Inner.initialize(scope, "x", Tree.primitive(456)), new global.Error("Duplicate variable initialization"));
    Assert.throws(() => Inner.initialize(scope, "z", Tree.primitive(456)), new global.Error("Missing variable declaration for variable initialization"));
    return Tree.Bundle([
      Tree.Lift(aran_expression),
      Tree.Lift(Inner.initialize(Inner._extend_dynamic(scope, "dynamic"), "t", Tree.primitive(789), (frame, identifier, expression) => {
        Assert.equal(frame, "dynamic");
        Assert.equal(identifier, "t");
        return expression;
      })),
      Tree.Lone([], Inner.EXTEND_EMPTY(scope, (scope) => {
        return Tree.Lift(Inner.initialize(scope, "y", Tree.primitive(0)));
      }))
    ]);
  }), Lang.PARSE_BLOCK(`{
    let $x, _y, $y;
  ...
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    ...
    return Tree.Bundle([
      Tree.Lift(Inner.initialize(scope, "x", Tree.primitive(123))),
      Tree.Lift(Inner.lookup(Inner._extend_binding(scope, "foo", "bar"), "x", "ctx", {
        on_miss: () => Assert.fail(),
        on_dynamic_frame: () => Assert.fail(),
        on_dead_hit: () => Assert.fail(),
  ...
  Lang._match_block(Inner.EXTEND_STATIC(false, (scope1) => {
    Assert.equal(Inner._declare(scope1, "x", "tag"), true);
  ...
  // horizon //
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    scope = Inner._extend_binding(scope, "foo", "bar");
    Assert.equal(Inner._declare(scope, "x", "tag"), true);
    Assert.equal(Inner._declare(scope, "y", "tag"), true);
    Assert.throws(() => Inner._get_foreground(scope), new global.Error("Missing horizon scope frame for foreground query"));
    Assert.throws(() => Inner._get_background(scope), new global.Error("Missing horizon scope frame for background query"));
    scope = Inner._extend_horizon(scope);
    return Tree.Lone([], Inner.EXTEND_STATIC(scope, (scope) => {
      scope = Inner._extend_binding(scope, "foo", "qux");
      Assert.equal(Inner._declare(scope, "z", "tag"), true);
      Assert.equal(Inner._declare(scope, "t", "tag"), true);
      Assert.deepEqual(Inner._get_foreground(scope), ["z", "t"]);
      Assert.deepEqual(Inner._get_binding(Inner._get_background(scope), "foo"), "bar");
      return Tree.Lift(Tree.primitive(123));
    }));
  }), Lang.PARSE_BLOCK(`{
    let $x, $y;
  ...
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), true);
    Assert.equal(Inner._declare(scope, "y", "tag"), true);
    return Tree.Lone([], Inner.EXTEND_STATIC(Inner._extend_closure(Inner._extend_binding(scope, "foo", "bar")), (scope) => {
      Assert.equal(Inner._declare(scope, "x", "tag"), true);
      Assert.equal(Inner._declare(scope, "z", "tag"), true);
      return Tree.Lift(Inner.eval(scope, Tree.primitive(123)));
    }));
  }), Lang.PARSE_BLOCK(`{
    let $x, $y, _y;
    _y = false;
  ...
  Assert.equal(Inner._get_binding(global.JSON.parse(scopes.null), "foo"), "bar");
