...
State._run_session({nodes:[], serials:new Map(), evals}, [], () => {
  ...
  Assert.throws(() => Inner._declare(Inner._make_eval([]), "x", "tag"), new Error("Cannot declare beyond eval frames"));
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    Assert.equal(Inner._declare(scope, "x", "tag"), undefined);
  ...
  Assert.throws(() => Inner.initialize(Inner._make_eval([]), "x"), new Error("Cannot initialized beyond eval frames"));
  Lang._match_block(Inner.EXTEND_STATIC(Inner._make_root(), (scope) => {
    Assert.throws(() => Inner.initialize(Inner._extend_use_strict(scope), "x", Tree.primitive(123)), new Error("Missing matching static scope frame for initialization"));
    Assert.equal(Inner._declare(scope, "x", "tag"), undefined);
  ...
  [evals.null, JSON.parse(JSON.stringify(evals.null))].forEach((frame_array) => {
    Lang._match_block(Inner.EXTEND_STATIC(Inner._make_eval(frame_array), (scope) => {
