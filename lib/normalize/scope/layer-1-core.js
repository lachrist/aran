...
const initialize = (scope, kind, identifier, maybe, callbacks) => {
  while (scope !== ROOT_SCOPE) {
    if (ArrayLite.includes(scope.kinds, kind)) {
      if (scope.type === DYNAMIC_FRAME_TYPE) {
        return callbacks.dynamic(scope.frame);
      }
      // console.assert(scope.type === STATIC_FRAME_TYPE);
      if (global_Reflect_getOwnPropertyDescriptor(scope.frame, identifier) === void 0) {
      ...
      if (binding.kind !== kind) {
        Throw.abort(null, `Kind mismatch during variable initialization`);
      }
      if (binding.initialized !== false) {
        Throw.abort(null, `Duplicate variable initialization`);
      }
      binding.initialized = maybe ? null : true;
      if (maybe) {
        binding.has_dynamic_deadzone = true;
      }
      let nullable_expression = null;
      if (binding.has_dynamic_deadzone) {
        nullable_expression = Tree.__write__(Stratum._meta(identifier), Tree.primitive(true));
      }
      return callbacks.static(Stratum._base(identifier), nullable_expression);
    }
    scope = scope.parent;
  }
...
const import_initialize_callback_prototype = {
  dynamic: (frame) => {
    Throw.abort(null, `Cannot import variable on dynamic frame`);
  },
  static: function (stratified_identifier, nullable_expression) {
    let statement = Tree.__Import__(stratified_identifier, this.source);
    if (nullable_expression !== null) {
      statement = Tree.Bundle([statement, Tree.Lift(nullable_expression)]);
    }
    return statement;
  }
};

exports.ImportInitialize = (scope, kind, identifier, source, maybe) => initialize(scope, kind, identifier, maybe, {
  __proto__: import_initialize_callback_prototype,
  source
});

const initialize_callback_prototype = {
  dynamic: (frame) => ({
    static: false,
    value: frame
  }),
  static: function (stratified_identifier, nullable_expression) {
    let expression = Tree.__write__(stratified_identifier, this.expression);
    if (nullable_expression !== null) {
      expression = Tree.sequence(expression, nullable_expression);
    }
    return {
      static: true,
      value: expression
...
exports._initialize = (scope, kind, identifier, expression, maybe) => initialize(scope, kind, identifier, maybe, {
  __proto__: initialize_callback_prototype,
  expression
});
