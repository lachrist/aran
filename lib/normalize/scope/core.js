...
const extend = (nullable_scope, is_closure, is_use_strict, dynamic, kontinuation) => {
  ...
  for (let identifiers in scope.bindings) {
    ...
    if (scope.bindings[identifier].has_dynamic_deadzone) {
      ...
      aran_statement_array_2[aran_statement_array_2.length] = Build.Expression(Build.write(meta(identifier)), Build.primitive(false));
    }
  }
...
// exports.initialize_optional = (scope, identifier, aran_expression) => {
//   // console.assert(scope.misses !== null);
//   const aran_statement = Build.Expression(initialize(scope, identifier, aran_expression));
//   scope.optionals.set(aran_statement, identifier);
//   return aran_statement;
// };
...
exports.lookup = (scope, identifier, {on_miss, on_hit, on_deadzone, on_dynamic}, context) => {
  ...
  const loop = (nullable_scope, enclosed) => {
    ...
    if (identifier in nullable_scope.bindings) {
      ...
      return Build.conditional(Build._read(meta(identifier)), on_hit(context, tag, access), on_deadzone(context));
    }
