...
// type Binding = Maybe (Initialized, HasDynamicDeadzone, Writable)
...
exports._is_writable = (scope, stratified_identifier) => {
  if (stratified_identifier[0] === "_") {
    return false;
  }
  // console.assert(aran_identifier[0] === "$");
  const identifier = global_Reflect_apply(global_String_prototype_substring, stratified_identifier, [1]);
  while (scope !== null) {
    if (scope.type === MUTABLE_STATIC_FRAME_TYPE || scope.type === IMMUTABLE_STATIC_FRAME_TYPE) {
      if (identifier in scope.frame) {
        return scope.frame[identifier];
      }
    }
    scope = scope.parent;
  }
  throw new global_Error("Missing identifier");
};
...
exports._declare = (scope, identifier, writable) => {
...
// exports.initialize = (scope, identifier, writable) => {
//   while (scope !== null) {
...
exports.lookup = (scope, identifier, context, {on_miss, on_dead_hit, on_live_hit, on_dynamic_frame}) => {
  ...
  const loop = (scope, escaped) => {
    ...
    if (identifier in scope.frame) {
      ...
      if (scope.frame[identifier].initialized) {
        return on_live_hit(context, scope.frame[identifier].writable, access);
      }
      if (!escaped) {
        return on_dead_hit(context, scope.frame[identifier].writable);
      }
      ...
      return Build.conditional(
        ...
        on_live_hit(context, scope.frame[identifier].writable, access),
        on_dead_hit(context, scope.frame[identifier].writable));
    }
    if (scope.type === MUTABLE_STATIC_FRAME_TYPE) {
