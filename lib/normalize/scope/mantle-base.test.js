...
const MantleBase = require("./mantle-base.js");

State._run_session({nodes:[], serials:new Map(), scopes:{__proto__:null}}, [], () => {
  // DeclareInitialize //
  Lang._match_block(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope) => {

    DeclareInitialize()
  }), Lang.PARSE_BLOCK(`{}`), Assert);
  // on_miss //
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    return Tree.Bundle([
      Tree.Lift(MantleBase.lookup(scope1, "x", "right", {
  ...
  // on_live_hit //
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    MantleBase._declare(scope1, "x", true);
    return Tree.Bundle([
      Tree.Lift(MantleBase.initialize(scope1, "x", Tree.primitive(123))),
      Tree.Lift(MantleBase.lookup(scope1, "x", "right", {
        on_miss: () => Assert.fail(),
  ...
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    ...
    return Tree.Bundle([
      ...
      Tree.Lift(MantleBase.initialize(scope1, "x", Tree.primitive(456)))
    ]);
  }), Lang.PARSE_BLOCK(`{
    let $$x;
    123;
  ...
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    return MantleMeta.Box(scope1, "object_box", true, Tree.primitive("object-box"), (box1) => {
      return MantleMeta.Box(scope1, "unscopables_box", true, Tree.primitive("unsopables-box"), (box2) => {
        const scope2 = MantleBase._extend_dynamic(scope1, box1, box2);
        return Tree.Lift(MantleBase.lookup(scope2, "x", "right", {
  ...
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    return MantleMeta.Box(scope1, "object_box", true, Tree.primitive("object-box"), (box1) => {
      const scope2 = MantleBase._extend_dynamic(scope1, box1, null);
      return Tree.Lift(MantleBase.lookup(scope2, "x", "right", {
        on_miss: (scope3, identifier, right) => {
          Assert.deepEqual(scope3, scope2);
          Assert.deepEqual(identifier, "x");
  ...
  Assert.throws(() => MantleBase.lookup(OuterCore._make_root(), "this", null, {
    on_miss: () => Assert.fail(),
    on_dead_hit: () => Assert.fail(),
    on_dynamic_hit: () => Assert.fail(),
    on_live_hit: () => Assert.fail()
  }), new Error("Missing special identifier"));
  Assert.throws(() => MantleBase.lookup(OuterCore._make_root(), "new.target", null, {
    on_miss: () => Assert.fail(),
    on_dead_hit: () => Assert.fail(),
    on_dynamic_hit: () => Assert.fail(),
    on_live_hit: () => Assert.fail()
  ...
  Assert.deepEqual(OuterCore.EXTEND_STATIC(OuterCore._make_root(), (scope1) => {
    MantleBase._declare(scope1, "this", true);
    MantleBase._declare(scope1, "new.target", true);
    Assert.throws(() => MantleBase.lookup(scope1, "this", null, {
      on_miss: () => Assert.fail(),
      on_dead_hit: () => Assert.fail(),
      on_dynamic_hit: () => Assert.fail(),
      on_live_hit: () => Assert.fail()
    }), new Error("Special identifier in deadzone"));
    Assert.throws(() => MantleBase.lookup(scope1, "new.target", null, {
      on_miss: () => Assert.fail(),
      on_dead_hit: () => Assert.fail(),
      on_dynamic_hit: () => Assert.fail(),
      on_live_hit: () => Assert.fail()
    ...
    return Tree.Bundle([
      Tree.Lift(MantleBase.initialize(scope1, "this", Tree.primitive(123))),
      Tree.Lift(MantleBase.initialize(scope1, "new.target", Tree.primitive(456))),
      MantleMeta.Box(scope1, "object_box", true, Tree.primitive("object-box"), (box) => {
        const scope2 = MantleBase._extend_dynamic(scope1, box, null);
        return Tree.Bundle([
          Tree.Lift(MantleBase.lookup(scope2, "this", null, {
            on_miss: () => Assert.fail(),
            on_dead_hit: () => Assert.fail(),
            on_dynamic_hit: () => Assert.fail(),
          ...
          Tree.Lift(MantleBase.lookup(scope2, "new.target", null, {
            on_miss: () => Assert.fail(),
            on_dead_hit: () => Assert.fail(),
