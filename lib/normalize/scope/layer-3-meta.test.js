...
State._run_session({nodes:[], serials:new Map(), scopes:{__proto__:null}}, [], () => {
  ...
  Lang._match_block(Meta.EXTEND_STATIC(Meta._make_root(), (scope) => {
    return Tree.Bundle([
      Tree.Lift(Meta.box(scope, "x", false, Tree.builtin("eval"), (box) => {
        Assert.throws(() => Meta.set(scope, box, Tree.primitive(123)), new Error("Cannot set a primitive/builtin/base box"));
        return Meta.get(scope, box);
      }))
    ]);
  }), Lang.PARSE_BLOCK(`{
  ...
  Lang._match_block(Meta.EXTEND_STATIC(Meta._make_root(), (scope) => {
    return Tree.Bundle([
      Tree.Lift(Meta.box(scope, "x", false, Tree.primitive("foo"), (box) => {
        Assert.throws(() => Meta.set(scope, box, Tree.primitive("bar")), new Error("Cannot set a primitive/builtin/base box"));
        return Meta.get(scope, box);
      }))
    ]);
  }), Lang.PARSE_BLOCK(`{
  ...
  Lang._match_block(Meta.EXTEND_STATIC(Meta._make_root(), (scope) => {
    return Tree.Bundle([
      Tree.Lift(Meta.box(scope, "x", false, Tree.primitive(123), (box) => {
        Assert.throws(() => Meta.set(scope, box, Tree.primitive(456)), new Error("Cannot set a primitive/builtin/base box"));
        return Meta.get(scope, box);
      }))
    ]);
  }), Lang.PARSE_BLOCK(`{
  ...
  Lang._match_block(Meta.EXTEND_STATIC(Meta._make_root(), (scope) => {
    return Tree.Bundle([
      Tree.Lift(Meta.box(scope, "x", false, Tree.sequence(Tree.primitive(123), Tree.primitive(456)), (box) => {
        Assert.throws(() => Meta.set(scope, box, Tree.primitive(789)), new Error("Cannot set a primitive/builtin/base box"));
        return Meta.get(scope, box);
      }))
    ]);
  }), Lang.PARSE_BLOCK(`{
  ...
  // Write //
  Lang._match_block(Meta.EXTEND_STATIC(Meta._make_root(), (scope) => {
    Meta._declare_base(scope, "y", true);
    return Tree.Bundle([
