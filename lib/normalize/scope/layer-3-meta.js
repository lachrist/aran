...
exports.ImportInitialize = Split.ImportInitializeBase;
...
const declare_initialize = ({scope, writable, identifier}, expression) => {
  identifier = declare(scope, writable, identifier);
  return {
...
const make = (scope, writable, identifier, expression, optimistic_callback, pessimistic_callback) => { // A
  ...
  const result = writable ? declare_initialize(context, expression) : Tree._dispatch_expression(dispatch_callback_object_1, context, expression);
...
exports.ImportBox = (scope, identifier, source, callback) => {
  identifier = declare(scope, "import", identifier);
  return Tree.Bundle(Split.ImportInitializeMeta(scope, "import", identifier, source, false), callback({
    type: META_IDENTIFIER_TYPE,
