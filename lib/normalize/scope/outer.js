...
exports.EXTEND_STATIC = (scope, base_kind_array, callback) => Inner.EXTEND_STATIC(
  scope,
  ArrayLite.concat(base_kind_array, [META_LET_KIND, META_CONST_KIND]),
  callback);
...
const is_variable_base = ({name}) => Stratum._is_base(name);
const from_variable_base = ({kind, duplicable, name}) => {
  kind,
  duplicable,
  name: Stratum._get_body(name)};
exports._get_visibility_base = (scope, _json, _variables1, _variables2) => (
  {scope:_scope, variables:_variables} = Inner._get_visibility(scope),
  {
    scope: _scope,
    variables: ArrayLite.map(
      ArrayLite.filter(variables, Stratum._is_base),
      Stratum._get_body)});

  {
    foreground: ArrayLite.map(
      ArrayLite.filter(_variables1 is_variable_base),
      from_variable_base),
    background: _variables2});

exports._is_constant_base = (scope, identifier, _kind) => (
  _kind = Inner._get_kind_static(
    scope,
    Stratum._base(identifier)),
  _kind === BASE_CONST_KIND || _kind === BASE_CLASS_KIND);
exports._is_constant_meta = (scope, identifier) => (
  _kind = Inner._get_kind_static(
    scope,
    Stratum._meta(identifier)),
  _kind === META_CONST_KIND);
