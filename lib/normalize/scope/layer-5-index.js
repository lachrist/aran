...
const is_import_variable = ({kind}) => kind === "import";
...
exports.SCRIPT = (completion, variables, callback) => (
  ...
  (
    ArrayLite.some(variables, is_import_variable) ?
    abort("Script block should not contain import variables") :
    make_global(
      [],
      completion,
      variables,
...
exports.EVAL = (scope, completion, variables, callback) => (
  ...
  (
    ArrayLite.some(variables, is_import_variable) ?
    abort("Eval block should not contain import variables") :
    Base.EXTEND_STATIC(
      clean_case(scope),
      (
