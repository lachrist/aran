  ...
  has_loose_variable: function (tag) { return (
    ...
    ArrayLite.some(this.global_variable_array, (variable) => (
      (
        ...
        variable.kind === "function") &&
      variable.name === this.identifier)) :
    abort("Only the global declarative dynamic frame can be query for loose variable")) }};
...
const populate = (scope, variables, callback) => Tree.Bundle(
  [
    Tree.Bundle(
      ArrayLite.map(
        variables,
        (variable) => Base.Declare(scope, variable.kind, variable.name, false))),
    callback(scope)]);
...
const stain = (scope) => Scope._extend_binding(scope, CASE_KEY, true);

const clean = (scope) => (
  Scope._get_binding(scope, CASE_KEY) ?
  Scope._extend_binding(scope, CASE_KEY, false) :
  scope);
...
exports.PARAM = (scope, optimization, variables, callback1, callback2, _variables1, _variables2) => (
        ...
        [
          ...
          Tree.Lone(
            ...
            Base.EXTEND_STATIC(
              scope,
              ["let", "const", "let"],
              (scope) => ppopulate(scope, _variables2, callback2)))]))));

exports.CLOSURE = (scope, strict, context, optimization, variables, callback1, callback2, _variables1, _variables2) => (
  scope = clean(scope),
            ...
            (
              ...
              Meta.Box(
                ...
                (box) => Tree.Lone(
                  [],
                  Base.EXTEND_STATIC(
                    Base._extend_dynamic(scope, ["var", "function"], CLOSURE_TAG, false, false, box),
                    ["let", "const", "class"],
                    (scope) => populate(scope, _variables2, callback2)))))])),
      (
        optimization.is_head_eval_free ?
