...
State._run_session({nodes: [], serials: new Map(), scopes: {__proto__:null}}, [], () => {
  Lang._match_block(Split.EXTEND_STATIC(Split._make_root(), ["kind-base"], ["kind-meta"], (scope) => {
    ...
    // Declare //
    Assert.deepEqual(Split._declare_base(scope, "kind-base", "x_base"), {static:true, value:true});
    Assert.deepEqual(Split._declare_meta(scope, "kind-meta", "x_meta"), {static:true, value:true});
    // Initialize //
    ...
    // Assert.deepEqual(Split._is_writable_meta(scope, identifier1), true);
    // Assert.deepEqual(Split._is_writable_meta(scope, identifier2), false);
    Assert.deepEqual(Split._declare_base(scope, "kind-base", "y_base"), {static:true, value:true});
    Assert.deepEqual(Split._declare_meta(scope, "kind-meta", "y_meta"), {static:true, value:true});
