...
// Invariant hypothesis: `OuterCore._declare_meta` and `OuterCore.initialize_meta`, and `OuterCore.lookup_meta` are only access in this module.
...
const declare_initialize_meta = (context, expression) => {
  const identifier = OuterCore._declare_meta(context.scope, context.identifier, context.writable);
  return {
    remainder: OuterCore.initialize_meta(context.scope, identifier, expression),
    box: {
...
const dispatch_callback_object_1 = {
  ...
  //   if (Stratum._is_meta(stratified_identifier) ? OuterCore._get_tag_meta(context.scope, identifier) : OuterCore._get_tag_base(context.scope, identifier)) {
  //     return {
  //       remainder: null,
  //       box: {
...
exports.get = (scope, box) => {
  ...
  //   return OuterCore.lookup_meta(scope, box.value, null, lookup_callbacks);
  // }
  ...
  //   return OuterCore.lookup_base(scope, box.value, null, lookup_callbacks);
  // }
  // console.assert(false);
};
...
exports.set = (scope, box, expression) => {
  if (box.type === META_IDENTIFIER_TYPE) {
    return OuterCore.lookup_meta(scope, box.value, expression, lookup_callbacks);
  }
  // if (box.type === BASE_IDENTIFIER_TYPE) {
  //   return OuterCore.lookup_base(scope, box.value, expression, lookup_callbacks);
  // }
