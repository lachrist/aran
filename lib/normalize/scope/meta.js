...
const dispatch_callback_object = {
  ...
  sequence: (context, expression, first_expression, second_expression) => {
    const result = Lang._dispatch_expression(dispatch_callback_object, context, second_expression);
    return {
...
const make = (scope, identifier, writable, expression, optimistic_callback, pessimistic_callback) => { // A
  ...
  const result = writable ? declare_initialize_meta(context, expression) : Lang._dispatch_expression(dispatch_callback_object, context, expression);
