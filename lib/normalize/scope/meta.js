...
const declare_initialize_meta = (context, expression) => {
  const identifier = Outer._declare_meta(context.scope, context.writable, context.identifier);
  return {
    remainder: Outer.initialize_meta(context.scope, context.writable, identifier, expression),
    box: {
...
const dispatch_callback_object_1 = {
  ...
  read: (context, expression, stratified_stratified_identifier) => {
    ...
    if (Stratum._is_meta(stratified_identifier) ? Outer._is_constant_meta(context.scope, identifier) : Outer._is_constant_base(context.scope, identifier)) {
      return {
        remainder: null,
        box: {
