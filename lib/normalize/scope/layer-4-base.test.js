"use strict";
Error.stackTraceLimit = 1/0;
const Assert = require("assert").strict;
require("../../tree.js")._toggle_debug_mode();

const Tree = require("../../tree.js");
const Lang = require("../../lang");
const State = require("../state.js");
const Base = require("./layer-4-base.js");

State._run_session({nodes:[], serials:new Map(), scopes:{__proto__:null}}, [], () => {
  // _extend_dynamic //
  Assert.throws(
    () => Base._extend_dynamic(
      Base._make_root(),
      ["var", "let"],
      "tag",
      false,
      Base._primitive_box(123)),
    new global.Error("Cannot mix rigid variables and loose variables inside dynamic frame"));
  // Declare //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["var", "let"],
      (scope) => Tree.Bundle(
        [
          Base.Declare(scope, "let", "x"),
          (
            Assert.throws(() => Base.Declare(scope, "let", "x"), new global.Error("Duplicate variable declaration")),
            Tree.Bundle([])),
          Base.Declare(scope, "var", "y"),
          Base.Declare(scope, "var", "y"),
          Base.Box(
            scope,
            false,
            "frame",
            Tree.primitive(123),
            (box) => Tree.Bundle(
              [
                Base.Declare(
                  Base._extend_dynamic(scope, ["const"], "tag", false, box),
                  "const",
                  "z"),
                Base.Declare(
                  Base._extend_dynamic(scope, ["var"], "tag", false, box),
                  "var",
                  "t")]))])),
    Lang.PARSE_BLOCK(
      `{
        let $$x, $$y;
        $$y = void 0;
        (
          #Reflect.has(123, "z") ?
          throw new #SyntaxError("Variable 'z' has already been declared (this should have been signaled as an early syntax error, please consider submitting a bug repport)") :
          #Reflect.defineProperty(
            123,
            "z",
            {
              __proto__: null,
              ["value"]: #"@deadzone",
              ["writable"]: false,
              ["enumerable"]: true,
              ["configurable"]: false}));
        (
          #Reflect.has(123, "t") ?
          void 0 :
          #Reflect.defineProperty(
            123,
            "t",
            {
              __proto__: null,
              ["value"]: void 0,
              ["writable"]: true,
              ["enumerable"]: true,
              ["configurable"]: false})); }`),
    Assert);
  // initialize //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["let"],
      (scope) => Tree.Bundle(
        [
          Tree.Lift(
            Base.initialize(
              scope,
              "var",
              "x",
              Tree.primitive(123),
              false,
              (scope, identifier, expression) => (
                Assert.deepEqual(identifier, "x"),
                expression))),
          Base.Declare(scope, "let", "y"),
          Tree.Lift(
            Base.initialize(
              scope,
              "let",
              "y",
              Tree.primitive(456),
              false,
              (scope, kind, identifier) => Assert.fail())),
          Base.Box(
            scope,
            false,
            "frame",
            Tree.primitive(789),
            (box) => Tree.Lift(
              Base.initialize(
                Base._extend_dynamic(scope, ["const"], "tag", false, box),
                "const",
                "z",
                Tree.primitive(901),
                false,
                (scope, kind, identifier) => Assert.fail())))])),
    Lang.PARSE_BLOCK(
      `{
        let $$y;
        123;
        $$y = 456;
        (
          #Reflect.has(789, "z") ?
          (
            (#Reflect.get(789, "z") === #"@deadzone") ?
            #Reflect.defineProperty(
              789,
              "z",
              {
                __proto__: null,
                value: 901}) :
            throw new #SyntaxError("Variable 'z' has already been initialized (this should never happen, please consider submitting a bug repport)")) :
          throw new #SyntaxError("Variable 'z' has not yet been declared (this should never happen, please consider submitting a bug repport)")); }`),
    Assert);
  // _is_available //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["var", "let"],
      (scope) => (
        scope = Base._extend_dynamic(
          scope,
          [],
          "tag",
          false,
          Base._primitive_box(123)),
        Tree.Bundle(
          [
            Base.Declare(scope, "var", "x"),
            Base.Declare(scope, "let", "y"),
            Tree.Lift(
              Tree.primitive(
                Base._is_available(
                  scope,
                  "var",
                  "x",
                  {
                    foo: "bar",
                    has_rigid_variable: function (tag) { return (
                      Assert.deepEqual(this.foo, "bar"),
                      Assert.deepEqual(tag, "tag"),
                      false) },
                    has_loose_variable: () => Assert.fail()}))),
              Tree.Lift(
                Tree.primitive(
                  Base._is_available(
                    scope,
                    "let",
                    "y",
                    {
                      foo: "bar",
                      has_rigid_variable: function (tag) { return (
                        Assert.deepEqual(this.foo, "bar"),
                        Assert.deepEqual(tag, "tag"),
                        false) },
                      has_loose_variable: function (tag) { return (
                        Assert.deepEqual(this.foo, "bar"),
                        Assert.deepEqual(tag, "tag"),
                        false) } })))]))),
    Lang.PARSE_BLOCK(`{
      let $$x, $$y;
      $$x = void 0;
      true;
      false; }`),
    Assert);
  // lookup >> miss //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      [],
      (scope) => (
        Assert.throws(
          () => Base.lookup(
            scope,
            "this",
            {
              foo: "bar",
              on_miss: () => Assert.fail(),
              on_live_hit: () => Assert.fail(),
              on_dead_hit: () => Assert.fail(),
              on_dynamic_hit: () => Assert.fail()}),
          new global.Error("Missing special identifier")),
        Tree.Lift(
          Base.lookup(
            scope,
            "x",
            {
              foo: "bar",
              on_miss: function () { return (
                Assert.deepEqual(this.foo, "bar"),
                Tree.primitive(123)) },
              on_live_hit: () => Assert.fail(),
              on_dead_hit: () => Assert.fail(),
              on_dynamic_hit: () => Assert.fail()})))),
    Lang.PARSE_BLOCK(`{123;}`),
    Assert);
  // lookup >> live hit //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["let", "const"],
      (scope) => Tree.Bundle(
        [
          Base.Declare(scope, "let", "x"),
          Tree.Lift(
            Base.initialize(
              scope,
              "let",
              "x",
              Tree.primitive(123),
              false,
              () => Assert.fail())),
          Base.Declare(scope, "const", "this"),
          Tree.Lift(
            Base.initialize(
              scope,
              "const",
              "this",
              Tree.primitive(456),
              false,
              () => Assert.fail())),
          Tree.Lift(
            Base.lookup(
              scope,
              "x",
              {
                foo: "bar",
                on_miss: () => Assert.fail(),
                on_live_hit: function (writable, access) { return (
                  Assert.deepEqual(this.foo, "bar"),
                  Assert.deepEqual(writable, true),
                  access(
                    Tree.primitive(789))) },
                on_dead_hit: () => Assert.fail(),
                on_dynamic_hit: () => Assert.fail()})),
          Tree.Lift(
            Base.lookup(
              scope,
              "this",
              {
                foo: "bar",
                on_miss: () => Assert.fail(),
                on_live_hit: function (writable, access) { return (
                  Assert.deepEqual(this.foo, "bar"),
                  Assert.deepEqual(writable, false),
                  access(
                    Tree.primitive(901))) },
                on_dead_hit: () => Assert.fail(),
                on_dynamic_hit: () => Assert.fail()}))])),
    Lang.PARSE_BLOCK(`
      {
        let $$x, $$this;
        $$x = 123;
        $$this = 456;
        $$x = 789;
        $$this = 901; }`),
    Assert);
  // lookup >> dead hit //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["let", "const"],
      (scope) => Tree.Bundle(
        [
          Base.Declare(scope, "let", "x"),
          Base.Declare(scope, "const", "this"),
          Tree.Lift(
            Base.lookup(
              scope,
              "x",
              {
                foo: "bar",
                on_miss: () => Assert.fail(),
                on_live_hit: () => Assert.fail(),
                on_dead_hit: function () { return (
                  Assert.deepEqual(this.foo, "bar"),
                  // Assert.deepEqual(writable, true),
                  Tree.primitive(123)) },
                on_dynamic_hit: () => Assert.fail()})),
          (
            Assert.throws(
              () => Tree.Lift(
                Base.lookup(
                  scope,
                  "this",
                  {
                    on_miss: () => Assert.fail(),
                    on_live_hit: () => Assert.fail(),
                    on_dead_hit: () => Assert.fail(),
                    on_dynamic_hit: () => Assert.fail()})),
              new global.Error("Special identifier in deadzone")),
            Tree.Bundle([]))])),
    Lang.PARSE_BLOCK(`
      {
        let $$x, $$this;
        123; }`),
    Assert);
  // lookup >> on_dynamic_hit >> special_identifier //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      ["var"],
      (scope) => Tree.Bundle(
        [
          Base.Declare(scope, "var", "this"),
          Tree.Lift(
            Base.lookup(
              Base._extend_dynamic(
                scope,
                [],
                "tag",
                false,
                Base._primitive_box(123)),
              "this",
              {
                foo: "bar",
                on_miss: () => Assert.fail(),
                on_live_hit: function (writable, access) { return (
                  Assert.deepEqual(this.foo, "bar"),
                  Assert.deepEqual(writable, true),
                  access(
                    Tree.primitive(456))) },
                on_dynamic_hit: () => Assert.fail()}))])),
    Lang.PARSE_BLOCK(
      `{
        let $$this;
        $$this = void 0;
        $$this = 456; }`),
    Assert);
  // lookup >> on_dynamic_hit >> (not-unscopables && loose) //
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      [],
      (scope) => Tree.Lift(
        Base.lookup(
          Base._extend_dynamic(
            scope,
            ["var"],
            "tag",
            false,
            Base._primitive_box(123)),
          "x",
          {
            foo: "bar",
            on_miss: function () { return (
              Assert.deepEqual(this.foo, "bar"),
              Tree.primitive(456)) },
            on_live_hit: () => Assert.fail(),
            on_dead_hit: () => Assert.fail(),
            on_dynamic_hit: function (tag, box) { return (
              Assert.deepEqual(this.foo, "bar"),
              Assert.deepEqual(tag, "tag"),
              Base.get(scope, box)) }}))),
    Lang.PARSE_BLOCK(
      `{
        (
          #Reflect.has(123, "x") ?
          123 :
          456); }`),
    Assert);
  // lookup >> on_dynamic_hit >> (unscopables && rigid)
  Lang._match_block(
    Base.EXTEND_STATIC(
      Base._make_root(),
      [],
      (scope) => Tree.Lift(
        Base.lookup(
          Base._extend_dynamic(
            scope,
            ["let"],
            "tag",
            true,
            Base._primitive_box(123)),
          "x",
          {
            foo: "bar",
            on_miss: function () { return (
              Assert.deepEqual(this.foo, "bar"),
              Tree.primitive(456)) },
            on_live_hit: () => Assert.fail(),
            on_dead_hit: function () { return Tree.primitive(789) },
            on_dynamic_hit: function (tag, box) { return (
              Assert.deepEqual(this.foo, "bar"),
              Assert.deepEqual(tag, "tag"),
              Base.get(scope, box)) }}))),
    Lang.PARSE_BLOCK(
      `{
        let $_unscopables;
        (
          (
            #Reflect.has(123, "x") ?
            (
              $_unscopables = #Reflect.get(123, #Symbol.unscopables),
              (
                (
                  (typeof $_unscopables === "object") ?
                  $_unscopables :
                  (typeof $_unscopables === "function")) ?
                !#Reflect.get($_unscopables, "x") :
                true)) :
            false) ?
          (
            (#Reflect.get(123, "x") === #"@deadzone") ?
            789 :
            123) :
          456); }`),
    Assert);
});
