...
exports._declare = (scope, identifier, tag) => {
  while (scope !== null) {
    if (scope.type === EVAL_FRAME_TYPE) {
      throw new global_Error("Cannot declare beyond eval frames");
    }
    if (scope.type === STATIC_FRAME_TYPE) {
      if (identifier in scope.frame) {
...
exports.initialize = (scope, identifier, expression) => {
  ...
  while (scope !== null) {
    if (scope.type === EVAL_FRAME_TYPE) {
      throw new global_Error("Cannot initialized beyond eval frames");
    }
    if (scope.type == STATIC_FRAME_TYPE) {
      if (identifier in scope.frame) {
        if (scope.frame[identifier] === null) {
...
exports.lookup = (scope, identifier, context, callbacks) => {
  ...
  const loop = (scope, escaped) => {
    ...
    if (identifier in scope.frame) {
      ...
      // console.assert(scope.frame[identifier].has_dynamic_deadzone || is_eval_free);
    ...
    if (scope.type === USE_STRICT_FRAME_TYPE) {
      ...
      for (let identifier in scope.frame) {
        ...
        if (binding !== null && !ArrayLite.includes(stratified_identifier_array, Stratum._base(identifier))) {
          if (binding.status === DISTANTLY_INITIALIZED_STATUS || (binding.status === NON_INITIALIZED_STATUS && !is_closure_free)) {
            // console.assert(binding.has_dynamic_deadzone || is_eval_free);
          ...
          static_frame[identifier] = global_Object_assign({}, binding);
        }
      }
...
exports._make_eval = (frame_array) => {
  let scope = null;
