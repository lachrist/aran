...
State._run_session({nodes: [], serials: new Map(), scopes: {__proto__:null}}, [], () => {
  Assert.deepEqual(Outer.EXTEND_STATIC(Outer._extend_horizon(Outer._make_root()), (scope) => {
    Assert.throws(() => Outer._declare_meta(Outer._extend_dynamic(scope, "foo"), "x", "bar"), new global.Error("Cannot declare meta variable on dynamic frame"));
    Assert.throws(() => Outer.initialize_meta(Outer._extend_dynamic(scope, "foo"), "x", Tree.primitive("bar")), new global.Error("Cannot initialize meta variable on dynamic frame"));
    Assert.deepEqual(Outer._declare_base(scope, "x1", true), true);
    Assert.deepEqual(Outer._declare_base(scope, "x2", false), true);
    Assert.deepEqual(Outer._get_foreground_base(scope), ["x1", "x2"]);
