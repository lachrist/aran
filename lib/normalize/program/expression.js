...
visitors.Super = (scope, node, context) => ( // console.assert(Scope._get_binding_self_nullable_box(scope) !== null)
  Scope._get_binding_super_nullable_box(scope) === null ?
  Builtin.get_prototype_of(
    Scope._get_binding_self_nullable_box(scope)) :
  Tree.conditional(
...
visitors.ObjectExpression = (scope, node, context, _closure) => (
  _closure = (self_nullable_box, _closure1, _closure2) => (
    _closure1 = (node) => Scope.box(
      ...
      (prototype_box) => Tree.conditional(
        ...
        Scope.get(prototype_box),
        Builtin.grab("Object.prototype"))),
    _closure2 = (property, _key_box) => (// console.assert(Query._is_not_proto_property(property) && Query._is_not_spread_property(property))
      (
        ...
        (
          property.kind === "init" ?
          {
            ...
            value: Expression.visit(
              ...
              {
                __proto__: Expression._default_context,
                tag: property.method ? "method" : null,
                name: _key_box,
          ...
          {
            __proto__: null,
            [property.kind]: Scope.box(
              scope,
              "ExpressionObjectName",
              false,
              Tree.binary(
                "+",
                Tree.primitive(property.kind + " "),
                Scope.get(scope, key_box)),
              (name_box) => Expression.visit(
                scope,
                property.value,
        ...
        (
          ...
          Tree.primitive(
            (
              ...
              property.key.name :
              property.key.value))),
        { // console.assert(property.kind === "init")
          __proto__: null,
          value: Expression.visit(scope, property.value, Expression,_default_context),
          writable: true,
          enumerable: true,
          configurable: true}]),
    (
      (
        ArrayLite.every(node.properties, Query._is_not_spread_property) &&
        (
      ...
      (
        (
          node.properties.length === 0 ||
          Query._is_not_proto_property(node.properties[0])) ?
  ...
  (
    (
      ArrayLite.some(node.properties, Query._is_method_property) ||
      ArrayLite.some(node.properties, Query._is_accessor_property)) ?
    Scope.box(
      scope,
...
visitors.CallExpression = (scope, node, dropped, box) => (
  ...
  (
    ...
    Tree.conditional( // console.assert(node.optional === false)
      Scope.read(scope, "this"),
      Builtin._throw_reference_error("Super constructor may only be called once"),
      Tree.sequence(
        Scope.write(
          scope,
          "this",
          (
            ...
            Builtin.construct(
              Scope._get_binding_super_nullable_box(scope), // console.assert(Scope._get_binding_super_nullable_box(scope) !== null)
              Builtin.concat(
                ArrayLite.map(
                  node.arguments,
                  (argument) => (
                    ...
                    Builtin.construct_array(
                      [
                        Expression.visit(scope, argument, Expression._default_context)]))))) :
            Tree.construct(
              Scope._get_binding_super_nullable_box(scope),
              ArrayLite.map(
                node.arguments,
                (argument) => Expression.visit(scope, argument, Expression._default_context))))),
        Scope.read(scope, "this"))) :
