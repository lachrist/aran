...
// ReturnStatement
Assert.deepEqual(Eval._has_direct_eval_call(parse(`(function () { return eval(x); });`).body[0].expression.body), true);
Assert.deepEqual(Eval._has_direct_eval_call(parse(`(function () { return; });`).body[0].expression.body), false);
// BlockStatement
...
Assert.deepEqual(Eval._has_direct_eval_call(parse(`x + eval(x);`)), true);
// YieldExpression
Assert.deepEqual(Eval._has_direct_eval_call(parse(`(function * () { yield eval(x); });`).body[0].expression.body), true);
// AwaitExpression
Assert.deepEqual(Eval._has_direct_eval_call(parse(`(async function () { await eval(x); });`).body[0].expression.body), true);
// UnaryExpression
Assert.deepEqual(Eval._has_direct_eval_call(parse(`!eval(x);`)), true);
// MemberExpression
Assert.deepEqual(Eval._has_direct_eval_call(parse(`eval(x).k;`)), true);
