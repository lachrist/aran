...
const DIRECT_EVAL_CALL_ACCESS = {
  is_direct_eval_call: true,
  is_this_read: true,
  is_new_target_read: true,
  is_arguments_read: true,
  is_arguments_written: true,
  is_callee_read: true,
  is_callee_written: true
};
...
exports._access = (node) => {
  ...
  try {
    ...
    if (error === MARKER) {
      return DIRECT_EVAL_CALL_ACCESS;
    }
    throw error;
  } finally {
    ...
    for (let index = 0; index < pattern.elements.length; index++) {
      if (pattern.elements[index] !== null) {
        if (pattern.elements[index].type === "RestElement") {
          access = combine(access, visit_pattern(pattern.elements[index].argument, scoping));
        } else {
...
const visit_expression = (expression) => {
  ...
  if (expression.type === "FunctionExpression") {
    ...
    // Plus, callee is not an expensive variable to instantiate //
  ...
  if (expression.type === "ArrowFunctionExpression") {
    ...
    if (expression.expression) {
      ...
      access = combine(access, visit_block(expression.body));
    }
    return remove_closure_declaration(access);
  }
  // Application //
  if (Eval._is_direct_eval_call(expression)) {
    throw MARKER;
  }
...
const visit_statement = (statement) => {
  if (statement.type === "EmptyStatement" || statement.type === "BreakStatement" || statement.type === "ContinueStatement" || statement.type === "DebuggerStatement") {
    return EMPTY;
  }
