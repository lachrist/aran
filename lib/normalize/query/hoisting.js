...
exports._get_parameter_array_hoisting = (pattern_array) => {
  const hoisting = {__proto__:null};
  for (let index = 0; index < pattern_array.length; index++) {
    if (pattern_array[index].type === "RestElement") {
      collect(hoisting, pattern_array[index].argument, true);
    } else {
      collect(hoisting, pattern_array[index], true);
    }
  }
  return hoisting
};

exports._get_parameter_hoisting = (pattern) => collect({__proto__:null}, pattern, true);
