...
const test_param = (code, identifiers) => {
  ...
  Assert.deepEqual(Hoisting._get_parameter_hoisting(Acorn.parse("(" + code + " = 123);").body[0].expression.left, true), hoisting);
};
...
const test_shallow = (code, identifiers1, identifiers2) => {
  ...
  Assert.deepEqual(Hoisting._get_shallow_hoisting(Acorn.parse(code).body), hoisting);
};
...
const test_deep = (code, identifiers) => {
  const hoisting = {__proto__:null};
  identifiers.forEach((identifier) => { hoisting[identifier] = true });
  Assert.deepEqual(Hoisting._get_deep_hoisting(Acorn.parse(code).body), hoisting);
};
