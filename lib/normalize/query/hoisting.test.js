...
const test_param = (code, identifiers) => {
  ...
  Assert.deepEqual(Hoisting._param({__proto__:null}, Acorn.parse("(" + code + " = 123);").body[0].expression.left, true), hoisting);
};
...
const test_shallow = (code, identifiers1, identifiers2) => {
  ...
  Assert.deepEqual(Hoisting._shallow({__proto__:null}, Acorn.parse(code).body), hoisting);
};
...
const test_deep = (code, identifiers) => {
  const hoisting = {__proto__:null};
  identifiers.forEach((identifier) => { hoisting[identifier] = true });
  Assert.deepEqual(Hoisting._deep({__proto__:null}, Acorn.parse(code).body), hoisting);
};
