...
State._run_session({nodes: [], serials: new Map, scopes: {__proto__:null}}, [], () => {
  Scope.EXTEND_STATIC(null, {__proto__:null}, (scope) => {
    return Scope.Box(scope, "x", false, Tree.primitive(123), (box) => {
      ...
      // _extend //
      Assert.throws(() => Completion._extend(Completion._arrow, Acorn.parse(`123;`).body, -1), new Error("Out-of-range offset"));
      Assert.throws(() => Completion._extend(Completion._arrow, Acorn.parse(`123;`).body, 1), new Error("Out-of-range offset"));
      Assert.deepEqual(Completion._extend(Completion._arrow, Acorn.parse(`123;`).body, 0), Completion._arrow);
