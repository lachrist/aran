  ...
  makeConditionalEffect(
    ...
    [
      makeExpressionEffect(
        makeThrowErrorExpression(
          "TypeError",
          `Cannot define global object with ${variable}`,
          hash,
        ),
        hash,
      ),
 ...
 * >}
 */
export const declareGlobal = (hash, variable, { eval: configurable, kinds }) =>
  initSequence(
    [
      configurable
        ? null
      ...
      includes(kinds, "function-sloppy-near")
        ? makePrefixPrelude(
            makeDefineGlobalEffect(hash, variable, configurable),
          )
        : makePrefixPrelude(
            makeConditionalEffect(
              makeApplyExpression(
