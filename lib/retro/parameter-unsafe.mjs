...
import { makeAmbientIntrinsicExpression } from "./intrinsic.mjs";
    ...
    body: [
      {
        ...
        argument: {
          type: "NewExpression",
          callee: makeAmbientIntrinsicExpression("SyntaxError", config),
...
/**
 * @type {(
 *   parameter: import("./parameter-unsafe.d.ts").UnsafeParameter,
 *   config: import("./config-internal.d.ts").InternalConfig,
 * ) => import("estree-sentry").Expression<{}>}
 */
const makeAlternateExpression = (parameter, config) => {
  if (parameter === "import.meta" || parameter === "new.target") {
    return makeAmbientIntrinsicExpression("aran.deadzone_symbol", config);
