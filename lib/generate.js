...
const lift_matcher = ["ExpressionStatement", () => true];
...
const visit_statement = (statement, context, identifiers, _is_inlined) => (
  _is_inlined = (
    Tree.match(null, statement, lift_matcher) ||
    Tree.match(null, statement, lone_matcher)),
...
const statement_callback_object = {
  __proto__: null,
  // Atomic Statement //
  DeclareEnclaveStatement: (context, node, kind, identifier, expression) => context.build.EnclaveDeclare(
    kind,
    identifier,
  ...
  ReturnStatement (context, node, expression) => context.build.Return(
    visit_expression(expression, context, NORMAL_USAGE)),
  BreakStatement (context, node, label) => context.build.Break(
    context.generate.label(label)),
  DebuggerStatement (context, node) => context.build.Debugger(),
  // Compound Statement //
  ExpressionStatement: (context, node, block) => context.build.Lone(
    visit_block(block, context, "lone", null)),
  IfStatement (context, node, expression, block1, block2) => context.build.If(
    visit_expression(expression, context, NORMAL_USAGE),
    visit_block(block1, context, "then", null),
    visit_block(block2, context, "else", null)),
  WhileStatement: (context, node, expression, block) => context.build.While(
    visit_expression(expression, context, NORMAL_USAGE),
    visit_block(block, context, "do", null)),
  TryStatement: (context, node, block1, block2, block3) => context.build.Try(
...
const expression_callback_object = {
  __proto__: null,
  // Literal //
  PrimitiveExpression: (context, node, primitive) => context.build.primitive(
    primitive),
  IntrinsicExpression: (context, node, intrinsic) => context.build.intrinsic(
    context.usage,
    context.namespace.intrinsic,
    intrinsic),
  ClosureExpression: (context, node, sort, asynchronous, generator, block, _identifier) => context.build.closure(
    context.usage,
    sort,
    asynchronous,
    generator,
  ...
  // Environment //
  ReadExpression: (context, node, identifier) => context.build.read(
  ...
  WriteExpression: (context, node, identifier, expression, _code) => context.build.write(
    context.usage,
    context.generate.identifier(identifier),
    visit_expression(expression, context, NORMAL_USAGE)),
  WriteEnclaveExpression: (context, node, strict, identifier, expression, _code) => (
    ...
    context.build.enclave_write(
      ...
      identifier,
      visit_expression(expression, context, NORMAL_USAGE))),
  SuperMemberEnclaveExpression: (context, node, expression) => context.build.enclave_super_member(
    visit_expression(expression, context, NORMAL_USAGE)),
  SuperCallEnclaveExpression: (context, node, expression) => context.build.enclave_super_call(
    visit_expression(expression, context, NORMAL_USAGE)),
  EvalExpression: (context, node, expression) => context.build.eval(
    visit_expression(expression, context, NORMAL_USAGE)),
  RequireExpression: (context, node, expression) => context.build.require(
    visit_expression(expression, context, NORMAL_USAGE)),
  AwaitExpression: (context, node, expression) => context.build.await(
    visit_expression(expression, context, NORMAL_USAGE)),
  YieldExpression: (context, node, delegate, expression) => context.build.yield(
    delegate,
    visit_expression(expression, context, NORMAL_USAGE)),
  ImportExpression: (context, node, specifier, source) => context.build.import(
    `${context.namespace.import}_${specifier === null ? `0` : specifier}_${encode(source)}`),
  ExportExpression: (context, node, specifier, expression, _code) => context.build.export(
    context.usage,
    `${context.namespace.export}_${specifier}`,
    visit_expression(expression, context, NORMAL_USAGE)),
  // Control //
  ConditionalExpression: (context, node, expression1, expression2, expression3) => context.build.conditional(
  ...
  SequenceExpression: (context, node, expression1, expression2) => context.build.sequence(
    visit_expression(expression1, context, DROPPED_USAGE),
    visit_expression(expression2, context, context.usage === DROPPED_USAGE ? DROPPED_USAGE : NORMAL_USAGE)),
  ThrowExpression: (context, node, expression1, expression2) => context.build.throw(
    visit_expression(expression1, context, NORMAL_USAGE)),
  // Combiner //
  UnaryExpression: (context, node, operator, expression) => context.build.unary(
    operator,
    visit_expression(expression, context, NORMAL_USAGE)),
  BinaryExpression: (context, node, operator, expression1, expression2) => context.build.binary(
    operator,
    visit_expression(expression1, context, NORMAL_USAGE),
    visit_expression(expression2, context, NORMAL_USAGE)),
  ObjectExpression: (context, node, expression, properties) => context.build.object(
    visit_expression(expression, context, NORMAL_USAGE),
    ArrayLite.map(
      properties,
  ...
  ApplyExpression: (context, node, expression1, expression2, expressions, _is_this_undefined, _result, _nullable_result_1, _results, _nullable_result_2) => (
    _is_this_undefined = Tree.match(null, expression2, primitive_undefined_matcher),
