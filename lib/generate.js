...
const visit_program = (program, context, options) => Tree._dispatch_program(
  program_callback_object,
  global_Object_assign(context, options),
  program);
...
const builders = {
  code: {
    ...
    _program: (context, node, preludes, block) => (
      `${context.newline}"use strict";` +
      Tree._dispatch(callbacks,
      codes.join("") +
      `${this.__newline__}((() => ${block}) ());`),
    // Prelude //
    _import: (specifier, source, namespace) => (
      specifier === null ?
      `${this.__newline__}import * as ${namespace.import(specifier, source)} from ${global_JSON_stringify(source)};` :
      `${this.__newline__}import {${specifier} as ${namespace.import(specifier, source)}} from ${global_JSON_stringify(source)};`),
    _export: (specifier) => (
      `${this.__newline__}let ${namespace.export(specifier)};` +
      `${this.__newline__}export {${namespace.export(specifier)} as ${specifier}};`),
    // console.assert((specifier1 === null) === (specifier2 === null))
    _aggregate: (specifier1, source, specifier2) => (
      specifier1 === null ?
      `${this.__newline__}export * from ${global_JSON_stringify(source)};` :
      `${this.__newline__}export {${specifier1} as ${specifier2}} from ${global_JSON_stringify(source)};`),
    // Block //
