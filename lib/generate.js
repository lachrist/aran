...
build.code.BLOCK = (context, input_identifier, identifiers, codes, _callee) => (
...
const statement_callback_object = {
  ...
  Continue: (context, node, label) => context.build.Continue(
    context.generate.label(label)),
...
const make_closure_callback = (sort) => (context, node, block, _identifier) => (
  ...
  (
    _identifier = `${context.namespace["actual-callee"]}_${global_String(fetch(context, "program").counter++)}`,
    context.identifiers[context.identifiers.length] = _identifier,
    context.build.closure(
      context.usage,
      sort,
...
const expression_callback_object = {
  ...
  enclave_read: (context, node, identifier) => context.build.enclave_read(
    identifier),
  enclave_typeof: (context, node, identifier) => context.build.enclave_typeof(
    identifier),
  write: (context, node, identifier, expression, _code) => context.build.write(
    context.usage,
    context.generate.identifier(identifier),
    visit_expression(expression, context, NORMAL_USAGE)),
  enclave_write: (context, node, strict, identifier, expression, _code) => context.build.enclave_write(
    context.generate.identifier("input"),
    context.namespace,
    strict,
    context.usage,
  ...
  apply: (context, node, expression1, expression2, expressions, _is_this_undefined, _result, _nullable_result_1, _results, _nullable_result_2) => (
    ...
    (
      ...
      context.build.apply(context.namespace.intrinsic, _result, _nullable_result_1, _results) :
      _nullable_result_2))};
