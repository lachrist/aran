import { includes, listKey, mapIndex } from "./util/index.mjs";
...
const SCOPE_ITEM_ENUM = listKey(SCOPE_ITEM_RECORD);
...
const SOURCE_ENUM = listKey(SOURCE_RECORD);
 ...
 *   path: string,
 *   candidate: unknown,
 * ) => import("./unbuild/scope").NodeFrame}
 */
const sanitizeNodeFrame = (path, candidate) => {
 ...
 */
const sanitizeRootContext = (path, candidate) => {
  if (typeof candidate !== "object" || candidate === null) {
    throw new AranInputError(`${path}.context`, "an object", candidate);
  }
  return /** @type {any} */ (candidate);
};
...
const sanitizePackScope = (path, candidate) => {
  ...
  return {
    context: sanitizeRootContext(`${path}.context`, context),
    frames: sanitizeArray(`${path}.frames`, frames, sanitizeNodeFrame),
  };
};
