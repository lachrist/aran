...
export const sanitizeContext = (path, candidate) => {
  ...
  if (type === "global") {
    ...
    return {
      ...
      situ: sanitizeEnumeration(
        `${path}.situ`,
        getDefault(candidate, "situ", "none"),
        ["program", "function", "method", "constructor", "derived-constructor"],
      ),
    };
  } else {
...
export const sanitizeProgram = (path, candidate) => {
  ...
  if (kind === "module") {
    ...
    } else if (context.type === "local") {
      return { kind, base, root, context };
    } else if (context.type === "aran") {
      return { kind, base, root, context };
    } else {
...
export const sanitizeConfig = (path, candidate) => ({
  ...
  debug_alias: sanitizeBoolean(
    `${path}.debug_alias`,
    getDefault(candidate, "debug_alias", false),
  ),
});
