...
import { AranTypeError, AranSimpleInputError } from "./error.mjs";
import {
...
const sanitizeString = (report, candidate) => {
  if (typeof candidate !== "string") {
    throw new AranSimpleInputError(report, "a string", candidate);
  }
  return candidate;
};
...
const sanitizeNumber = (report, candidate) => {
  if (typeof candidate !== "number") {
    throw new AranSimpleInputError(report, "a number", candidate);
  }
  return candidate;
};
...
const sanitizeFunction = (report, candidate) => {
  if (typeof candidate !== "function") {
    throw new AranSimpleInputError(report, "a boolean", candidate);
  }
  return candidate;
};
...
export const sanitizeVariable = (report, candidate) => {
  ...
  if (!apply(testRegExp, variable_regexp, [candidate])) {
    throw new AranSimpleInputError(
      report,
      "a valid estree identifier",
      candidate,
    );
  }
...
const sanitizeArray = (report, candidate, sanitizeItem) => {
  if (isArray(candidate)) {
    ...
    throw new AranSimpleInputError(report, "an array", candidate);
  }
};
...
const sanitizeObject = (report, candidate) => {
  if (typeof candidate !== "object" || candidate === null) {
    throw new AranSimpleInputError(report, "an object", candidate);
  }
  return candidate;
};
...
const sanitizeSingleton = (report, candidate, singleton) => {
  if (candidate === singleton) {
    ...
    throw new AranSimpleInputError(report, stringifyJson(singleton), candidate);
  }
};
...
const sanitizeFrame = (report, candidate) => {
  ...
  if (typeof type !== "string") {
    throw new AranSimpleInputError(`${report}.type`, "a string", type);
  }
  return /** @type {any} */ (candidate);
};
