...
import { AranTypeError, AranIllegalInputError } from "./report.mjs";
import {
...
const sanitizeModuleProgram = (report, candidate) => {
  ...
  if (type === "Program") {
    ...
    throw new AranIllegalInputError({
      target: `${report}.type`,
      expect: "either 'Program' or 'EarlySyntaxError'",
      actual: type,
    });
  }
};
...
const sanitizeFlexiblePointcutEntry = (report, [key, val]) => [
  sanitizeVariable(`${report}@key`, key),
  {
    kind: sanitizeEnumeration(
      `${report}.${key}.kind`,
      getDefault(val, "kind", "before"),
      flexible_aspect_kind_enumeration,
    ),
    pointcut: /** @type {any} */ (
      sanitizeFunction(`${report}.${key}.pointcut`, val)
    ),
  },
];
