...
ArrayLit.forEach(["error", "argument", "unary", "binary", "get"], (key) => {
  exports[key] = () => {
    throw new Error(key+" shoudl not appear here");
  }
});
...
exports.closure = ({1:block}, namespace) => ({
  ...
  id: {
    ...
    name: "callee" },
  params: [],
  body: Visit.block(block, {tag:"closure", namespace}),
  generator: false,
  expression: false,
  async: false});
...
exports.sequence = ({1:expression1, 2:expression2}, namespace) => {
  const node1 = Visit.expression(expression1, namespace);
  const node2 = Visit.expression(expression2, namespace);
  return {
    type: "SequenceExpression",
    expressions: ArrayLite.concat(
      node1.type === "SequenceExpression" ? node1.expressions : [node1],
      node2.type === "SequenceExpression" ? node2.expressions : [node2])};
};

exports.eval = ({1:expression}) => ({
  type: "CallExpression",
  callee: {
    type: "Identifier",
    name: "eval" },
...
exports.construct = ({1:expression, 2:expressions}, namespace) => {
  if (expression[0] === "builtin" && expression[1] === "RegExp" && expressions.length === 2) {
    if (expressions[0][0] === "primitive" && typeof expressions[0][1] === "string") {
      if (expressions[1][0] === "primitive" && typeof expressions[1][1] === "string") {
        const valid = ArrayLite.every((expressions[1][1]), (character, index, string) => {
          return ArrayLite.includes("gimuy", character) && ArrayLite.lastIndexOf(string, character) === index;
        });
        return {
          type: "Literal",
          regex: {
...
exports.apply = ({1:expression1, 2:expression2, 3:expressions}, namespace) => {
  if (expression2[0] === "primitive" && expression2[1] === void 0) {
    ...
    if (expression1[0] === "builtin" && expression1[1] === "AranUnary" && expressions.length === 2) {
      if (expressions[0][0] === "primitive" && ArrayLite.includes(Enumeration.UnaryOperator, expressions[0][1])) {
        return {
          type: "UnaryExpression",
          prefix: true,
          operator: expressions[0][0],
          argument: Visit.expression(expressions[1], namespace)
        };
      }
    }
    if (expression1[0] === "builtin" && expression1[1] === "AranBinary" && expressions.length === 3) {
      if (expressions[0][0] === "primitive" && ArrayLite.includes(Enumeration.BinaryOperator, expressions[0][1])) {
        return {
          type: "BinaryExpression",
          prefix: true,
          operator: expressions[0][0],
          left: Visit.expression(expressions[1], namespace),
          right: Visit.expression(expressions[2], namespace)
        };
      }
    }
    if (expression1[0] === "builtin" && expression1[1] === "AranGet" && expressions.length === 2) {
      return {
        type: "MemberExpression",
        computed: true,
