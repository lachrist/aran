...
exports.PatternNames = namesof;

exports.BlockNames = (nodes, kind) => ArrayLite.flatMap(
  node.body,
  (node) => (
...
exports.ClosureNames = (nodes) => {
  ...
  while (length) {
    ...
    if (node.type === "IfStatement") {
      ...
      const array = collect(Array.map(node.declarations, (declaration) => declaration.id));
      if (node.kind === "var") {
        for (let index = array.lengt - 1; index >= 0; index--) {
          if (!ArrayLite.includes(names, array[index])) {
            names[names.length] = array[index];
          }
        }
      } else {
        node.AranVariableNames = reverse(array);
      }
