...
const intrinsic_generate_identifier = "BUILTIN";
...
// Failures //
//////////////
const failure = (pointcut, context, message) => Assert.throws(
  () => aran.weave(
    {
      type: "Program",
      body: []},
...
failure(true, global.Symbol("foo"), "The context should either be null/undefined (for 'script' mode), a string (for 'script' mode or 'global-eval' mode), a number (for 'local-eval' mode), or an object");

failure(true, {__proto__: null, mode:"local-eval", serial:"foo"}, "In 'local-eval' mode, the serial should be a number");
failure(true, {__proto__: null, mode:"local-eval", serial:0/0}, "In 'local-eval' mode, the serial should not be NaN");
failure(true, {__proto__: null, mode:"local-eval", serial:0.5}, "In 'local-eval' mode, the serial should be an integer");
failure(true, {__proto__: null, mode:"local-eval", serial:-1}, "In 'local-eval' mode, the serial should be a positive integer");
failure(true, {__proto__: null, mode:"local-eval", serial:global.Number.MAX_SAFE_INTEGER}, "In 'local-eval' mode, the serial should refer to a node representing a direct eval call");
failure(true, 0/0, "In 'local-eval' mode, the serial should not be NaN");

failure(true, "foo", "The mode should either be 'script', 'global-eval', or 'local-eval'");

failure("foo", null, "The pointcut must be either an array, a closure, a boolean, or an object");

/////////////
// Success //
/////////////
const success = (code1, pointcut, context, code2) => diff(
  generate(
    aran.weave(
      parse(code1),
      pointcut,
...
success(
  ...
  false,
  "global-eval",
  `((${intrinsic_generate_identifier}, ${apply_generate_identifier}, eval) => {
    ${apply_generate_identifier} = ${apply_generate_identifier} || ${intrinsic_generate_identifier}["Reflect.apply"];
    eval = eval || ${intrinsic_generate_identifier}["eval"];
    return ((() => {
      "use strict";
      ...
      return (${callee_generate_identifier + "1"} = (...${arguments_generate_identifier}) => {
        ...
        ${advice_instrument_identifier} = (0, ${intrinsic_generate_identifier}["Reflect.get"])(${arguments_generate_identifier}, 0);
        {
          let ${Stratum._base(Stratum._base("this"))};
          ${Stratum._base(Stratum._base("this"))} = ${intrinsic_generate_identifier}["global"];
          return 123;
        }
    ...
    return ((() => {
      ...
      return (${callee_generate_identifier + "1"} = (...${arguments_generate_identifier}) => {
        ...
        ${advice_instrument_identifier} = (0, ${intrinsic_generate_identifier}["Reflect.get"])(${arguments_generate_identifier}, 0);
        {
          try {
            let ${Stratum._base(Stratum._base("this"))};
            ${advice_instrument_identifier}["enter"]("global-eval", [], {__proto__:null}, ["this"], 0);
            ${Stratum._base(Stratum._base("this"))} = ${advice_instrument_identifier}["write"](
              "this",
              ${advice_instrument_identifier}["intrinsic"]("global", ${intrinsic_generate_identifier}["global"], 0),
              0);
            return ${advice_instrument_identifier}["primitive"](123, 1);
          } catch (${error_generate_identifier}) {
            throw ${advice_instrument_identifier}["failure"]("global-eval", ${error_generate_identifier}, 0);
          } finally {
            ${advice_instrument_identifier}["leave"]("global-eval", 0);
          }
        }
      }, delete ${callee_generate_identifier + "1"}.length, delete ${callee_generate_identifier + "1"}.name, ${callee_generate_identifier + "1"});
    }) ());
            ...
            return (
              ${callee_generate_identifier + "1"} = (...${arguments_generate_identifier}) => {
                ...
                ${advice_instrument_identifier} = (0, ${intrinsic_generate_identifier}["Reflect.get"])(${arguments_generate_identifier}, 0);
                {
                  ...
                  (0, ${intrinsic_generate_identifier}["Object.defineProperty"])(
                    (0, ${intrinsic_generate_identifier}["Object.defineProperty"])(
                      (
                            ...
                            ${callee_generate_identifier} = ${callee_instrument_identifier + "1"};
                            {
                              (
                                ...
                                (
                                  (
                                    () => {
                                      throw new ${intrinsic_generate_identifier}["ReferenceError"]("Cannot access 'x' before initialization"); })
                                  ()));}
                            return void 0;},
              ...
              ${advice_instrument_identifier} = (0, ${intrinsic_generate_identifier}["Reflect.get"])(${arguments_generate_identifier}, 0);
              {
                ...
                return (
                  ${Stratum._base(Stratum._meta("ExpressionCallEvalCallee_1_1"))} = (
                    (0, ${intrinsic_generate_identifier}["Reflect.has"])(
                      ${intrinsic_generate_identifier}["global"],
                      "eval") ?
                    (0, ${intrinsic_generate_identifier}["Reflect.get"])(
                      ${intrinsic_generate_identifier}["global"],
                      "eval") :
                    (
                      (
                        () => { throw new ${intrinsic_generate_identifier}["ReferenceError"]("eval is not defined"); })
                      ())),
                  (
                    (
                      ${Stratum._base(Stratum._meta("ExpressionCallEvalCallee_1_1"))} ===
                      ${intrinsic_generate_identifier}["eval"]) ?
                    eval(456) :
