...
// enclave-kind //
Tree.EnclaveDeclare("var", "foo", Tree.primitive(123));
Tree.EnclaveDeclare("let", "foo", Tree.primitive(123));
Tree.EnclaveDeclare("const", "foo", Tree.primitive(123));
Assert.throws(
  () => Tree.EnclaveDeclare("bar", "foo", Tree.primitive(123)),
  new Error(`Invalid enumeration-based atomic node: expected a enclave-kind, got "bar"`));
...
Tree._toggle_debug_mode();
// Bundle //
Assert.deepEqual(Tree.BLOCK(["l", "m"], ["x", "y"], [
  Tree.Bundle([
    Tree.Break("k"),
    Tree.Bundle([
      Tree.Break("l"),
