...
Assert.throws(() => Tree.primitive(() => {}), new Error("Invalid predicate-based atomic node"));
// Label & Identifeir //
["Break", "read"].forEach((name) => {
  Tree[name]("foo");
  Assert.throws(() => Tree[name](" foo"), new Error("Invalid predicate-based atomic node"));
  Assert.throws(() => Tree[name]("foo "), new Error("Invalid predicate-based atomic node"));
  Assert.throws(() => Tree[name]("foo bar"), new Error("Invalid predicate-based atomic node"));
  Assert.throws(() => Tree[name]("0foo"), new Error("Invalid predicate-based atomic node"));
  Assert.throws(() => Tree[name]("in"), new Error("Invalid predicate-based atomic node"));
  Assert.throws(() => Tree[name](123), new Error("Invalid predicate-based atomic node"));
});
// Source && Specifier //
...
Assert.throws(() => Tree.BLOCK(["callee"], []), new Error("Invalid predicate-based atomic node"));
Assert.throws(() => Tree.BLOCK(["arguments"], []), new Error("Invalid predicate-based atomic node"));
...
Assert.throws(() => Tree.builtin(123), new Error("Invalid predicate-based atomic node"));
// Invalid node of enumeration-based type //
Tree.unary("!", Tree.primitive(123));
Assert.throws(() => Tree.unary("foo", Tree.primitive(123)), new Error("Invalid enumeration-based atomic node"));
...
Assert.deepEqual(Tree._dispatch_statement({
  __proto__: callbacks,
  Bundle: (context, statement, statements) => {
    Assert.deepEqual(context, "ctx");
    Assert.deepEqual(statement, Tree.Bundle([Tree.Break("k"), Tree.Break("l"), Tree.Break("m")]));
    Assert.deepEqual(statements, [Tree.Break("k"), Tree.Break("l"), Tree.Break("m")]);
    return "foo"
  }
}, "ctx", Tree.Bundle([Tree.Bundle([Tree.Break("k"), Tree.Break("l")]), Tree.Break("m")])), "foo");
Assert.deepEqual(Tree._dispatch_statement({
  __proto__: callbacks,
