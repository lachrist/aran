...
// dispatch && extract && allign //
{
  ...
  const test = (type, fields, _node) => (
    _node = global.Reflect.apply(Tree[type], global.undefinde, fields),
    // dispatch //
    Assert.deepEqual(
      Tree.dispatch(
        context,
  ...
  // 1 //
  test("BundleStatement", [[Tree.DebuggerStatement()]]);
  // 2 //
  ...
  // 4 //
  test("ClosureExpression", ["function", true, true, Tree.Block(["x"], [], Tree.PrimitiveExpression(123))]);
  // 5 //
  Assert.throws(
    () => Tree.extract(
      "ctx",
