...
visitors.closure = ({1:block}) => (
  "(function callee () {" + visit1(block) + newline() +
  "})");
...
visitors.trap = ({1:string, 2:expressions, 3:serial}) => (
  (
    ...
    string === "apply" ||
    string === "construct") ?
  namespace + "." + string + "(" + mapjoin(expressions, visit1, "," + newline()) + ", " + serial + ")" :
  (
    expressions.length ?
...
visitors.While = ({1:labels, 2:expression, 3:block}) => (
  labelize(labels) + "while (" + newline() +
  visit1(expression) + ") {" + visit1(block) + newline() +
  "}");
...
visitors.Switch = ({1:labels, 2:block}) => (
  ((index) => (
    ...
    ArrayLite.join(
      ArrayLite.map(
        ArrayLite.slice(block[1], 0, index - 1,
        visit0),
      newline()) + newline(),
    labelize(labels) + "switch (true) {" + visit() + newline() +
    ArrayLite.join(
      ArrayLite.map(
        ArrayLite.slice(block[1], index, block[1].length),
        visit0),
      newline()) + newline() +
    "}}")))
