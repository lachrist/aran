...
/** @type {(unknown: unknown) => unknown is EstreeNode} */
const isEstreeNode = (unknown) =>
  typeof unknown === "object" && unknown !== null && hasOwn(unknown, "type");

/**
 * @type {(
 *   accumulation: { nodes: EstreePattern[], scope: Scope },
 *   node: EstreePattern,
 * ) => { nodes: EstreePattern[], scope: Scope}}
 */
...
const annotate = (node, scope) => {
  ...
  if (node.type === "Program") {
    return {
      ...node,
      body: reduce(node.body, accumulateDeclaration, {
        nodes: /** @type {EstreeProgramStatement[]} */ ([]),
        scope: {
          ...scope,
          ...reduceEntry(
            map(listKey(hoistBlock(node.body)), makeNegativeEntry),
...
const annotateDeclaration = (node, scope) => {
  ...
  if (node.type === "VariableDeclaration") {
    const { nodes, scope: new_scope } = reduce(
      ...
      {
        scope,
        nodes: /** @type {EstreeVariableDeclarator[]} */ ([]),
      },
    );
    return {
      node: {
  ...
  if (node.type === "SwitchCase") {
    const { nodes, scope: new_scope } = reduce(
      node.consequent,
      accumulateDeclaration,
      { nodes: /** @type {EstreeStatement[]} */ ([]), scope },
    );
    // switch (x) {
    //   case 123:
    //     y; // not initialized
...
/** @type {(node: EstreeNode, key: PropertyKey) => boolean} */
...
/** @type {(node: EstreeNode, key: PropertyKey, variable: string) => boolean} */
...
/** @type {(node: EstreeProgram) => EstreeProgram} */
export const annotateDeadzone = (node) => annotate(node, {});

/** @type {(node: EstreeIdentifier) => boolean | null} */
export const isDeadzone = (node) =>
  hasOwn(node, "deadzone") ? /** @type {boolean} */ (node.deadzone) : null;
