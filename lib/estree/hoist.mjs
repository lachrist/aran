...
/** @type {(node:estree.AssignmentProperty | estree.RestElement) => estree.Variable[]} */
...
/**@type {<K extends "var" | "let" | "const">(kind: K) => (variable: estree.Variable) => [estree.Variable, K]} */
...
/** @type {(nodes: estree.Node[]) => Record<estree.Variable, "function" | "var">} */
export const hoistClosure = (nodes) =>
  reduceEntry(flatMap(nodes, hoistClosureEntry));

/** @type {(node: estree.Node) => [estree.Variable, "let" | "const" | "class"][]} */
...
/** @type {(nodes: estree.Node[]) => Record<estree.Variable, "let" | "const" | "class">} */
