 ...
 * @type {(
 *   report: string,
 *   candidate: import("estree-sentry").Program<import("./hash").HashProp>,
 * ) => import("estree-sentry").ModuleProgram<import("./hash").HashProp>}
 */
const guardModuleProgram = (report, candidate) => {
  if (candidate.sourceType !== "module") {
    throw new AranConfigError({
      target: `${report}.sourceType`,
      expect: "'module'",
      actual: candidate.sourceType,
    });
  }
  return candidate;
};
 ...
 *   report: string,
 *   candidate: import("estree-sentry").Program<import("./hash").HashProp>,
 * ) => import("estree-sentry").ScriptProgram<import("./hash").HashProp>}
 */
const guardScriptProgram = (report, candidate) => {
  if (candidate.sourceType !== "script") {
    throw new AranConfigError({
      target: `${report}.sourceType`,
      expect: "'script'",
      actual: candidate.sourceType,
...
export const guardSource = (report, candidate) => {
  ...
  const root = guardWithAnnotation(
  ...
  switch (kind) {
    case "script": {
      return {
        ...
        root: guardScriptProgram(`${report}.root`, root),
      };
    }
    case "module": {
      return {
        kind,
        situ: guardGlobalSitu(`${report}.situ`, get(object, "situ")),
        root: guardModuleProgram(`${report}.root`, root),
      };
    }
    case "eval": {
      return {
