...
import { every, isEmptyArray, map } from "./util/index.mjs";
...
export const makeProgram = (kind, situ, head, body, tag) => {
  if (isHeadlessRoutineBlock(body)) {
    ...
    throw new AranError("program body cannot have head", {
      kind,
      situ,
      head,
      body,
...
export const makeClosureExpression = (kind, asynchronous, body, tag) => {
  if (kind === "arrow" || kind === "function" || kind === "method") {
    if (isHeadlessRoutineBlock(body)) {
      ...
      throw new AranError("closure body cannot have head", {
        kind,
        asynchronous,
        body,
      });
