...
const proceed = (mapping, node1, node2, generate, callback) => {
  ...
  try {
    Fs.writeFileSync(__dirname + "/tmp1.txt", generate(node1), "utf8");
    Fs.writeFileSync(__dirname + "/tmp2.txt", generate(node2), "utf8");
    const child = ChildProcess.spawnSync("diff", ["--side-by-side", __dirname + "/tmp1.txt", __dirname + "/tmp2.txt"], {
      __proto__: null,
      encoding: "utf8"
    });
    /* istanbul ignore if */
    if (child.status !== 1) {
      throw new global_Error("Unexpected status from diff command");
    }
    /* istanbul ignore if */
    if (child.stderr !== "") {
      throw new global_Error("Non empty stderr from diff command");
    }
    /* istanbul ignore if */
    if (child.signal !== null) {
      throw new global_Error("Unexpected signal from diff command");
    }
    return callback(false, mapping + "\n" + child.stdout);
  } finally {
    Fs.unlinkSync(__dirname + "/tmp1.txt");
    Fs.unlinkSync(__dirname + "/tmp2.txt");
  }
};
...
const check = (path, primitive1, primitive2, callback, argument) => (
  primitive1 !== primitive2 ?
  `Structural mismatch at ${path} between ${print(primitive1)} and ${print(primitive2)}` :
  callback(argument));

const check_all = (path, primitives1, primitives2, callback, _next) => (
  _next = (index) => (
...
const statement_callback_object = {
  ...
  Return: (path, statement1, statement2, expression1, expression2) => Tree._allign_expression(
    ...
    expression1,
    expression2),
  Break: (path, statement1, statement2, label1, label2) => check(path + ".break-label", label1, label2, EMPTY),
  Continue: (path, statement1, statement2, label1, label2) => check(path + ".continue-label", label1, label2, EMPTY),
...
const expression_callback_object = {
  __type_mismatch__: type_mismatch,
  primitive: (path, expression1, expression2, primitive1, primitive2) => check(path + ".primitive-body", primitive1, primitive2, EMPTY),
  builtin: (path, expression1, expression2, builtin1, builtin2) => check(path + ".builtin-body", builtin1, builtin2, EMPTY),
  ...
  throw: (path, expression1, expression2, expression11, expression21) => Tree._allign_expression(
    ...
    expression11,
    expression21),
  eval: (path, expression1, expression2, identifiers1, expression11, identifiers2, expression21) => check(
    path + ".eval-identifiers",
    identifiers1.length,
    identifiers2.length,
    () => combine(
      path,
