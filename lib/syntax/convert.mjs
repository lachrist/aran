...
/** @type {(loc: estree.SourceLocation | null | undefined) => Location} */
...
/** @type {(node: estree.Pattern) => aran.Parameter | aran.Variable}  */
...
/** @type {(node: estree.VariableDeclarator) => aran.Variable} */
...
/** @type {(node: estree.VariableDeclaration) => aran.Variable[]} */
...
/** @type {(node: estree.Program) => aran.Program<aran.Variable, Location>} */
...
/** @type {(node: estree.Node) => aran.Link<Location>} */
export const convertLink = (node) => {
  if (node.type === "ImportDeclaration") {
    if (node.specifiers.length === 0) {
      ...
      return makeImportLink(
        /** @type {estree.Source} */ (node.source.value),
        /** @type {estree.Specifier} */ (node.specifiers[0].imported.name),
        locate(node.loc),
      );
    }
    ...
    if (node.source == null) {
      ...
      return makeExportLink(
        /** @type {estree.Specifier} */ (node.specifiers[0].exported.name),
        locate(node.loc),
      );
    } else {
      return makeAggregateLink(
        /** @type {estree.Source} */ (node.source.value),
        /** @type {estree.Specifier} */ (node.specifiers[0].local.name),
        /** @type {estree.Specifier} */ (node.specifiers[0].exported.name),
        locate(node.loc),
      );
    }
...
/** @type {(nodes: estree.Node[]) => { head: aran.Variable[], tail: estree.Node[]}} */
...
/** @type {(nodes: estree.Node[], parent: estree.Node) => aran.PseudoBlock<aran.Variable, Location>} */
...
/** @type {(node: estree.Node) => aran.ClosureBlock<aran.Variable, Location>} */
...
/** @type {(node: estree.Node, labels: estree.Label[]) => aran.ControlBlock<aran.Variable, Location>} */
...
/** @type {(node: estree.Node) => aran.ControlBlock<aran.Variable, Location>} */
...
/** @type {(node: estree.BlockStatement) => aran.ControlBlock<aran.Variable, Location>} */
...
/** @type {(node: estree.Node) => aran.Statement<aran.Variable, Location>} */
...
/** @type {(node: estree.Node) => estree.Specifier} */
...
/** @type {(node: estree.Node) => aran.Effect<aran.Variable, Location>[]} */
  ...
  if (node.type === "ConditionalExpression") {
    ...
    if (node.left.type === "Identifier") {
      ...
      return {
        ...
        variable: /** @type {estree.Variable} */ (node.left.elements[0].name),
        right: convertExpression(node.right),
        tag: locate(node.loc),
      };
    } /* c8 ignore start */ else {
...
/** @type {(node: estree.Node) => aran.Expression<aran.Variable, Location>} */
export const convertExpression = (node) => {
  if (node.type === "Literal") {
    ...
    return {
      ...
      variable: /** @type {estree.Variable} */ (node.argument.elements[0].name),
      tag: locate(node.loc),
    };
    ...
    return {
      ...
      variable: /** @type {estree.Variable} */ (node.elements[0].name),
      tag: locate(node.loc),
    };
  } else if (node.type === "Identifier") {
    if (node.name === "undefined") {
    ...
    return {
      ...
      source: /** @type {estree.Source} */ (node.left.value),
      import:
...
/** @type {(node: estree.Program) => aran.Node<aran.Variable, Location>} */
