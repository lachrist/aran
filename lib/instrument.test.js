...
const block = (tag, cut, callee, identifiers, statements) => (
  ...
  `{
    try {
      ...
      throw ${trap("failure", cut.serial, JSON.stringify(tag), "error")}; }
    finally {
...
test(
  `
    ...
    export exported1;
    aggregate imported1 from "source" as exported2;
    {}`,
  [
    ["enter", "module", [], {__proto__: null}, [], "BLOCK1"],
    ["aggregate", "imported2", "source", "exported2"]
  ],
  `{}`,
  `{}`);
...
test(
  ...
  [
    ...
    ["leave", "script", "BLOCK1"]],
  `${block((counter++, "script"), {serial:counter++, labels:[]}, null, [advice_identifier, parameters_identifier], `
