    ...
    ${(
      `input = ${trap(
        ...
        `{
          ...
          labels: #Array.of(${frame.labels.map(show.label).map(JSON.stringify).join(", ")}),
          identifiers: #Array.of(${frame.identifiers.filter(is_not_namespace_identifier).map(show.identifier).map(JSON.stringify).join(", ")})}`,
        "input")};`)}
    ${statements}
  ...
  Instrument(
      ...
      return #global;}`),
    {
      ...
      pointcut: {
        ...
        primitive: true,
        builtin: false},
      namespace: {callee:callee_identifier}}),
  Lang._parse_program(`{
