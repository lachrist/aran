      ...
      ${(
        ...
        `(
          ...
          ${parameter_array_object[tag].reduce(
            ...
            trap("enter", JSON.stringify(tag), `#Array.of(${cut.labels.map(show.label).map(JSON.stringify).join(", ")})`, parameters_identifier, `#Array.of(${identifiers.filter((identifier) => !identifier.startsWith(callee_identifier)).map(show.identifier).map(JSON.stringify).join(", ")})`, String(cut.serial)))});` :
        `${trap("enter", JSON.stringify(tag), `#Array.of(${cut.labels.map(show.label).map(JSON.stringify).join(", ")})`, `{__proto__:null}`, `#Array.of(${identifiers.filter((identifier) => !identifier.startsWith(callee_identifier)).map(show.identifier).map(JSON.stringify).join(", ")})`, String(cut.serial))};`)}
      ${statements}
...
test(
  `{
      ...
      return 456;}`,
  [
    ["enter", "program", [], {__proto__: null}, [], "BLOCK1"],
    ["debugger", "Debugger1"],
...
test(
  ...
  [
    // Begin //
    ["enter", "program", [], {__proto__: null}, [show.identifier("x")], "BLOCK1"],
    // builtin //
...
test(
  ...
  `{
    return () => {
      let ${advice_identifier}, ${parameters_identifier};
      ${advice_identifier} = #Reflect.get(arguments, 0);
      ${block("program", {serial:0, labels:[]}, null, [callee_identifier + "0", callee_identifier + "1", callee_identifier + "2", callee_identifier + "3"], `
        ${trap("drop", `(
