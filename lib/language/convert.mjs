...
const convertPseudoBlock = (nodes, parent) => {
...
const convertClosureBlock = (node) => {
...
const convertControlBlock = (node, labels) => {
  if (node.type === "LabeledStatement") {
    return convertControlBlock(node.body, [
      ...labels,
...
export const convertStatement = (node) => {
  if (node.type === "BlockStatement") {
    ...
    return {
      type: "TryStatement",
      try: convertControlBlock(node.block, []),
      catch: convertControlBlock(node.handler.body, []),
      finally: convertControlBlock(node.finalizer, []),
      tag: locate(node.loc),
    };
