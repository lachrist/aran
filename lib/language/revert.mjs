...
/** @type {(node: Program) => EstreeProgram} */
...
/** @type {(node: Link) => EstreeModuleDeclaration} */
export const revertLink = (node) => {
  if (node.type === "ImportLink") {
    ...
    if (node.import === null) {
      ...
      if (node.export === null) {
        throw new DynamicError("aggregate named link requires an export", node);
      }
      return {
        type: "ExportNamedDeclaration",
        declaration: null,
...
/** @type {(node: Block) => EstreeBlockStatement} */
...
/** @type {(node: Block) => EstreeStatement} */
...
export const revertNakedBlock = (node) => {
  if (node.labels.length > 0) {
    throw new DynamicError("unexpected label in block", node);
  }
  return revertInnerBlock(node);
};

/** @type {(node:Statement) => EstreeStatement} */
...
/** @type {(effects: Effect[]) => EstreeExpression} */
...
/** @type {(effect: Effect) => EstreeExpression} */
...
/** @type {(node: Expression) => EstreeExpression} */
