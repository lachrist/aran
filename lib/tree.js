...
const syntax = {
  ...
  statement: {
    ...
    Debugger: [],
    Bundle: [["statement"]],
    EnclaveDeclare: ["enclave-kind", "enclave-write-identifier", "expression"],
    // BlockFull //
    Lone: ["block"],
...
exports._toggle_debug_mode = () => {
  ...
  const check = (type, node) => {
    if (typeof type === "string") {
      ...
      } else if (type in enumerations) {
        Throw.assert(ArrayLite.includes(enumerations[type], node), null, `Invalid enumeration-based atomic node: expected a ${type}, got ${Throw.inspect(node)}`);
      } else /* istanbul ignore else */ if (type in predicates) {
        Throw.assert(predicates[type](node), null, `Invalid predicate-based atomic node: expected a ${type}, got ${Throw.inspect(node)}`);
      } else {
        Throw.abort(null, `Unrecognized type: ${type}`);
      }
      return node;
    }
