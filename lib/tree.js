...
const syntax = {
  ...
  Expression: {
    ...
    EvalExpression: [["VariableIdentifier", "*"], "Expression"],
...
const enumerations = {
  ...
  Enclave: [
    ...
    "strict-module",
    // Script //
    "strict-script", "sloppy-script",
    // Eval //
    "strict-program", "sloppy-program",
...
exports.toggleDebugMode = () => {
  ...
  const closures = ((() => {
    ...
    const types = {
      ...
      enclave_loose_variable_array: "array",
      enclave_rigid_variable_array: "array"
    };
    ...
    const enclaves = ArrayLite.filter(enumerations.Enclave, (enclave) => enclave !== null);
    const whitelist = {
      __proto__: null,
      enclave_variable_array: enclaves,
      enclave_this: enclaves,
      enclave_arguments: enclaves,
      enclave_new_target: [
        "strict-derived-constructor",
        "strict-constructor",
        "strict-method", "sloppy-method",
      ...
      enclave_loose_variable_array: [
        ...
        "sloppy-program",
        "strict-script", "sloppy-script"]};
    const checkout = (enclave, result) => ArrayLite.forEach(
      keys,
      (key) => Throw.assert(
        (
          (
            key in whitelist &&
            ArrayLite.includes(whitelist[key], enclave)) ||
          (
            types[key] === "array" ?
            result[key].length === 0 :
            result[key] === false)),
        null,
        `Illegal ${key}${key in whitelist ? ` for ${enclave === null ? "absent" : enclave} enclave` : ""}`));
    ...
    const make_enclave_read_result = (identifier) => (
      identifier === "new.target" ?
      {
        __proto__: empty,
        enclave_new_target: true} :
      (
        identifier === "this" ?
        {
          __proto__: empty,
        ...
        (
          ...
          {
            __proto__: empty,
            enclave_arguments: true} :
          {
            __proto__: empty,
            enclave_variable_array: [identifier]})));
    ...
    return {
      ...
      ScriptProgram: (enclave, result) => (
        ...
        Throw.assert(
          (
            enclave === null ||
            enclave === "sloppy-script" ||
            enclave === "strict-script"),
          null,
      ...
      EvalProgram: (enclave, identifiers, result) => (
        ...
        ArrayLite.forEach(
          ...
          (identifier) => Throw.assert(
            ...
            null,
            `Program-unbound variable: ${identifier}`)),
        checkout(
          enclave,
          {
            __proto__: result,
      ...
      Block: (identifiers, result) => (
        ...
        Throw.assert(result.enclave_rigid_variable_array.length === 0, null, `Cannot have rigid enclave variable declaration inside blocks`),
        {
          __proto__: result,
          completion: false,
      ...
      DeclareEnclaveStatement: (kind, identifier, result) => combine(
        ...
        {
          __proto__: empty,
          [`enclave_${kind === "var" ? "loose" : "rigid"}_variable_array`]: [identifier]}),
      // Statement >> Compound //
      ...
      ReadEnclaveExpression: (identifier) => make_enclave_read_result(identifier),
      TypeofEnclaveExpression: (identifier) => make_enclave_read_result(identifier),
      ClosureExpression: (sort, asynchronous, generator, result) => (
        Throw.assert(
          result.enclave_loose_variable_array.length === 0,
          null,
          ...
          `Constructor closure expression cannot be asynchronous nor generator`),
        {
          ...
          enclave_variable_array: result.enclave_variable_array}),
      // Expression >> Consumer //
      ...
      EvalExpression: (identifiers, result) => combine(
        check_not_assignment(result),
        {
          __proto__: empty,
          variables: identifiers}),
