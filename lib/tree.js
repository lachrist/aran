...
exports.toggleDebugMode = () => {
  ...
  const check = (type, node) => {
    if (typeof type === "string") {
      if (type === "Expression*" || type in database) {
        Throw.assert(global_Reflect_apply(global_WeakSet_prototype_has, database[type === "Expression*" ? "Expression" : type], [node]), null, `Invalid compound node: expected a ${type}, got ${Throw.inspect(node)}`);
        if (type === "Expression") {
          Throw.assert(node[0] !== "WriteExpression" && node[0] !== "ExportExpression" && node[0] !== "WriteEnclaveExpression", null, `Assignment expression should be directly dropped`);
        }
      } else {
