...
/** @type {(escape: estree.Variable, variable: weave.Variable) => estree.Variable} */
const escapeVariable = (escape, variable) =>
  /** @type {estree.Variable} */ (`${escape}${variable}`);

/**
 * @type {(
 *   node: weave.ControlBlock,
 *   escape: estree.Variable,
 *   variables: weave.Variable[],
 * ) => weave.ControlBlock}
 */
export const escapeControlBlock = (node, escape, variables) => {
  const slice = /** @type {weave.Variable[]} */ (
    ...
    case "WriteEffect":
      if (includes(variables, node.variable)) {
        return makeWriteEnclaveEffect(
          escapeVariable(escape, /** @type {weave.Variable} */ (node.variable)),
          escapeExpression(node.right, escape, variables),
        );
      } else {
