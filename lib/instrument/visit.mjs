...
const compileContextCache = (context) => {
  const { stringifyLabel, stringifyVariable } = context;
  return {
    makers: {
      ...
      makeVariableExpression: context.inline.variable
        ...
        : (digest_variable) =>
            makeMetaDefineExpression(
              mangleVarVariable(stringifyVariable(digest_variable)),
              digest_variable,
            ),
      makeSerialExpression: context.inline.serial
        ? makeJsonExpression
        : (path, serial) =>
            makeMetaDefineExpression(mangleNewVariable(path, "serial"), serial),
    },
  };
};
    ...
    listOptimizeTryStatement(
      [
        ...(trap === null
          ...
          : [
              makeEffectStatement(
                makeWriteEffect(mangleArgVariable(null), trap),
              ),
              ...map(parent.parameters, (parameter) =>
                makeEffectStatement(
                  makeMetaWriteEffect(
                    ...
                    makeGetExpression(
                      makeMetaReadExpression(mangleArgVariable(null)),
...
export const instrumentExpression = (node, context, cache) => {
  ...
  switch (node.type) {
    ...
    case "ParameterExpression":
      return makeTrapExpression(context, cache, {
        ...
        value: context.overwritten[node.parameter]
          ? makeMetaReadExpression(mangleArgVariable(node.parameter))
          : node,
        serial,
      });
    ...
    case "ClosureExpression": {
      ...
      return hasMetaSaveExpression(variable, [expression])
        ? makeSequenceExpression(
            makeMetaWriteEffect(variable, expression),
            makeMetaDefineExpression(variable, undefined),
          )
        : expression;
    }
