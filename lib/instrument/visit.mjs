...
/**
 * @type {<S extends Json, L extends Json, V extends Json>(
 *   makePoint: (input: Expression<Usage>) => Point<S, L, V>,
 *   parameters: Parameter[],
 *   context: Context<S, L, V>
 * ) => Statement<Usage>[]}
 */
const listEnterStatement = (makePoint, parameters, context) => {
  ...
  const triggered = cut(point, context.pointcut);
  if (triggered) {
    const right = makeTriggerExpression(point, context);
...
const instrumentClosureBlock = (
  ...
  const enter_statement_array = listEnterStatement(
    (expression) =>
      makeEnterPoint(
        expression,
...
const instrumentPseudoBlock = (
  ...
  const enter_statement_array = listEnterStatement(
    (expression) => makeEnterPoint(expression, node.tag),
    parameters,
    context,
  );
  const overwritten = {
