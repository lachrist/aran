...
NAMESPACE._builtin_AranThrowReferenceError = function (message) {
  throw new NAMESPACE._builtin_ReferenceError(message);
};

NAMESPACE._builtin_AranThrowTypeError = function (message) {
  throw new NAMESPACE._builtin_TypeError(message);
};

NAMESPACE._builtin_AranToObject = function (object) {
  if (object === null || object === void 0)
    return object;
  return NAMESPACE._builtin_Object(object);
};

NAMESPACE._builtin_AranHold = function (object, name) {
  while (object) {
...
NAMESPACE._builtin_AranRest = function (iterator) {
  var result = [];
...
NAMESPACE._builtin_AranInitializeObject = function () {
  var object = {};
  for (var index = 0, length = arguments.length; index < length; index += 3) {
    NAMESPACE._builtin_Object_defineProperty(object, arguments[index+1], (
      ...
      {configurable:true, writable:true, [arguments[index]]:arguments[index+2]}));
  }
  return object;
};

NAMESPACE._builtin_AranUnary = function (operator, argument) {
  switch (operator) {
...
NAMESPACE._builtin_AranBinary = function (operator, left, right) {
  switch (operator) {
