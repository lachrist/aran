 ...
 * ) => aran.Expression<weave.ResAtom>}
 */
export const makeObjectExpression = (entries) =>
  makeApplyExpression(
    makeIntrinsicExpression("Object.fromEntries"),
    makePrimitiveExpression({ undefined: null }),
    [makeArrayExpression(map(entries, makeArrayExpression))],
  );
...
export const makeJsonExpression = (json) => {
  if (
    ...
    return makeObjectExpression(map(listEntry(json), makeJsonEntry));
  } else {
