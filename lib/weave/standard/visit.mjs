...
const ADVICE_RENAME = /** @type {import("../rename").Rename} */ ("");

const ADVICE_VARIABLE = mangleAdviceVariable(ADVICE_RENAME);
 ...
 *   kind: import("./aspect").ControlKind,
 *   context: import("./context").Context,
...
const weaveStatement = (node, context) => {
  switch (node.type) {
    ...
    case "ReturnStatement": {
      const argument = weaveExpression(node.result, context);
      return [
        makeReturnStatement(
    ...
    case "IfStatement": {
      const argument = weaveExpression(node.test, context);
      return [
        makeIfStatement(
          context.pointcut["test@before"]("if", node.tag)
            ? makeTrapExpression(
                "test@before",
                context.depth,
                [makePrimitiveExpression("if"), argument],
                node.tag,
              )
            : argument,
        ...
        makeWhileStatement(
          context.pointcut["test@before"]("while", node.tag)
            ? makeTrapExpression(
                "test@before",
                context.depth,
                [makePrimitiveExpression("while"), argument],
                node.tag,
              )
            : argument,
...
const weaveEffect = (node, context) => {
  switch (node.type) {
    case "ExpressionEffect": {
      const argument = weaveExpression(node.discard, context);
    ...
    case "WriteEffect": {
      const argument = weaveExpression(node.value, context);
      return makeWriteEffect(
        ...
        context.pointcut["write@before"](node.variable, node.tag)
          ? makeTrapExpression(
              "write@before",
              context.depth,
              [makePrimitiveExpression(node.variable), argument],
              node.tag,
            )
          : argument,
      );
      ...
      return makeExportEffect(
        ...
        context.pointcut["export@before"](node.export, node.tag)
          ? makeTrapExpression(
              "export@before",
              context.depth,
              [makePrimitiveExpression(node.export), argument],
              node.tag,
            )
          : argument,
      );
      ...
      return makeConditionalEffect(
        context.pointcut["test@before"]("conditional", node.tag)
          ? makeTrapExpression(
              "test@before",
              context.depth,
              [makePrimitiveExpression("conditional"), argument],
              node.tag,
            )
          : argument,
...
const weaveExpression = (node, context) => {
  switch (node.type) {
    ...
    case "YieldExpression": {
      const argument1 = weaveExpression(node.item, context);
      const argument2 = context.pointcut["yield@before"](
        node.delegate,
        node.tag,
      )
        ? makeTrapExpression(
            "yield@before",
            context.depth,
            [makePrimitiveExpression(node.delegate), argument1],
            node.tag,
          )
        : argument1;
      const result1 = makeYieldExpression(node.delegate, argument2);
      const result2 = context.pointcut["yield@after"](node.delegate, node.tag)
        ? makeTrapExpression(
            "yield@after",
            context.depth,
    ...
    case "AwaitExpression": {
      const argument1 = weaveExpression(node.promise, context);
      const argument2 = context.pointcut["await@before"](node.tag)
        ? makeTrapExpression(
            "await@before",
            context.depth,
            [argument1],
        ...
        : argument1;
      const result1 = makeAwaitExpression(argument2);
      const result2 = context.pointcut["await@after"](node.tag)
        ? makeTrapExpression("await@after", context.depth, [result1], node.tag)
        : result1;
      return result2;
    }
    case "EvalExpression": {
      if (hasOwn(context.reboot, node.tag)) {
        const argument1 = weaveExpression(node.code, context);
        if (context.pointcut["eval@before"](node.tag)) {
          const argument2 = makeTrapExpression(
            "eval@before",
            context.depth,
            [
              makeJsonExpression({
          ...
          const result1 = makeEvalExpression(argument2);
          const result2 = context.pointcut["eval@after"](node.tag)
            ? makeTrapExpression(
                "eval@after",
                context.depth,
                [result1],
          ...
          return makeSequenceExpression(
            [makeExpressionEffect(argument1)],
            makeApplyExpression(
              makeIntrinsicExpression("aran.throw"),
              makeIntrinsicExpression("undefined"),
              [
    ...
    case "ApplyExpression": {
      ...
      const self = weaveExpression(node.this, context);
      const input = map(node.arguments, (argument) =>
