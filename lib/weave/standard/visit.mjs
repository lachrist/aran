...
import {
  ...
  trapSegmentBlockSetup,
  trapClosureBlockSetup,
  trapProgramBlockSetup,
} from "./trap.mjs";
...
const weaveSegmentBlock = (node, parent, context1) => {
  ...
  const setup = trapSegmentBlockSetup(
    context1.pointcut,
    context1.depth,
    parent.kind,
    tag,
 ...
 */
const trapRoutineBlockSetup = (tag, parent, { pointcut, depth }) => {
  switch (parent.type) {
    case "program": {
      return trapProgramBlockSetup(
        pointcut,
        depth,
        parent.initial,
        parent.kind,
    ...
    case "closure": {
      return trapClosureBlockSetup(pointcut, depth, parent.kind, tag);
    }
    default: {
...
const weaveRoutineBlock = (node, parent, context1) => {
  ...
  const parameters = listParameter(parent.kind);
  const variables = map(node.bindings, getBindingVariable);
  const setup = trapRoutineBlockSetup(tag, parent, context1);
  const context2 = isTreeEmpty(setup)
    ? context1
    : { ...context1, depth: incrementDepth(context1.depth) };
  const declaration = trapBlockDeclarationOverwrite(
