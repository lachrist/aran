...
export const ADVICE_RENAME = /** @type {import("../rename").Rename} */ ("");

export const ADVICE_VARIABLE = mangleAdviceVariable(ADVICE_RENAME);
...
export const trapBlockFrame = (
  ...
  predicate(kind, path)
    ? [
        makeEffectStatement(
          makeExpressionEffect(
            makeTrapExpression("block@frame", [
              makeReadExpression(mangleStateVariable(depth)),
              makePrimitiveExpression(kind),
 ...
 * ) => import("../atom").ResStatement[]}
 */
export const trapBlockOverframe = (
  { "block@overframe": predicate },
  depth,
  kind,
  parameters,
  variables,
  ...
  predicate(kind, path)
    ? concat_XX(
        makeEffectStatement(
          makeWriteEffect(
            FRAME_VARIABLE,
            makeTrapExpression("block@overframe", [
              makeReadExpression(mangleStateVariable(depth)),
              makePrimitiveExpression(kind),
              makeFrameExpression(parameters, variables),
              makePrimitiveExpression(path),
 ...
 *   depth: import("../depth").Depth,
 *   kind: import("../parametrization").BlockKind,
 *   argument: import("../atom").ResExpression | null,
 *   path: import("../../path").Path,
 * ) => import("../atom").ResExpression | null}
 */
export const trapBlockSuccess = (
  { "block@success": predicate },
  depth,
  kind,
  argument,
  path,
  ...
  predicate(kind, path)
    ? makeTrapExpression("block@success", [
        makeReadExpression(mangleStateVariable(depth)),
        makePrimitiveExpression(kind),
        argument ?? makeIntrinsicExpression("undefined"),
        makePrimitiveExpression(path),
      ])
    : argument;
 ...
 * ) => import("../atom").ResExpression}
 */
export const trapBlockFailure = (
  { "block@failure": predicate },
  depth,
  kind,
  argument,
  path,
  ...
  predicate(kind, path)
    ? makeTrapExpression("block@failure", [
        makeReadExpression(mangleStateVariable(depth)),
        makePrimitiveExpression(kind),
        argument,
        makePrimitiveExpression(path),
...
export const trapReturnBefore = (
  ...
  predicate(path)
    ? makeTrapExpression("return@before", [
        makeReadExpression(mangleStateVariable(depth)),
        argument,
        makePrimitiveExpression(path),
      ])
...
export const trapClosureAfter = (
  ...
  predicate(kind, asynchronous, generator, path)
    ? makeTrapExpression("closure@after", [
        makeReadExpression(mangleStateVariable(depth)),
        makePrimitiveExpression(kind),
        makePrimitiveExpression(asynchronous),
        makePrimitiveExpression(generator),
        result,
        makePrimitiveExpression(path),
      ])
    : result;
