...
export const trapRoutineBlockBefore = (
  ...
  if (isProgramKind(kind)) {
    return program_predicate(kind, path)
      ? [
          makeExpressionEffect(
    ...
    return closure_predicate(kind, path)
      ? [
          makeExpressionEffect(
...
export const trapControlBlockBefore = (
  { "control-block@before": predicate },
  depth,
  kind,
  labels,
  path,
  ...
  predicate(kind, path)
    ? [
        makeExpressionEffect(
          makeTrapExpression("control-block@before", [
            makeReadExpression(mangleStateVariable(depth)),
            makePrimitiveExpression(kind),
...
export const trapControlBlockAfter = (
  ...
  predicate(kind, path)
    ? [
        makeExpressionEffect(
          makeTrapExpression("control-block@after", [
            makeReadExpression(mangleStateVariable(depth)),
            makePrimitiveExpression(kind),
            makePrimitiveExpression(path),
          ]),
