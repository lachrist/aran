...
/**
 * @type {<L>(
 *   pointcut: import("./pointcut").ObjectPointcut<L>,
...
const cutObject = (pointcut, point) => {
  switch (point.type) {
    ...
    case "block@failure": {
      return cut3(pointcut["block@failure"], point.frame, null, point.location);
    }
    case "block@completion": {
      return cut3(
        pointcut["block@completion"],
        point.frame,
        null,
        point.location,
      );
    ...
    case "block@leave": {
      return cut2(pointcut["block@leave"], point.frame, point.location);
    }
    ...
    case "branch@after": {
      return cut3(pointcut["branch@after"], point.kind, null, point.location);
    }
    case "import@after": {
      return cut4(
