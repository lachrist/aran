...
const weaveSegmentBlock = (target, parent, old_context) => {
   ...
   */
  const bindings = [
    has_no_setup ? null : makeStateBinding(context.depth),
    has_no_declaration ? null : FRAME_BINDING,
...
const weaveRoutineBlock = (target, parent, old_context) => {
  ...
  /**
   * @type {import("../../util/tree").Tree<import("../atom").ResBinding>}
   */
  const bindings = [
    is_program
      ? mapTree(
          map(listValue(old_context.pointcut), listName),
          makeAdviceBinding,
        )
      : null,
    is_program ? makeStateBinding(old_context.depth) : null,
    has_no_declaration ? null : FRAME_BINDING,
