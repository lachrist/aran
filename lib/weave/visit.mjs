 ...
 *   node: aran.Node<weave.ArgAtom>,
 * ) => weave.OriginPath}
 */
const updateOriginPath = (origin, node) =>
  node.tag.origin !== null ? node.tag.origin : origin;
 ...
 */
export const weaveProgram = ({ node, path }, parent_origin, options) => {
  const origin = updateOriginPath(parent_origin, node);
  switch (node.type) {
    case "EvalProgram":
      return makeEvalProgram(
        weaveClosureBlock(drill({ node, path }, "body"), origin, options, {
          type: "program",
          kind: "eval",
          links: [],
        }),
    ...
    case "ScriptProgram":
      return makeScriptProgram(
        escapePseudoBlock(
          weavePseudoBlock(drill({ node, path }, "body"), origin, options, {
            type: "program",
            kind: "script",
            links: [],
          }),
 ...
 */
const weaveClosureBlock = ({ node, path }, parent_origin, options, parent) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  drill({ node, path }, "completion");
  ...
  const block = trapClosureBlock(
    {
      statements: [
        ...
        ...flatMap(drillAll(drillArray({ node, path }, "statements")), (pair) =>
          weaveStatement(pair, origin, options),
        ),
      ],
      completion: makeTrapExpression(
        makeCompletionPoint(
          parent,
          weaveExpression(drill({ node, path }, "completion"), origin, options),
          location,
        ),
        options,
      ),
 ...
 */
const weaveControlBlock = ({ node, path }, parent_origin, options, parent) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  const location = locate(options.root, origin, path);
 ...
 */
const weavePseudoBlock = ({ node, path }, parent_origin, options, parent) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  const location = locate(options.root, origin, path);
  const statements = flatMap(
    drillAll(drillArray({ node, path }, "statements")),
    (pair) => weaveStatement(pair, origin, options),
  );
 ...
 */
const weaveStatement = ({ node, path }, parent_origin, options) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  const location = locate(options.root, origin, path);
  switch (node.type) {
    case "EffectStatement":
      return map(
        weaveEffect(drill({ node, path }, "inner"), origin, options),
        makeEffectStatement,
      );
    ...
    case "BlockStatement":
      return [
        makeBlockStatement(
          weaveControlBlock(drill({ node, path }, "do"), origin, options, {
            type: "block",
            kind: "naked",
          }),
        ),
    ...
    case "IfStatement":
      return [
        makeIfStatement(
          ...
          weaveControlBlock(drill({ node, path }, "then"), origin, options, {
            type: "block",
            kind: "then",
          }),
          weaveControlBlock(drill({ node, path }, "else"), origin, options, {
            type: "block",
            kind: "else",
          }),
        ),
    ...
    case "TryStatement":
      return [
        makeTryStatement(
          weaveControlBlock(drill({ node, path }, "try"), origin, options, {
            type: "block",
            kind: "try",
          }),
          weaveControlBlock(drill({ node, path }, "catch"), origin, options, {
            type: "block",
            kind: "catch",
          }),
          weaveControlBlock(drill({ node, path }, "finally"), origin, options, {
            type: "block",
            kind: "finally",
          }),
        ),
 ...
 */
const weaveEffect = ({ node, path }, parent_origin, options) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  const location = locate(options.root, origin, path);
  switch (node.type) {
      ...
      return [
        makeConditionalEffect(
          ...
          flatMap(drillAll(drillArray({ node, path }, "positive")), (pair) =>
            weaveEffect(pair, origin, options),
          ),
 ...
 * ) => aran.Expression<weave.ResAtom>}
 */
export const weaveExpression = ({ node, path }, parent_origin, options) => {
  const origin = updateOriginPath(parent_origin, node);
  const { locate } = options;
  const location = locate(options.root, origin, path);
  switch (node.type) {
    ...
    case "SequenceExpression":
      return reduceReverse(
        weaveEffect(drill({ node, path }, "head"), origin, options),
        makeFlipSequenceExpression,
        weaveExpression(drill({ node, path }, "tail"), origin, options),
      );
    ...
    case "ApplyExpression":
      return makeTrapExpression(
        {
          type: "apply",
          callee: weaveExpression(
            drill({ node, path }, "callee"),
            origin,
            options,
          ),
          this: weaveExpression(drill({ node, path }, "this"), origin, options),
          arguments: map(
            drillAll(drillArray({ node, path }, "arguments")),
            (pair) => weaveExpression(pair, origin, options),
          ),
          location,
        },
        options,
