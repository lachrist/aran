          ...
          /** @type {weave.ResVariable[]} */ (listKey(body.tag)),
          {
            makeReadExpression: (variable, _free) =>
              makeGetExpression(
                ...
                makePrimitiveExpression(`${root}.${variable}`),
              ),
...
const listParentParameter = (parent) => {
  if (parent.type === "block") {
    ...
    if (parent.kind === "eval") {
      ...
      throw new AranTypeError("invalid program kind", parent.kind);
    }
  } else if (parent.type === "function") {
    return parent.kind === "arrow"
      ? ["function.arguments"]
      : ["new.target", "this", "function.arguments"];
  } else {
...
const weaveClosureBlock = ({ node, path }, options, parent) => {
  ...
  const location = locate(node.tag.path, options.root);
  ...
  const [variables, bindings] = unzip(
    filter(
      ...
      ([_variable, binding]) => {
        ...
        if (binding.type === "callee" && parent.type === "function") {
...
export const weaveExpression = ({ node, path }, options) => {
  ...
  switch (node.type) {
    ...
    case "FunctionExpression": {
      const body = weaveClosureBlock(drill({ node, path }, "body"), options, {
        type: "function",
        kind: node.kind,
        callee: path,
      });
      ...
      return some(
        listValue(recordFree(expression)),
        (binding) => binding.type === "callee" && binding.path === path,
      )
        ? makeSequenceExpression(
            makeWriteCalleeEffect(path, expression),
