...
import {
  listUnsafeParameterEarlyEffect,
  makeClosureFrame,
  makeControlFrame,
} from "./frame.mjs";
...
export const weaveProgram = ({ node, path }, options) =>
  makeProgram(
    ...
    node.situ,
    node.head,
    weaveClosureBlock(drill({ node, path }, "body"), options, node),
  );
...
const weaveClosureBlock = ({ node, path }, options, parent) => {
  ...
  if (schema.failure === null && schema.leave === null) {
    ...
    const setup = setupBinding(
      concatX_X(
        frame.type === "program" ? [pairup(ADVICE_VARIABLE, undefined)] : [],
        pairup(COMPLETION_VARIABLE, undefined),
        bindings,
      ),
    );
    return makeClosureBlock(
      ...
      concatXXX_(
        map(
          parent.type === "Program"
            ? listUnsafeParameterEarlyEffect(parent)
            : [],
          makeEffectStatement,
        ),
