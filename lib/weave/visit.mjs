...
import { isParameterHeader, listHeaderParameter } from "../header.mjs";
 ...
 *   kind: import("../context").Context["source"],
 * ) => aran.Program<weave.ResAtom>}
 */
export const weaveProgram = ({ node, path }, options, kind) =>
  makeProgram(
    node.head,
...
const listParentParameter = (parent) => {
  switch (parent.type) {
    ...
    case "program": {
      return flatMap(
        filterNarrow(parent.head, isParameterHeader),
        listHeaderParameter,
      );
    }
    default: {
