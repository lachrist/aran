...
const flaten = (statement) => statement[0] === "Bundle" ? ArrayLite.flatMap(statement[1], flaten) : [statement];
...
exports._toggle_normal_mode = () => {
  ...
  ArrayLite.forEach(["expression", "statement"], (type) => {
  ...
  const check = (type, node) => {
    if (typeof type === "string") {
      if (type === "block" || type === "statement" || type === "expression") {
        ...
        // console.assert(type === "primitive" || type === "identifier" || type === "label");
...
exports._dispatch_expression = (expression, context, callbacks) => {
...
exports._dispatch_statement = (statement, context, callbacks) => {
...
exports._dispatch_block = (block, context, callbacks) => callbacks[block[0]](context, block, block[1], block[2]);
