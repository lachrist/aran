...
import { mangleEval, mangleParameter, mangleProxy } from "./mangle.mjs";
import {
 ...
 * ) => estree.Expression}
 */
const makeEvalArrow = (situ, config) => {
  switch (situ) {
    case "global": {
 ...
 * ) => estree.Statement}
 */
export const initializeEvalProgramParameter = (situ, config) => ({
  ...
  declarations: [
    {
      ...
      init: makeEvalArrow(situ, config),
    },
  ],
});
 ...
 * ) => string}
 */
const makeStaticBackKey = (header) => {
  if (
 ...
 * ) => estree.Expression}
 */
const makeDynamicBackKey = (parameter) => {
  if (
    parameter === "scope.read" ||
    parameter === "scope.typeof" ||
    parameter === "scope.discard" ||
 ...
 * ) => estree.Expression}
 */
const makeProxyParameter = (parameter, config) => {
  if (parameter === "this") {
    return {
      type: "ThisExpression",
    };
    ...
    return {
      ...
      body: {
        ...
        callee: {
          ...
          optional: false,
          computed: true,
          object: mangleProxy(config),
          property: makeDynamicBackKey(parameter),
        },
 ...
 * ) => estree.Statement}
 */
export const initializeProxyParameter = (parameter, config) => ({
  type: "VariableDeclaration",
  kind: "let",
  declarations: [
    {
      ...
      init: makeProxyParameter(parameter, config),
    },
  ],
});
...
const makeBackProperty = (header) => ({
  ...
  key: {
    type: "Literal",
    value: makeStaticBackKey(header),
  },
  value: makeBackArrow(header),
});
 ...
 * ) => estree.Statement[]}
 */
export const initializeBackRecord = (headers, config) =>
  concatXX_(
 ...
 * ) => string}
 */
const compileBackProperty = (header) =>
  `  ${makeStaticBackKey(header)}: ${compileBackArrow(header)},\n`;
 ...
 * ) => string}
 */
const compileBackRecord = (headers) =>
 ...
 * ) => estree.Statement[]}
 */
export const updateBackRecord = (headers, config) =>
  headers.length === 0
    ? []
    : [
        {
                ...
                },
                {
                  ...
                  argument: {
                    ...
                    arguments: [
                      {
                        type: "Literal",
                        value: compileBackRecord(headers),
                      },
                    ],
                  },
                },
