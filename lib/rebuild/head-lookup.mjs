 ...
 * ) => estree.Expression}
 */
const makeStaticLookupArrow = (operation, variables, context) => {
 ...
 * ) => estree.Expression}
 */
const makeDynamicLookupArrow = (operation, variables, context) => {
  const var_arg = mangleArgument("var", context.escape);
  const val_arg = mangleArgument("val", context.escape);
  const rec_arg = mangleArgument("rec", context.escape);
  return {
    type: "CallExpression",
    optional: false,
    callee: {
 ...
 */
export const listLookupDeclarator = (mode, head, context) => {
  ...
  return map(LOOKUP, (operation) => ({
    ...
    init: dynamic
      ? makeDynamicLookupArrow(operation, variables, context)
      : makeStaticLookupArrow(operation, variables, context),
  }));
};
