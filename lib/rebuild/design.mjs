...
import { every, hasOwn } from "../util/index.mjs";
...
export const isObjectDesign = (node) =>
  node.type === "ApplyExpression" &&
  node.callee.type === "IntrinsicExpression" &&
  node.callee.intrinsic === "Object.fromEntries" &&
  node.this.type === "PrimitiveExpression" &&
  node.arguments.length === 1 &&
  isListDesign(node.arguments[0], isPairDesign);

/**
 * @type {(
 *   node: aran.Expression<rebuild.Atom>,
 * ) => node is import("./design").EmptyObjectDesign}
 */
export const isEmptyObjectDesign = (node) =>
  node.type === "ApplyExpression" &&
  node.callee.type === "IntrinsicExpression" &&
  node.callee.intrinsic === "Object.create" &&
  node.this.type === "PrimitiveExpression" &&
  node.arguments.length === 1;
