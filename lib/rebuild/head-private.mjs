 ...
 * ) => key is estree.PrivateKey}
 */
const isStaticKey = (key) => key !== null;
 ...
 * ) => estree.Expression}
 */
const makeDynamicPrivateArrow = (operation, keys, _config) => {
  const key_arg = "key";
  const obj_arg = "obj";
  const val_arg = "val";
  const rec_arg = "rec";
 ...
 * ][]}
 */
const makePrivateDeclarator = (operation, head, config) => {
  ...
  if (keys.length === 0) {
    ...
    return [
      [
        ...
        every(keys, isStaticKey)
          ? makeStaticPrivateArrow(operation, keys, config)
          : makeDynamicPrivateArrow(
              operation,
              filterNarrow(keys, isStaticKey),
              config,
            ),
      ],
    ];
...
export const listPrivateDeclarator = (head, config) =>
  flatMap(OPERATION, (operation) =>
    makePrivateDeclarator(operation, head, config),
  );
