...
import { isDynamicPrivateHeader, isStaticPrivateHeader } from "../header.mjs";
import {
  compileGet,
  filterNarrow,
  map,
 ...
 * ) => estree.Expression}
 */
const makeStaticPrivateArrow = (operation, keys, context) => {
  const obj_arg = "obj";
  const key_arg = "key";
  const val_arg = "val";
  return {
 ...
 * ) => estree.Expression}
 */
const makeDynamicPrivateArrow = (operation, keys) => {
  const key_arg = "key";
  const obj_arg = "obj";
  const val_arg = "val";
  const rec_arg = "rec";
 ...
 */
export const listPrivateDeclarator = (head, context) => {
  const keys = removeDuplicate(
    map(filterNarrow(head, isStaticPrivateHeader), getKey),
  );
  const dynamic = some(head, isDynamicPrivateHeader);
  return map(PRIVATE, (operation) => ({
    ...
    init: dynamic
      ? makeDynamicPrivateArrow(operation, keys)
      : makeStaticPrivateArrow(operation, keys, context),
  }));
};
