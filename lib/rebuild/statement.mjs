...
export const rebuildStatement = (node, config) => {
  switch (node.type) {
    ...
    case "BlockStatement": {
      return rebuildControlBlock(node.do, config);
    }
    case "IfStatement": {
      return {
        type: "IfStatement",
        test: rebuildExpression(node.if, config),
    ...
    case "TryStatement": {
      return {
        ...
        handler: {
          ...
          param: {
            type: "Identifier",
            name: mangleParameter("catch.error"),
          },
    ...
    case "WhileStatement": {
      return {
        type: "WhileStatement",
        test: rebuildExpression(node.while, config),
        body: rebuildControlBlock(node.do, config),
      };
    }
    default: {
