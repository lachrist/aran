 ...
 */
const rebuildModuleHeader = (header, context) => {
  switch (header.type) {
    case "import": {
      return [
        {
          ...
          declarations: [
            {
              ...
              id: {
                type: "Identifier",
                name: mangleExport(header.export, context.escape),
              },
            },
          ],
        },
    ...
    default: {
      throw new AranTypeError("invalid module header", header);
    }
  }
};
 ...
 * ) => (estree.ModuleDeclaration | estree.Statement)[]}
 */
export const rebuildModuleHead = (head, context) =>
  flatMap(removeDuplicateModuleHeader(head), (node) =>
    rebuildModuleHeader(node, context),
  );
