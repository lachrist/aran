 ...
 * ) => string}
 */
export const mangleEval = (escape) => `eval_${escape}`;
 ...
 */
export const checkClash = (variable, { intrinsic_variable, escape_prefix }) => {
  if (variable === intrinsic_variable) {
    return {
      type: "intrinsic",
      variable,
    };
  ...
  for (const prefix of [IMPORT, EXPORT, ARGUMENT]) {
    if (apply(startsWith, variable, [`${prefix}_${escape_prefix}_`])) {
      return {
        type: "escape",
        variable,
        escape: escape_prefix,
        prefix,
      };
    }
  }
