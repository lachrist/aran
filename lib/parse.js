...
module.exports = (code, options) => {
  Throw.assert(options.parser !== null, null, `Missing external parser`);
  if (options.source === "script" || options.source === "global-eval") {
  ...
  // console.assert(source === "local-eval");
  const context = {
    ["strict-mode"]: false,
    ["super-call"]: false,
  ...
  if (options.context !== null) {
    ...
    Throw.assert(options.scopes !== null, null, `Missing options.scopes`);
    Throw.assert(global_Reflect_getOwnPropertyDescriptor(options.scopes, options.serial) !== void 0, null, `Missing options.scopes[options.serial]`);
    global_Object_assign(context, options.scopes[options.serial].context);
  }
  Throw.assert(!context["super-call"] || context["strict-mode"], null, `Violation of super-call => strict-mode`);
  Throw.assert(!context["super-call"] || context["super-property"], null, `Violation of super-call => super-property`);
  Throw.assert(!context["super-property"] || context["new-target"], null, `Violation of super-propety => new-target`);
  const directive = context["strict-mode"] ? `"use strict";` : `"use normal";`;
  let node = null;
  if (context["super-call"]) {
    node = parse.script(`${directive} (class extends Object { constructor () { ${code} } });`, options);
    node.body = node.body[1].expression.body.body[0].value.body.body;
  } else if (context["super-property"]) {
    node = parse.script(`${directive} ({name () { ${code} }});`, options);
    node.body = node.body[1].expression.properties[0].value.body.body;
  } else if (context["new-target"]) {
    node = parse.script(`${directive} (function () { ${code} });`, options);
    node.body = node.body[1].expression.body.body;
  } else {
    node = parse.script(`${directive} (() => { ${code} });`, options);
    node.body = node.body[1].expression.body.body;
  }
