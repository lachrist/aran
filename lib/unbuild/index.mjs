 ...
 * }}
 */
const unsequenceProgram = ({ head, tail }, root) => {
  for (const { data: syntax_error } of filterNarrow(
    head,
    isSyntaxErrorPrelude,
  )) {
    throw new AranSyntaxError(toSyntaxErrorCause(syntax_error, root));
  }
  return {
  ...
  if (source.situ.type === "global" || source.situ.type === "local") {
    ...
    return unsequenceProgram(
      prependSequence(
        ...
        unbuildProgram(source.root, ROOT_META, {
          scope: mode,
          annotation,
          unbound,
    ...
    const scope = hasUseStrictDirective(source.root.body)
      ? useStrict(unpackScope(source.situ.scope))
      : unpackScope(source.situ.scope);
    ...
    return unsequenceProgram(
      prependSequence(
        map(report, makeSyntaxErrorPrelude),
        unbuildProgram(source.root, source.situ.meta, {
          scope,
          annotation,
          unbound,
          sort: getProgramSort(source),
