 ...
 */
export const makeNameExpression = (name, origin) => {
  switch (name.type) {
    case "anonymous":
      return makePrimitiveExpression("", origin);
    case "static":
    ...
    case "dynamic":
      return name.kind === "get" || name.kind === "set"
        ? makeBinaryExpression(
            "+",
            makePrimitiveExpression(`${name.kind}`, origin),
            makeReadExpression(name.base, origin),
            origin,
          )
        : makeReadExpression(name.base, origin);
    default:
