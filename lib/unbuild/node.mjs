 ...
 * ) => N}
 */
export const tellLog = (node, data) => ({
  ...node,
  tag: {
    path: node.tag.path,
    tells: [...node.tag.tells, { type: "log", data }],
  },
});
 ...
 * ) => N}
 */
export const tellEarlyError = (node, data) => ({
  ...node,
  tag: {
    path: node.tag.path,
    tells: [...node.tag.tells, { type: "early-error", data }],
  },
});
 ...
 * ) => N}
 */
export const tellHeader = (node, data) => ({
  ...node,
  tag: {
    path: node.tag.path,
    tells: [...node.tag.tells, { type: "header", data }],
  },
});
 ...
 * ) => unbuild.Variable[]}
 */
const listDeclaration = (nodes) => {
  /** @type {import("../../type/unbuild.js").Variable[]} */
  const variables = [];
  let length = 0;
 ...
 * ) => X[]}
 */
const compileListTag = (list) => (node) => {
  /** @type {X[]} */
  const array = [];
  let length = 0;
  /** @type {aran.Node<unbuild.Atom>[]} */
...
export const listLog = compileListTag(({ tag: { tells } }) =>
  map(filterNarrow(tells, isLogTell), getData),
);

export const listHeader = compileListTag(({ tag: { tells } }) =>
  map(filterNarrow(tells, isHeaderTell), getData),
);
...
export const makeControlBlock = (labels, variables, statements, path) => ({
  type: "ControlBlock",
  labels,
  variables: [...variables, ...listDeclaration(statements)],
  statements,
  tag: { path, tells: EMPTY },
});
...
// */
// export const listControlStatement = (labels, variables1, statements, path) => {
//   const variables2 = listMetaDeclaration(statements);
//   if (labels.length === 0 && variables1.length === 0 && variables2.length === 0) {
//     return statements;
//   } else {
...
export const makeClosureBlock = (variables, statements, completion, path) => ({
  type: "ClosureBlock",
  variables: [...variables, ...listDeclaration([...statements, completion])],
  statements,
  completion,
  tag: { path, tells: EMPTY },
});
...
export const makeInitMetaEffect = (variable, right, path) => ({
  ...
  tag: {
    path,
    tells: [{ type: "declaration", data: variable }],
  },
});
