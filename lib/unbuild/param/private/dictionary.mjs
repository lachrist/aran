...
import { hasOwn, map, mapMaybe, pairup } from "../../../util/index.mjs";
 ...
 * ) => {
 *   type: "success",
 *   value: { [k in estree.PrivateKey]: PrivateDictionary<D> }
 * } | {
 *   type: "failure",
 *   error: string,
 * }}
        ...
        (many) => {
          ...
          return makeSetupExpression(
            mapSetup(
              sequenceSetup(
                map(
                  zipMeta(metas.cook, entries),
                  ([meta, [key, dictionary]]) => {
                    const setup = cookPrivateDictionary(
                      { path, meta },
                      context,
                      {
                        dictionary,
                    ...
                    return mapSetup(setup, (dictionary) =>
              ...
              (entries) => {
                const outcome = combinePrivateDictionary(
                  entries,
                  overwritePrivateDescriptor,
                );
                switch (outcome.type) {
