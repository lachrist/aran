 ...
 * ) => import("./hoist-private").Binding[]}
 */
const hoistProgram = (node, digest, mode) => {
 ...
 * ) => import("./hoist-private").Binding[]}
 */
const hoistClosure = (node, digest, mode) => {
 ...
 * ) => import("./hoist-private").Binding[]}
 */
const hoistStaticBlock = (node, digest, mode) =>
  scopeBindingArray(
 ...
 * ) => import("./hoist-private").Binding[]}
 */
const hoistRoot = (node, digest, mode) => {
  if (node.type === "Program") {
    return hoistProgram(node, digest, mode);
  } else if (
    node.type === "FunctionDeclaration" ||
    node.type === "FunctionExpression" ||
    node.type === "ArrowFunctionExpression"
  ) {
    return hoistClosure(node, digest, mode);
  } else if (node.type === "StaticBlock") {
    return hoistStaticBlock(node, digest, mode);
  } else {
    throw new AranTypeError(node);
  }
};
 ...
 * }}
 */
export const hoist = (node, digest, mode) => {
  const illegal = getIllegalRecord(mode);
  const bindings = map(hoistRoot(node, digest, mode), (binding) =>
 ...
 * ) => import("./hoist-public").Binding[]}
 */
export const listBinding = (registery, hash) =>
  hasOwn(registery, hash) ? registery[hash] : EMPTY;
