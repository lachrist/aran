 ...
 * ) => {[key in import("../../hash").Hash]?: null}}
 */
export const recordCompletion = (root, digest) =>
  reduceEntry(map(loopAll(root.body, digest, true, {}).hashes, pairupNull));
  ...
  reduce(
    ...
    (result1, index) => {
      ...
      return {
        ...
        hashes: concatXX(result1.hashes, result2.hashes),
      };
    },
...
const getPathArray = compileGet("hashes");
...
const loop = (node, digest, maybe_last, labels) => {
  if (node.type === "BlockStatement") {
    ...
    return {
      previous_maybe_last: false,
      hashes: maybe_last ? [digest(node)] : EMPTY,
    };
  } else {
