 ...
 * }}
 */
export const annotateHoisting = (node, digest, mode) => {
  const illegal = getIllegalRecord(mode);
  const bindings = map(hoistProgram(node, digest, mode), (binding) =>
    reportIllegalBinding(binding, illegal),
  );
  /** @type {import("./hoisting-public").Binding[]} */
  const unbound = [];
  /** @type {import("./hoisting-public").Hoisting} */
...
/**
 * @type {(
 *   registery: import("./hoisting-public").Hoisting,
 *   hash: import("../../hash").Hash
 * ) => import("./hoisting-public").Binding[]}
 */
export const consultHoisting = (registery, hash) =>
