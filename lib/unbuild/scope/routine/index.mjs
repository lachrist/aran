...
/**
 * @type {import(".").FunctionRoutine}
 */
const FUNCTION_ROUTINE = {
  type: "function",
  intermediaries: null,
  result: null,
};
 ...
 * ) => import(".").ConstructorRoutine}
 */
const makeConstructorRoutine = ({ derived, self, field }) => ({
  type: "constructor",
  intermediaries: null,
  result: null,
  derived,
...
export const extendConstructorRoutine = (hash, meta, options, scope) =>
  bindSequence(
    ...
    (setup) =>
      initSequence([makePrefixPrelude(setup)], {
        ...scope,
        routine: makeConstructorRoutine(options),
      }),
  );
 ...
 * ) => import(".").MethodRoutine}
 */
export const makeMethodRoutine = ({ proto }) => ({
  type: "method",
  intermediaries: null,
  result: null,
  proto,
 ...
 * >}
 */
export const extendMethodRoutine = (hash, _meta, options, scope) =>
  initSequence(
    map(listSetupThisMethodEffect(hash, scope.mode), makePrefixPrelude),
    {
      ...scope,
      routine: makeMethodRoutine(options),
    },
  );
 ...
 * >}
 */
export const extendArrowRoutine = (_hash, _meta, {}, scope) =>
  zeroSequence({
    ...scope,
    routine: {
      ...scope.routine,
