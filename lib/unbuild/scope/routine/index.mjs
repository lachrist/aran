 ...
 * @type {(
 *   routine: import(".").Routine,
 * ) => "*illegal*" | "ExpressionStatement" | "ReturnStatement"}
 */
const getOriginKind = (routine) => {
  if (routine.intermediaries === null) {
    return routine.type === "root" ? "ExpressionStatement" : "ReturnStatement";
  } else {
    const intermediary = routine.intermediaries[0];
    switch (intermediary) {
      case "arrow": {
 ...
 * >}
 */
export const extendFunctionRoutine = (hash, meta, scope, { mode }) =>
  bindSequence(
    liftSequence_X__(
      makeConditionalEffect,
      makeReadExpression("new.target", hash),
 ...
 * >}
 */
export const extendConstructorRoutine = (hash, meta, scope, options) =>
  bindSequence(
    liftSequence_X__(
      makeConditionalEffect,
      makeReadExpression("new.target", hash),
 ...
 * >}
 */
export const extendStaticBlock = (_hash, _meta, scope, _options) =>
  zeroSequence({
    ...scope,
    routine: {
      ...scope.routine,
