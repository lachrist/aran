...
import { listCacheEffect, listCacheStatement } from "../../cache.mjs";
...
export const listScopeInitializeStatement = (
  ...
  switch (scope.type) {
    ...
    case "dynamic": {
      ...
      return listCacheStatement(right, path, metas.right, (right) => [
        makeIfStatement(
...
export const listScopeWriteEffect = (
  ...
  switch (scope.type) {
    ...
    case "dynamic": {
      const metas = splitMeta(meta, ["right", "next"]);
      return listCacheEffect(right, path, metas.right, (right) => [
        makeConditionalEffect(
          makeWithExistExpression(strict, scope.with, variable, path),
          [
            makeExpressionEffect(
