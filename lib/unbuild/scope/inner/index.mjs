 ...
 *   | { type: "block"; with: unbuild.Variable | null }
 * )} FrameContext
 */
 ...
 *   },
 *   kinds: Record<estree.Variable, estree.VariableKind>,
 *   frame_context: FrameContext<T>,
 * ) => Scope<T>}
 */
export const extendScope = ({ strict, scope }, kinds, frame_context) => ({
  frame: makeFrame(strict, kinds, frame_context),
  parent: scope,
});
 ...
 * ) => unbuild.Variable[]}
 */
export const listScopeFreeVariable = ({ scope }) => {
  /** @type {unbuild.Variable[]} */
  const variables = [];
  while (scope !== null) {
    if (scope.frame.dynamic !== null) {
      push(variables, scope.frame.dynamic);
    }
    scope = scope.parent;
  }
  return variables;
