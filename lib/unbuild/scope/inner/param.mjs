...
import { forkMeta, mangleMetaVariable, nextMeta } from "../../mangle.mjs";
import {
  makeApplyExpression,
...
export const makeSetSuperExpression = (
  ...
  if (param.type === "program") {
    ...
    if (param.kind === "arrow") {
      ...
      return isNotArrowClosureParam(param)
        ? makeCacheExpression(key, path, mangleMetaVariable(meta), (key) =>
            makeCacheExpression(
              value,
              path,
              mangleMetaVariable(nextMeta(meta)),
              (value) =>
                makeLongSequenceExpression(
                  listClosureSetSuperEffect(strict, param, key, value, path),
                  value,
...
const makeIsProperObjectExpression = (result, path, meta) =>
  makeCacheExpression(result, path, mangleMetaVariable(meta), (result) =>
    makeConditionalExpression(
      makeBinaryExpression(
        "===",
      ...
      return result === null
        ...
        : makeCacheExpression(
            ...
            (result) =>
              param.super === null
                ? makeConditionalExpression(
                    makeIsProperObjectExpression(result, path, forkMeta(meta)),
                    result,
                    makeReadExpression("this", path),
                    path,
                  )
