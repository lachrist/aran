...
export const listRootSaveEffect = ({ path, meta }, frame, operation) => {
  ...
  } else if (operation.type === "module") {
    if (frame.sort.kind === "module") {
      return initSequence([makeHeaderPrelude(operation.header)], EMPTY);
    } else if (frame.sort.kind === "script" || frame.sort.kind === "eval") {
      return listEarlyErrorEffect(
        `Illegal module declaration in ${frame.sort.kind} program`,
        path,
      );
    } else {
      throw new AranTypeError(frame.sort);
