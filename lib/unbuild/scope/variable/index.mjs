...
import { appendArray, hasNarrowKey, map } from "../../../util/index.mjs";
 ...
 *   scope: S,
 *   frame: import(".").VariableFrame,
 * ) => S}
 */
export const extendVariable = (scope, frame) => ({
  ...scope,
 ...
 * @type {<S extends import(".").VariableScope>(
 *   scope: S,
 *   frames: import(".").VariableFrame[],
 * ) => S}
 */
export const extendVariableBatch = (scope, frames) => ({
  ...scope,
  variable: appendArray(frames, scope.variable),
});
  ...
  if (frames === null) {
    ...
    switch (head.type) {
      ...
      case "illegal": {
        ...
        return result === null
          ? loopStandard(duplicate, perform, hash, meta, root, tail, operation)
          : result;
      }
      case "proxy": {
 ...
 *   R,
 *   import("../../prelude").SyntaxErrorPrelude | W0 | W1 | W2 | W3 | W4 | W5 | W6,
 *   X,
 * >}
 */
export const compileStandard =
  (access, initialize, duplicate, perform) =>
