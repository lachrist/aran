...
} from "./variable/root/index.mjs";
import {
  listRegularInitEffect,
...
} from "./variable/proxy/index.mjs";

export { setupRootFrame } from "./variable/root/index.mjs";
export { setupFakeFrame } from "./variable/proxy/index.mjs";
export {
  setupRegularFrame,
  setupProgramFrame,
} from "./variable/regular/index.mjs";
...
export { setupPrivateFrame } from "./private/frame.mjs";
export {
...
/**
 * @type {(
 *   frame: import("./variable/root").RootFrame,
 * ) => import(".").Scope}
 */
 ...
 * @type {<X>(
 *   next: import(".").Scope,
 *   pair: import("../../util/pair").Pair<X, import(".").NodeFrame>,
 * ) => import("../../util/pair").Pair<X, import(".").Scope>}
 */
const extendScopePair = (next, { fst, snd }) => ({
  fst,
  snd: {
    here: snd,
    next,
...
/**
 * @type {<X>(
 *   pair: import("../../util/pair").Pair<X, import("./variable/root").RootFrame,>,
 * ) => import("../../util/pair").Pair<X, import(".").Scope>}
 */
const makeRootScopePair = ({ fst, snd }) => ({
  fst,
  snd: makeRootScope(snd),
});

/**
 * @type {import("./perform").ListScopeInitEffect<
 *   import(".").Scope,
 * >}
 */
export const listScopeInitEffect = (hash, meta, scope, operation) => {
  if (scope.next === null) {
    return liftSequenceX(
      makeRootScopePair,
      listRootInitEffect(hash, meta, scope.here, operation),
    );
  } else {
    if (scope.here.type === "regular") {
      const maybe = listRegularInitEffect(hash, meta, scope.here, operation);
      if (maybe === null) {
        return listScopeInitEffect(hash, meta, scope.next, {
          ...operation,
          distant: true,
        });
      } else {
        return liftSequence_X(extendScopePair, scope.next, maybe);
      }
    } else {
      throw new AranExecError("hit non-regular frame during init operation", {
        hash,
        meta,
        scope,
        operation,
...
/**
 * @type {import("./perform").ListScopeSaveEffect<
 *   import(".").Scope,
 * >}
 */
...
/**
 * @type {import("./perform").makeScopeLoadExpression<
 *   import(".").Scope,
 * >}
 */
