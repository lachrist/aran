 ...
 * ) => boolean}
 */
export const willScopeUpdateCompletion = (scope, path) => {
  let current = scope;
  while (current.parent !== null) {
    if (isRoutineFrame(current.frame)) {
      return willUpdateCompletion(current.frame, path);
    } else {
      current = current.parent;
    }
  }
  throw new AranError("missing routine frame", { path, scope });
};
