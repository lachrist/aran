 ...
 * >}
 */
export const setupGlobalObjectStaticFrame = ({ path }, record, { mode }) =>
  mapSequence(
    flatSequence(
      map(listEntry(record), ([variable, kind]) =>
        mapSequence(
 ...
 * >}
 */
export const setupGlobalRecordStaticFrame = ({ path }, record, { mode }) =>
  mapSequence(
    flatSequence(
      map(listEntry(record), ([variable, kind]) =>
        mapSequence(
          setupGlobalRecordBinding({ path }, kind, { mode, variable }),
          (binding) => pairup(variable, binding),
        ),
      ),
    ),
