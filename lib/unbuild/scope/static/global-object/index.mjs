 ...
 */
const makeDeclareExpression = ({ path }, _context, { variable }) =>
  makeApplyExpression(
 ...
 * ) => aran.Effect<unbuild.Atom>[]}
 */
const listDeclareEffect = ({ path }, context, { variable }) => {
  switch (context.mode) {
    case "strict": {
      return [
        makeConditionalEffect(
 ...
 * >}
 */
export const bindGlobalObject = ({ path }, context, { kind, variable }) =>
  initSequence(
    [
      ...map(listClashEffect({ path }, context, { variable }), (node) =>
        makeHeadPrelude(makeEffectStatement(node, path)),
      ),
      ...map(listDeclareEffect({ path }, context, { variable }), (node) =>
...
export const makeGlobalObjectLoadExpression = (
  ...
  makeLoadExpression({ path }, context, {
    operation,
    record: cacheIntrinsic("aran.global"),
    variable,
  });
