    ...
    case "WriteEffect":
      return includes(/** @type {string []} */ (variables), node.variable)
        ? makeExpressionEffect(
            makeSetExpression(
              true,
              makeIntrinsicExpression("aran.cache", origin),
              makePrimitiveExpression(escape(root, node.variable), origin),
              escapeExpression(node.right, root, variables),
              origin,
            ),
            origin,
  ...
  switch (node.type) {
    ...
    case "ReadExpression":
      return includes(/** @type {string[]} */ (variables), node.variable)
        ...
        : makeReadExpression(node.variable, origin);
