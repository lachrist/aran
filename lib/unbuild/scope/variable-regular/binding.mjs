...
const PASS_SLOPPY_FUNCTION = { type: "pass" };

/** @type {import(".").SloppyFunction} */
const SKIP_SLOPPY_FUNCTION = { type: "skip" };
...
const initSloppyFunction = ({ path, meta }, binding) => {
  if (binding.sloppy_function === null) {
    return zeroSequence(SKIP_SLOPPY_FUNCTION);
  } else {
    if (
      binding.sloppy_function.away === 0 &&
      binding.sloppy_function.near === 0
    ) {
      return zeroSequence(PASS_SLOPPY_FUNCTION);
    }
    if (binding.sloppy_function.near > binding.sloppy_function.away) {
    ...
    if (binding.sloppy_function.away > binding.sloppy_function.near) {
