 ...
 */
const unbuildQuasi = (node, { cooked }) => {
            ...
            [
              makeArrayExpression(
                mapIndex(node.quasi.quasis.length, (index) =>
                  unbuildQuasi(node.quasi.quasis[index], { cooked: true }),
                ),
                hash,
              ),
              ...
              makeDataDescriptorExpression(
                {
                  value: makeApplyExpression(
                    ...
                    [
                      makeArrayExpression(
                        mapIndex(node.quasi.quasis.length, (index) =>
                          unbuildQuasi(node.quasi.quasis[index], {
                            cooked: false,
                          }),
                        ),
                        hash,
...
export const getPreludeTemplateBinding = (prelude) =>
  prelude.type === "template" ? [prelude.data.variable, "aran.deadzone"] : null;
