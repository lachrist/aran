  ...
  switch (node.key.type) {
    ...
    default:
      throw new DynamicSyntaxAranError(
        "invalid non-computed property key",
        node,
      );
  }
};
...
const extractConstructor = (node) => {
  ...
  switch (constructors.length) {
    ...
    default:
      throw new DynamicSyntaxAranError("multiple class constructor");
  }
};
...
const getMethodKey = (node) => {
  switch (node.key.type) {
    ...
    default:
      throw new DynamicSyntaxAranError("invalid non-computed method key", node);
  }
};
 ...
 * ) => aran.Expression<unbuild.Atom<S>>}
 */
export const unbuildClass = (node, context, { name }) => {
  const { serialize, digest } = context;
  const serial = serialize(node);
  const hash = digest(node);
  ...
  return node.superClass == null
    ...
    : makeSequenceExpression(
        ...
        unbuildClassBody(node.body, context, { name, super: super_ }),
        serial,
      );
};
