...
import { unbuildFunctionDeclaration } from "./function.mjs";
...
export const unbuildStatement = (node, context, labels) => {
  ...
  switch (node.type) {
    ...
    case "WithStatement": {
      const frame = {
        var: mangleMetaVariable(hash, BASENAME, "frame"),
    ...
    case "FunctionDeclaration":
      if (node.id == null) {
        ...
        return unbuildFunctionDeclaration(node, context, {});
      }
    ...
    case "SwitchStatement":
      return wrapBlock(
        ...
        (context) => {
          const discriminant = {
            var: mangleMetaVariable(hash, BASENAME, "discriminant"),
            val: unbuildExpression(node.discriminant, context, null),
          };
          const matched = {
            var: mangleMetaVariable(hash, BASENAME, "matched"),
            val: makePrimitiveExpression(false, serial),
          };
          return [
            makeEffectStatement(
              makeWriteEffect(discriminant.var, discriminant.val, serial, true),
              serial,
            ),
            makeEffectStatement(
              makeWriteEffect(matched.var, matched.val, serial, true),
              serial,
            ),
            ...flatMap(node.cases, (child) =>
              unbuildCase(child, context, {
                discriminant: discriminant.var,
                matched: matched.var,
              }),
            ),
          ];
