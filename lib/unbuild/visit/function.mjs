...
import { isClosureStrict, listPatternVariable } from "../../estree/index.mjs";
import {
  StaticError,
  enumerate,
  filterOut,
 ...
 * ) => aran.Expression<unbuild.Atom<S>>}
 */
export const unbuildFunction = (node, context, { kind, name: parent_name }) => {
  const name =
  ...
  const presence = {
    arguments:
      ...
      hasFreeVariable(node.body, /** @type {estree.Variable} */ ("arguments"))
    ...
    callee:
      ...
      hasFreeVariable(node.body, node.id.name)
        ...
        makeApplyExpression(
          ...
          [
            ...
            makeDataDescriptorExpression(
              ...
              null,
              null,
              makePrimitiveExpression(name === null ? "" : name, serial),
              serial,
            ),
          ],
          serial,
