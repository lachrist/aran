 ...
 */
export const toStaticDuplicateEarlyError = ({ variable, origin }) => ({
  type: "duplicate",
  frame: "static",
  variable,
  origin,
 ...
 */
const isStaticEarlyError = (error) =>
  error.type === "regular" ||
 ...
 */
const isDynamicEarlyError = (error) =>
 ...
 */
const reportStaticEarlyError = (error, { early_syntax_error, ...options }) => {
 ...
 * ) => import("./atom").Statement[]}
 */
const reportDynamicEarlyError = (error, options) => {
  const message = formatMessage(error);
