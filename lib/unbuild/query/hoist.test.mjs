...
assertEqual(hoistClosure("sloppy", parseScript("function f () {}").body), {
  f: "function",
});

assertEqual(hoistClosure("strict", parseModule("export var foo;").body), {
  foo: "var",
});

assertEqual(
  hoistClosure("strict", parseModule("export { foo } from 'source';").body),
  {},
);

assertEqual(
  hoistClosure("strict", parseModule("export default function () {};").body),
  {},
);

assertEqual(
  hoistClosure("strict", parseModule("export default class {};").body),
  {},
);
...
const testClosure = (code, variables) => {
  assertEqual(
    hoistClosure("strict", parseScript(code).body),
...
assertEqual(hoistBlock("strict", parseScript("class c {}").body), {
  c: "class",
});

assertEqual(hoistBlock("strict", parseScript("const c = 123;").body), {
  c: "const",
});

assertEqual(
  hoistBlock("strict", parseModule("export { foo } from 'source';").body),
  {},
);

assertEqual(hoistBlock("strict", parseModule("export let foo;").body), {
  foo: "let",
});

assertEqual(
  hoistBlock("strict", parseModule("export default class {};").body),
  {},
);

assertEqual(hoistBlock("strict", parseScript("123;").body), {});

assertEqual(hoistBlock("strict", parseScript("var foo;").body), {});

assertEqual(hoistBlock("strict", parseScript("let foo;").body), { foo: "let" });
