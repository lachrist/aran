...
/**
 * @type {(
 *   kind: Exclude<import("./hoist2").Kind, "import">,
 * ) => import("./hoist2").Write}
 */
 ...
 *   variable: import("../../estree").Variable,
 * ) => import("./hoist2").ExternalBinding}
 */
const makeExternalBinding = (root, variable) => {
  ...
  throw new AranError("missing external binding", { variable, root });
};
 ...
 *   kind: Exclude<import("./hoist2").Kind, "import">,
 *   variable: import("../../estree").Variable,
 * ) => import("./hoist2").InternalBinding}
 */
const makeInternalBinding = (kind, variable) => ({
  type: "internal",
  variable,
 ...
 */
const makeBinding = ({ kind, variable }, root) => {
  if (kind === "import") {
    return makeExternalBinding(root, variable);
  } else {
    return makeInternalBinding(kind, variable);
  }
};
 ...
 */
export const hoist = (root, path, mode, kind) => {
  const hoisting = hoistProgram(root, path, mode, kind);
  ...
  for (const hoist of hoisting) {
    if (hoist.status.type === "unbound") {
      if (!hasVariable(unbound, hoist.variable)) {
        unbound[unbound.length] = makeBinding(hoist, root);
      }
      ...
      if (hasOwn(bound, hoist.status.path)) {
        ...
        if (!hasVariable(bindings, hoist.variable)) {
          bindings[bindings.length] = makeBinding(hoist, root);
        }
      } else {
        bound[hoist.status.path] = [makeBinding(hoist, root)];
      }
