...
const getKindClash = (kind, other_kind) => {
  if (kind === "function-sloppy-closure") {
    if (
      other_kind === "param" ||
      ...
      other_kind === "function-sloppy-block" ||
      other_kind === "function-sloppy-closure" ||
      other_kind === "error"
    ) {
      return "ignore";
    } else {
  ...
  } else if (kind === "function-sloppy-block") {
    if (
      other_kind === "import" ||
      other_kind === "let" ||
      other_kind === "const" ||
    ...
    if (other_kind === "function-sloppy-closure") {
      return "ignore";
    } else if (
      other_kind === "param" ||
      other_kind === "param-legacy" ||
    ...
    if (other_kind === "var" || other_kind === "function-sloppy-closure") {
      return "ignore";
    } else if (
      other_kind === "param" ||
      other_kind === "param-legacy" ||
...
const MODULE_SCOPING = {
  ...
  "function-sloppy-block": true,
  "function-sloppy-closure": true,
  "class": true,
};
...
const SCRIPT_SCOPING = {
  ...
  "function-sloppy-block": false,
  "function-sloppy-closure": false,
  "class": false,
};
...
const isFreeBlockSloppyFunctionBinding = (binding) =>
  binding.type === "free" && binding.kind === "function-sloppy-block";
 ...
 * @type {(
 *   kind: import("./hoist-private").Kind,
 * ) => import("./hoist-public").SloppyFunctionProperty}
 */
const getSloppyFunctionProperty = (kind) => {
  if (kind === "function-sloppy-block") {
    return "nearby";
  } else if (kind === "function-sloppy-closure") {
    return "distant";
  } else if (
    kind === "var" ||
    kind === "let" ||
...
const toPublicBinding = (binding) => ({
  ...
  sloppy_function: getSloppyFunctionProperty(binding.kind),
});
...
/**
 * @type {(
 *   sloppy_function_property_1:
 *     import("./hoist-public").SloppyFunctionProperty,
 *   sloppy_function_property_2:
 *     import("./hoist-public").SloppyFunctionProperty,
 * ) => import("./hoist-public").SloppyFunctionProperty}
 */
const mergeSloppyFunctionProperty = (
  sloppy_function_property_1,
  sloppy_function_property_2,
) => {
  if (sloppy_function_property_1 === "nope") {
    return sloppy_function_property_2;
  } else if (sloppy_function_property_2 === "nope") {
    return sloppy_function_property_1;
  } else {
    if (sloppy_function_property_1 === sloppy_function_property_2) {
      return sloppy_function_property_1;
    } else {
      return "both";
    }
...
const mergeBinding = (binding1, binding2) => {
  if (
    ...
    return {
      ...binding1,
      sloppy_function: mergeSloppyFunctionProperty(
