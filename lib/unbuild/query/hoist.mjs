  ...
  if (node.type === "BlockStatement") {
    ...
    if (node.id == null) {
      ...
      switch (mode) {
        ...
        case "sloppy": {
          return [makeVarHoist(/** @type {estree.Variable} */ (node.id.name))];
        }
        default: {
...
export const hoistBlock = (mode, node) => {
  if (node.type === "VariableDeclaration") {
    ...
    if (node.id == null) {
      ...
      switch (mode) {
        case "strict": {
          return [makeLetHoist(/** @type {estree.Variable} */ (node.id.name))];
        }
        case "sloppy": {
          return [];
        }
    ...
    return node.id == null
      ? []
      : [makeLetHoist(/** @type {estree.Variable} */ (node.id.name))];
  } else if (node.type === "SwitchCase") {
