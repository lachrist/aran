...
const hoistClosure = (node, path, mode) => {
  ...
  const kind =
    ...
    everyNarrow(node.params, isIdentifier)
      ? "param-legacy"
      : "param";
...
/**
 * @type {(
 *   hoist: import("./hoist-private").FlagBinding,
 * ) => import("./hoist-public").EarlyError}
 */
const toEarlyError = ({ variable, origin, bind }) => ({
  cause: bind,
  variable,
  origin,
});
...
const hasDualSloppyFunction = (binding, bindings) => {
  ...
  throw new AranError("missing dual sloppy function binding", {
    binding,
    bindings,
  });
};
 ...
 *   mode: import("./hoist-private").Mode,
 * ) => {
 *   report: import("./hoist-public").EarlyError[],
...
export const hoist = (node, path, mode) => {
  ...
  /** @type {import("./hoist-public").EarlyError[]} */
  const report = [];
  for (const binding of bindings) {
    if (binding.type === "flag") {
      report[report.length] = toEarlyError(binding);
