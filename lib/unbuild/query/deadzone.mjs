...
/** @type {(unknown: unknown) => unknown is import("../../estree").Node} */
...
const annotate = (mode, node, scope) => {
  ...
  if (node.type === "BlockStatement") {
    return {
      ...
      body: reduce(node.body, (x, y) => accumulateDeclaration(mode, x, y), {
        nodes: /** @type {import("../../estree").Statement[]} */ ([]),
        scope: {
          ...scope,
          ...reduceEntry(
            map(
  ...
  if (node.type === "SwitchStatement") {
    return {
      ...
      cases: reduce(node.cases, (x, y) => accumulateDeclaration(mode, x, y), {
        nodes: /** @type {import("../../estree").SwitchCase[]} */ ([]),
        scope: {
          ...scope,
          ...reduceEntry(
            map(
  ...
  if (
    ...
    const { scope: new_scope, nodes } = reduce(
      ...
      {
        nodes: /** @type {import("../../estree").Pattern[]} */ ([]),
...
const annotateDeclaration = (mode, node, scope) => {
  ...
  if (node.type === "VariableDeclaration") {
    const { nodes, scope: new_scope } = reduce(
      ...
      {
        scope,
        nodes: /** @type {import("../../estree").VariableDeclarator[]} */ ([]),
      },
    );
    return {
      node: {
  ...
  if (node.type === "SwitchCase") {
    const { nodes, scope: new_scope } = reduce(
      node.consequent,
      (x, y) => accumulateDeclaration(mode, x, y),
      { nodes: /** @type {import("../../estree").Statement[]} */ ([]), scope },
    );
    // switch (x) {
    //   case 123:
    //     y; // not initialized
...
/** @type {(node: import("../../estree").Node, key: string) => boolean} */
...
/** @type {(node: import("../../estree").Node, key: string, variable: string) => boolean} */
...
/** @type {(node: import("../../estree").Identifier) => boolean | null} */
