...
/**
 * @type {(
 *   scope: import("../../util/tree").ArrayTree<
...
const lookup = (scope, variable) => {
  let closure = false;
  for (const binding of generateArrayTree(scope)) {
    if (binding === "closure") {
      closure = true;
    } else {
      if (binding.variable === variable) {
        if (closure) {
          return binding.baseline === "dead" ? "schrodinger" : binding.baseline;
        } else {
...
export const annotateDeadzone = (node, hoisting) => {
  /** @type {import("./deadzone").Deadzone} */
  // @ts-ignore
  const deadzone = { __proto__: null };
  for (const { hash, status } of generateArrayTree(
    zoneProgram(node, EMPTY, hoisting),
  )) {
    deadzone[hash] = status;
  }
  return deadzone;
};
/* eslint-enable local/no-impure */
