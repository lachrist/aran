...
const getKindClash = (kind, other_kind) => {
  ...
  if (kind === "class-self" || other_kind === "class-self") {
    // class-self always has its own scope
    return "ignore";
  }
  ///////////////////
  // function-self //
  ///////////////////
  if (kind === "function-self-sloppy" || kind === "function-self-strict") {
    if (other_kind === "param" || other_kind === "param-simple") {
      // optimization: function-self cannot be reached
      return "remove";
    } else {
      return "ignore";
    }
  ...
  switch (node.type) {
    ...
    case "IfStatement": {
      return [
        ...
        mapTree(
          node.alternate == null ? null : hoistStatement(node.alternate, mode),
          (binding) =>
