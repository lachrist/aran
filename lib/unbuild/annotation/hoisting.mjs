...
const hoistProgram = (node, kind, mode) => {
  ...
  return scopeBindingArray(
    ...
    flatMapIndex(node.body.length, (index) =>
      hoistProgramElement(node.body[index], next_mode),
    ),
    node._hash,
  );
};
  ...
  if (node.body.type === "BlockStatement") {
    ...
    return scopeBindingArray(
      ...
      scopeBindingArray(
        ...
        concatXX(
          flatMap(node.params, (child) => hoistPattern(child, kind, mode)),
          flatMap(node.body.body, (child) => hoistStatement(child, mode)),
        ),
        node.body._hash,
      ),
      node._hash,
    ...
    scopeBindingArray(
      ...
      concatXX(
        ...
        flatMapIndex(node.body.body.length, (index) =>
          hoistStatement(node.body.body[index], mode),
        ),
      ),
      node.body._hash,
    ),
...
const hoistSwitchCase = (node, mode) =>
  flatMapIndex(node.consequent.length, (index) =>
    hoistStatement(node.consequent[index], mode),
  );
  ...
  switch (node.type) {
    ...
    case "CallExpression": {
      return concatXX(
        ...
        flatMap(node.arguments, (argument) => hoistExpression(argument, mode)),
      );
    }
    case "NewExpression": {
      return concatXX(
    ...
    case "SequenceExpression": {
      return flatMap(node.expressions, (expression) =>
        hoistExpression(expression, mode),
      );
    }
...
const hoistPattern = (node, kind, mode) => {
  switch (node.type) {
    ...
    case "ArrayPattern": {
      return flatMap(node.elements, (element) =>
        element == null ? EMPTY : hoistPattern(element, kind, mode),
      );
    }
    case "ObjectPattern": {
      return flatMap(node.properties, (property) =>
        hoistPattern(property, kind, mode),
      );
    }
    case "RestElement": {
...
const hoistClassBody = (node, mode) =>
  flatMap(node.body, (child) => hoistClassEntry(child, mode));
  ...
  switch (node.type) {
    ...
    case "StaticBlock": {
      return scopeBindingArray(
        ...
        flatMapIndex(node.body.length, (index) =>
          hoistStatement(node.body[index], mode),
        ),
        node._hash,
      );
    }
  ...
  switch (node.type) {
    case "VariableDeclaration": {
      return flatMap(node.declarations, (child) =>
        hoistPattern(child, node.kind, mode),
      );
    }
    ...
    case "IfStatement": {
      return concatXXX(
        hoistExpression(node.test, mode),
        map(hoistStatement(node.consequent, mode), (binding) =>
          binding.kind === "function-sloppy-near" && isFreeBinding(binding)
            ? lockupBinding(binding, node.consequent._hash)
            : binding,
        ),
        map(
          ...
          (binding) =>
            ...
            node.alternate != null
              ? lockupBinding(binding, node.alternate._hash)
              : binding,
        ),
      );
    }
      ...
      return scopeBindingArray(
        ...
        concatXX(
          ...
          flatMapIndex(node.cases.length, (index) =>
            hoistSwitchCase(node.cases[index], mode),
          ),
        ),
        node._hash,
      );
...
export const annotateHoisting = (node, kind, mode) => {
  ...
  const bindings = map(hoistProgram(node, kind, mode), (binding) =>
