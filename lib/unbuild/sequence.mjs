...
/**
 * @type {(
 *   sequence: import("./sequence").ConditionSequence<
 *     aran.Expression<unbuild.Atom>
 *   >,
 *   path: unbuild.Path,
 * ) => aran.Expression<unbuild.Atom>}
...
/**
 * @type {(
 *   sequence: import("./sequence").PreludeSequence<{
...
export const sequenceClosureBlock = (
  ...
  reduce(
    ...
    reduce(
      ...
      tellLog,
      makeClosureBlock(
        map(filterNarrow(head, isDeclarationPrelude), getData),
        [
...
export const sequenceControlStatement = (
  ...
  const variables = map(filterNarrow(head, isDeclarationPrelude), getData);
  const statements = [
    ...map(filterNarrow(head, isEffectPrelude), ({ data }) =>
      makeEffectStatement(data, path),
    ),
