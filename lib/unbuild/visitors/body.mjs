 ...
 * ) => boolean}
 */
const canBecomeSloppyFunction = (binding) =>
  binding.duplicable &&
  binding.initial === "undefined" &&
  binding.write === "perform";
 ...
 */
const getSloppyFunctionVariable = (binding) =>
  canBecomeSloppyFunction(binding) ? binding.variable : null;
...
export const extractEvalFrame = (mode, bindings) => {
  switch (mode) {
    ...
    case "sloppy": {
      return {
        selection: filterMap(bindings, getSloppyFunctionVariable),
        remainder: filterOut(bindings, canBecomeSloppyFunction),
      };
    }
    default: {
...
export const unbuildClosureBody = (node, meta, scope, { simple }) => {
  ...
  const had_direct_eval_call = !simple && some(node.body, hasDirectEvalCall);
  const { selection, remainder } = had_direct_eval_call
    ? extractEvalFrame(scope.mode, hoist(hash, scope.annotation))
    : { selection: null, remainder: hoist(hash, scope.annotation) };
