 ...
 * ) => N[]}
 */
const unbuildItem = wrapOriginArray(
  ({ node, path }, context, { iterator, wrapNode, listFinalNode }) => {
    switch (node.type) {
      case "RestElement": {
 ...
 * ) => N[]}
 */
const unbuildProperty = wrapOriginArray(
  ({ node, path }, context, { right, wrapNode, listFinalNode }) => {
 ...
 * ) => N[]}
 */
const unbuildRest = wrapOriginArray(
  ({ node, path }, context, { right, keys, wrapNode, listFinalNode }) => {
    const rest = mangleMetaVariable(
      BASENAME,
 ...
 * ) => N[]}
 */
const unbuildPattern = wrapOriginArray(
  ({ node, path }, context, { right, wrapNode, listFinalNode }) => {
    switch (node.type) {
      case "Identifier":
 ...
 * ) => aran.Effect<unbuild.Atom>[]}
 */
export const unbuildPatternEffect = wrapOriginArray(
 ...
 */
export const unbuildPatternStatement = wrapOriginArray(
  ({ node, path }, context, { right }) =>
    unbuildPattern({ node, path }, context, {
      right,
