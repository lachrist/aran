 ...
 * >}
 */
const unbuildItem = (node, meta, context, kind, iterator, next, done) => {
 ...
 * >}
 */
const unbuildProperty = (node, meta, context, kind, object) => {
  const { _hash: hash } = node;
...
const unbuildRestProperty = (node, meta, context, kind, object, keys) => {
  const { _hash: hash } = node;
  return incorporateEffect(
    bindSequence(
      bindSequence(
 ...
 */
// https://262.ecma-international.org/14.0#sec-destructuring-binding-patterns-runtime-semantics-restbindinginitialization
const unbuildRestObject = (node, meta, context, kind, object, keys) => {
  const { _hash: hash } = node;
  return unbuildPattern(
    node.argument,
    meta,
 ...
 * >}
 */
export const unbuildPattern = (node, meta, context, kind, right) => {
  const { _hash: hash } = node;
          ...
          bindSequence(
            ...
            (key) =>
              listSetMemberEffect(
                ...
                forkMeta((meta = nextMeta(meta))),
                context.scope,
                { mode: context.mode, object, key, value: right },
              ),
          ),
      );
    }
