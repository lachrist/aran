      ...
      return unbuildPattern(drill({ node, path }, "argument"), context, {
        right: {
          var: mangleMetaVariable(
            path,
            makeFragment(LOCATION, /** @type {__unique} */ ("rest_array")),
          ),
    ...
    default: {
      return unbuildPattern({ node, path }, context, {
        right: {
          var: mangleMetaVariable(
            path,
            makeFragment(LOCATION, /** @type {__unique} */ ("item")),
          ),
...
const unbuildProperty = (
  ...
  unbuildPattern(drill({ node, path }, "value"), context, {
    right: {
      var: mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("property")),
      ),
      val: makeGetExpression(
...
const unbuildRest = (
  ...
  const rest = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("rest_object")),
  );
  return [
    wrapNode(
      makeWriteEffect(
      ...
      return listCacheSaveNode(right, path, wrapNode, (right) =>
        unbuildPattern(drill({ node, path }, "left"), context, {
          right: {
            var: mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("assignment")),
            ),
            val: makeConditionalExpression(
              makeBinaryExpression(
                "===",
                makeCacheLoadExpression(right, path),
                makePrimitiveExpression({ undefined: null }, path),
                path,
              ),
