...
const unbuildItem = (
  { node, path, meta },
  context,
  { iterable, wrapEffect, wrapStatement, listFinalNode },
) => {
  switch (node.type) {
    case "RestElement": {
      ...
      return unbuildPattern(sites.argument, context, {
        right: makeApplyExpression(
          ...
          [
            // Most iterable are also iterator but not always
            makeObjectExpression(
    ...
    default: {
      ...
      return unbuildPattern({ node, path, meta: metas.drill }, context, {
...
const unbuildPattern = (
  ...
  switch (node.type) {
    ...
    case "ArrayPattern": {
      const metas = splitMeta(meta, ["drill", "iterator", "iterable"]);
 ...
 */
const wrapArrow = (context, makeBody, path) => [
  makeExpressionEffect(
    makeApplyExpression(
      makeArrowExpression(
        false,
