 ...
 * ) => N[]}
 */
const compilePrepend = (wrapEffect) => (head, tail, path) => [
  wrapEffect(head, path),
  ...tail,
];
  ...
  switch (node.type) {
    case "RestElement": {
      ...
      return initCache(
        ...
        compilePrepend(wrapEffect),
        (rest) => [
          ...wrapStatement({ path, meta: metas.wrap }, context, {
            makeBody: (context) => [
              makeWhileStatement(
                makeUnaryExpression(
                  "!",
 ...
 */
const wrapStatementIdentity = (_site, context, { makeBody }) =>
  makeBody(context);

/**
 * @type {WrapStatement<aran.Effect<unbuild.Atom>>}
 */
const wrapStatementArrow = ({ path, meta }, context, { makeBody }) => [
  makeExpressionEffect(
    makeApplyExpression(
      makeArrowExpression(
        false,
        false,
        makeScopeClosureBlock({ path, meta }, context, {
