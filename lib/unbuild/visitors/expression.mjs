...
import { makeArrayCacheExpression, makeCacheExpression } from "../cache.mjs";
...
export const unbuildExpression = (
  ...
  switch (node.type) {
    ...
    case "LogicalExpression": {
      const metas = splitMeta(meta, ["left_cache", "left", "right"]);
      return makeCacheExpression(
        unbuildExpression(drill({ node, path }, "left"), context, {
          meta: metas.left,
          name: ANONYMOUS,
    ...
    case "CallExpression": {
      if (isDirectEvalCallExpression(node)) {
        const metas = splitMeta(meta, ["arguments", "cache", "eval"]);
        return makeArrayCacheExpression(
          map(
            zipMeta(
              metas.arguments,
    ...
    case "MemberExpression": {
      if (node.optional) {
        if (isNotSuperMemberExpression(node)) {
          const metas = splitMeta(meta, ["object", "object_cache", "key"]);
          return makeCacheExpression(
        ...
        return makeSequenceExpression(
          ...
          makeLongSequenceExpression(
            flatMap(
              ...
              ([meta, pair]) => unbuildProperty(pair, context, { meta, self }),
            ),
            makeReadExpression(self, path),
            path,
          ),
