...
import { unbuildUpdateLeftExpression } from "./update.mjs";
...
export const unbuildExpression = ({ node, path, meta }, context, {}) => {
  switch (node.type) {
    ...
    case "TaggedTemplateExpression": {
      const metas = splitMeta(meta, ["drill", "call"]);
      const sites = mapObject(
    ...
    case "AssignmentExpression": {
      ...
      if (node.operator === "=") {
        ...
        return unbuildUpdateLeftExpression(sites.left, context, {
          kontinue: (context, { name, old_value, kontinue }) => {
            const proceed = () =>
              makeInitCacheExpression(
                "constant",
                name === null
                ...
                (new_value) =>
                  makeLongSequenceExpression(
                    kontinue(context, {
                      new_value: makeReadCacheExpression(new_value, path),
                    }),
    ...
    case "UpdateExpression": {
      ...
      return unbuildUpdateLeftExpression(sites.argument, context, {
        kontinue: (context, { old_value: raw_old_value, kontinue }) =>
          makeInitCacheExpression(
            "constant",
            raw_old_value,
                  ...
                  return node.prefix
                    ? makeInitCacheExpression(
                        ...
                        (new_value) =>
                          makeLongSequenceExpression(
                            kontinue(context, {
                              new_value: makeReadCacheExpression(
                                new_value,
                                path,
                              ),
                            }),
    ...
    case "BinaryExpression": {
      if (
        ...
        const metas = splitMeta(meta, ["drill", "private"]);
        return makeHasPrivateExpression(
          { path, meta: metas.private },
          context,
          {
            object: unbuildExpression(
              drill({ node, path, meta: metas.drill }, ["right"]).right,
              context,
              {},
            ),
            key: /** @type {estree.PrivateKey} */ (
              /** @type {estree.PrivateIdentifier} */ (node.left).name
            ),
          },
        );
      } else {
    ...
    case "CallExpression": {
      const metas = splitMeta(meta, ["drill", "arguments", "eval", "call"]);
      const sites = mapObject(
    ...
    case "ChainExpression": {
      ...
      return unbuildChainElement(sites.expression, context, {
    ...
    case "MemberExpression": {
      if (isNotOptionalMemberExpression(node)) {
        return unbuildMember({ node, path, meta }, context, {
          object: false,
          kontinue: (_context, { member }) => member,
        });
      } else {
        return makeSyntaxErrorExpression(
