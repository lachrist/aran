...
export const unbuildExpression = ({ node, path, meta }, context, {}) => {
  switch (node.type) {
    ...
    case "ThisExpression": {
      return makeParamExpression(context, "this", path);
    }
    case "MetaProperty": {
      if (node.meta.name === "new" && node.property.name === "target") {
        return makeNewTargetExpression(context.strict, context.param, { path });
      } else if (node.meta.name === "import" && node.property.name === "meta") {
        return makeImportMetaExpression(context.strict, context.param, {
          path,
        });
      } else {
        return makeSyntaxErrorExpression(
      ...
      if (node.operator === "=") {
        ...
        if (isCallExpressionSite(sites.left)) {
          ...
          return makeInitCacheExpression(
            ...
            (right) =>
              makeLongSequenceExpression(
                unbuildPatternEffect(sites.left, context, {
    ...
    case "ImportExpression": {
      ...
      return makeApplyExpression(
        makeImportDynamicExpression(context.strict, context.param, { path }),
        makePrimitiveExpression({ undefined: null }, path),
    ...
    case "MemberExpression": {
      ...
      if (node.optional) {
        ...
        if (isNotSuperSite(sites.object)) {
          ...
          return makeParamGetSuperExpression(
            context,
            unbuildKeyExpression(sites.property, context, { computed }),
            path,
          );
