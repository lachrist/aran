 ...
 * >}
 */
const unbuildPrototype = (node, meta, context) => {
  ...
  if (node.properties.length === 0) {
    ...
    if (isProtoProperty(head)) {
      return mapSequence(
        unbuildProtoProperty(head, forkMeta((meta = nextMeta(meta))), context),
        (prototype) => ({
          prototype,
          properties: rest,
        }),
 ...
 * >}
 */
export const unbuildExpression = (node, meta, context) => {
  const { _hash: hash } = node;
  switch (node.type) {
    case "Literal": {
      if (node.regex != null) {
      ...
      return incorporateExpression(
        callSequence___X(
          ...
          liftSequence_XX(
            makeCall,
            context.mode,
            unbuildCallee(node.tag, forkMeta((meta = nextMeta(meta))), context),
            liftSequenceX(
    ...
    case "ArrowFunctionExpression": {
      return unbuildFunction(node, meta, context, ARROW_PARAM, ANONYMOUS_NAME);
    }
    ...
    case "ImportExpression": {
      return liftSequenceX_X_(
        ...
        makeScopeLoadExpression(
          ...
          forkMeta((meta = nextMeta(meta))),
          context.scope,
          { type: "read-import", mode: context.mode },
        ),
        makeIntrinsicExpression("undefined", hash),
        liftSequenceX(
          concat_,
    ...
    case "ObjectExpression": {
      return bindSequence(
        unbuildPrototype(node, forkMeta((meta = nextMeta(meta))), context),
 ...
 * >}
 */
export const unbuildNameExpression = (node, meta, context, name) => {
  switch (node.type) {
    case "ArrowFunctionExpression": {
      return unbuildFunction(node, meta, context, ARROW_PARAM, name);
    }
    case "FunctionExpression": {
      return unbuildFunction(
        node,
    ...
    default: {
      return unbuildExpression(node, meta, context);
    }
  }
};
