 ...
 */
export const unbuildExpression = wrapOrigin(
  ({ node, path }, context, { name }) => {
    switch (node.type) {
      case "Literal":
      ...
      case "AssignmentExpression": {
        ...
        if (node.operator === "=") {
          ...
          if (
            /** @type {estree.Expression} */ (node.left).type ===
            "CallExpression"
          ) {
        ...
        if (isDirectEvalCallExpression(node)) {
          const [head, body] = unzip(
            map(
              ...
              ({ node, path }) =>
                unbuildCacheExpression({ node, path }, context),
            ),
          );
          return makeLongSequenceExpression(
            body,
