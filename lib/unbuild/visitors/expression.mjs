...
const unbuildCacheExpression = ({ node, path }, context) => {
  const cache = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("cache")),
  );
  return [
    cache,
    makeWriteEffect(
  ...
  switch (node.type) {
        ...
        [
          ...flat(
            zip(
              ...
              map(drillAll(drillArray({ node, path }, "expressions")), (pair) =>
    ...
    case "LogicalExpression": {
      const left = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("left")),
      );
