 ...
 * >}
 */
const unbuildPrototype = ({ node, path, meta }, { scope, deadzone }) => {
  if (node.properties.length === 0) {
    ...
    if (isProtoPropertySite(head)) {
      return mapSequence(
        unbuildProtoProperty(head, { scope, deadzone }),
        (prototype) => ({
          prototype,
                ...
                callSequence_X(
                  ...
                  liftSequence_XXX(
                    ...
                    unbuildKey(
                      ...
                      { scope, deadzone, computed: node.left.computed },
                    ),
                    liftSequenceX__(
                      makeSequenceExpression,
                      liftSequenceX(
          ...
          if (node.argument.type === "Identifier") {
            return makeScopeLoadExpression(
              ...
              scope,
              {
                ...
                status: lookupDeadzone(deadzone, node.argument.name),
              },
            );
          } else {
          ...
          flatSequence(
            mapIndex(node.elements.length, (index) => {
              ...
              if (isNotNullishItemSite(site)) {
                return unbuildSpreadable(site, { scope, deadzone });
              } else {
                ...
                liftSequenceX(
                  ...
                  flatSequence(
                    map(properties, (property) =>
                      unbuildInitProperty(property, { scope, deadzone }),
                    ),
                  ),
                ),
              ),
                ...
                (self) =>
                  liftSequenceX__(
                    ...
                    liftSequenceX(
                      ...
                      flatSequence(
                        map(properties, (property) =>
                          unbuildProperty(property, { scope, deadzone, self }),
                        ),
                      ),
                    ),
