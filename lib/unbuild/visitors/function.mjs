...
import { toBindingEarlyError } from "../early-error.mjs";
import {
  cacheConstant,
  cacheWritable,
...
import { incorporateEffect, incorporateRoutineBlock } from "../incorporate.mjs";
 ...
 * >}
 */
const setupClosureScope = (
  { node, path, meta },
  { param, mode, self, bindings },
) =>
  liftSequenceXXX(
    concat_X_,
    setupClosureFrame(
    ...
    mode === "sloppy" && some(node.params, hasDirectEvalCall)
      ? liftSequenceX(
          concat_,
          setupEvalFrame(
            {
...
// TODO support again sloppy function declaration?
...
//  * ) => boolean}
//  */
// const isSimpleBinding = (binding) =>
//   binding.baseline === "live" && binding.write === "perform";
...
//       { mode },
//     ),
//     mode === "sloppy" && some([node.body], hasDirectEvalCall)
//       ? liftSequenceXX(
//           concat__,
//           setupRegularFrame(
//             {
...
//               meta: forkMeta((meta = nextMeta(meta))),
//             },
//             filterOut(bindings, isSimpleBinding),
//           ),
//           setupEvalFrame(
//             {
//               path,
//               meta: forkMeta((meta = nextMeta(meta))),
//             },
//             filter(bindings, isSimpleBinding),
//           ),
//         )
//       : liftSequenceX(
//           concat_,
...
export const unbuildFunction = (
  ...
  return prependSequence(
    map(map(report, toBindingEarlyError), makeEarlyErrorPrelude),
    bindSequence(
