...
const LOCATION = /** @type {__location} */ (
  "lib/unbuild/visitors/function.mjs"
);
  ...
  unbuildPatternStatement({ node, path }, context, {
    right: {
      var: mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("argument")),
      ),
      val:
...
const listHeadStatement = (
  pairs,
  context,
  { simple, path, self, callee, arguments: arguments_ },
) => [
  ...(callee === null
    ? []
    : listScopeInitializeStatement(
    ...
    case "success": {
      ...
      const arguments_ =
        ...
        isPresent(
          ...
          ? mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("arguments")),
            )
          : null;
