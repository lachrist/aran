...
const hasReturn = (node) => {
  switch (node.type) {
    ...
    case "TryStatement": {
      return (
        hasReturn(node.block) ||
        (node.handler == null ? false : hasReturn(node.handler.body)) ||
        (node.finalizer == null ? false : hasReturn(node.finalizer))
      );
    }
    case "BlockStatement": {
      return some(node.body, hasReturn);
    }
    case "StaticBlock": {
      return some(node.body, hasReturn);
    }
    case "IfStatement": {
      return (
        hasReturn(node.consequent) ||
        (node.alternate == null ? false : hasReturn(node.alternate))
      );
    }
    case "SwitchStatement": {
      return some(node.cases, (node) => some(node.consequent, hasReturn));
    }
    case "WhileStatement": {
...
const hasTryReturn = (node) => {
  switch (node.type) {
    ...
    case "BlockStatement": {
      return some(node.body, hasTryReturn);
    }
    case "StaticBlock": {
      return some(node.body, hasTryReturn);
    }
    case "IfStatement": {
      return (
        hasTryReturn(node.consequent) ||
        (node.alternate == null ? false : hasTryReturn(node.alternate))
      );
    }
    case "SwitchStatement": {
      return some(node.cases, (node) => some(node.consequent, hasTryReturn));
    }
    case "WhileStatement": {
      return hasTryReturn(node.body);
    }
    case "DoWhileStatement": {
      return hasTryReturn(node.body);
    }
 ...
 */
const makeClosureFrame = (param, self, delay_return) => {
  if (param.type === "arrow") {
    return ARROW_CLOSURE_FRAME;
  } else if (param.type === "method") {
    return { type: "routine-method", proto: param.proto };
                        ...
                        liftSequenceXX(
                          ...
                          liftSequence_X(
                            ...
                            setupRoutineFrame(
                              ...
                              makeClosureFrame(param, self, delay_return),
                              { mode },
                            ),
                          ),
                          mode === "sloppy" &&
