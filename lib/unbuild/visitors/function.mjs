  ...
  node.type === "RestElement"
    ? unbuildInitializePatternEffect(
        ...
        scope,
        {
          right:
            index === 0
              ? makeReadFunctionArgumentsExpression(scope, { path })
              : makeApplyExpression(
    ...
    : unbuildInitializePatternEffect({ meta, node, path }, scope, {
...
const listHeadEffect = (
  ...
  return [
                      ...
                      makeDataDescriptorExpression(
                        {
                          value: makeGetExpression(
                            makeReadFunctionArgumentsExpression(scope, {
                              path,
                            }),
                            makePrimitiveExpression("length", path),
                            path,
    ...
    ...flatMap(enumerate(sites.length), (index) =>
      unbuildParameter(sites[index], scope, { index }),
    ),
  ];
};
...
export const unbuildFunction = (
  ...
  const scope = {
    ...
    mode: /** @type {"strict" | "sloppy"} */ (
      parent_scope.mode === "strict" || isClosureStrict(node)
        ? "strict"
        : "sloppy"
    ),
  };
  ...
  const outcome = listClosureParameter(node, scope.mode === "strict");
  switch (outcome.type) {
    case "failure": {
    ...
    case "success": {
      ...
      // f = 123;
      // console.log(g()); // 123
      const callee = getCallee(scope.mode, node);
      const input = getInput(scope.mode, node);
                      ...
                      (scope) =>
                        initSequence(
                          [
                            ...
                            ...map(
                              listHeadEffect(drillArray(sites.params), scope, {
                                meta: metas.head,
                                simple,
                                self,
                                callee,
                            ...
                            ...(isBlockStatementSite(sites.body)
                              ? unbuildClosureBody(sites.body, scope, {
                                  parameters: [
                                    ...(input === null ? [] : [input]),
                          ...
                          makeReturnArgumentExpression(
                            ...
                            scope,
                            {
                              argument: isNotBlockStatementSite(sites.body)
                                ? unbuildExpression(sites.body, scope, {})
                                : null,
                            },
                          ),
                        ),
