...
const listHeadStatement = (
  ...
  const metas = splitMeta(meta, ["callee", "arguments", "params"]);
  return [
    ...(callee === null
      ? []
      : listScopeInitializeStatement(
    ...
    ...(pairs.length === 1 && pairs[0].node.type === "RestElement"
      ...
      : flatMap(
          zip(
            enumMeta(forkMeta(metas.params), pairs.length),
            enumerate(pairs.length),
          ),
          ([meta, index]) =>
            unbuildParameter(pairs[index], context, {
              meta: forkMeta(meta),
              index,
            }),
        )),
