...
/** @type {import("../annotate/hoisting-public").Binding} */
 ...
 * ) => import("../annotate/hoisting-public").Binding}
 */
 ...
 * >}
 */
const unbuildParameter = ({ node, path, meta }, { scope, deadzone, index }) => {
 ...
 * >}
 */
const listParameterEffect = ({ node, path, meta }, { scope, deadzone }) =>
  liftSequenceX(
    flat,
    flatSequence(
      mapReduceIndex(
        ...
        (deadzone, index) =>
          unbuildParameter(
            ...
            { scope, deadzone, index },
          ),
        (deadzone, index) =>
          updateDeadzonePattern(deadzone, "let", node[index]),
        deadzone,
      ),
    ),
  );
 ...
 *   context: import("../function").ClosureParam & {
 *     scope: import("../scope").Scope,
 *     deadzone: import("../deadzone").Deadzone,
...
export const unbuildFunction = (
  { node, path, meta },
  { scope: parent_scope, deadzone: parent_deadzone, name, ...param },
) => {
  const scope =
  ...
  if (unbound.length > 0) {
    throw new AranExecError("unbound binding in closure", { unbound });
  }
  ...
  const deadzone = declareDeadzone(captureDeadzone(parent_deadzone), bindings);
                              ...
                              ? liftSequenceX(
                                  ...
                                  liftSequenceX_(
                                    ...
                                    unbuildClosureBody(
                                      ...
                                      { scope, deadzone, hoisting, simple },
                                    ),
                                    path,
                                  ),
                                )
