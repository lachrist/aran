...
const makeUpdateValueExpression = (operator, left, right, path) => {
  const cache = {
    var: mangleMetaVariable(
      path,
      makeFragment(LOCATION, /** @type {__unique} */ ("update_logical_left")),
    ),
    val: left,
  };
  switch (operator) {
  ...
  switch (node.type) {
    ...
    case "MemberExpression": {
      ...
      const record = {
        object: mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("object")),
        ),
        key: mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("key")),
        ),
        value: mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("value")),
        ),
      };
      if (isNotSuperMemberExpression(node)) {
        ...
        const super_right = mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("super_right")),
        );
        if (prefix) {
          return makeSequenceExpression(
            makeWriteEffect(key.var, key.val, true, path),
    ...
    case "Identifier": {
      const old_value = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("old_value")),
      );
      const new_value = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("new_value")),
      );
      if (prefix) {
        return makeLongSequenceExpression(
          [
  ...
  switch (node.type) {
    ...
    case "MemberExpression": {
      ...
      const object = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("effect_object")),
      );
      const key = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("effect_key")),
      );
      if (isNotSuperMemberExpression(node)) {
        return [
          makeWriteEffect(
            ...
            makeScopeSetSuperExpression(
              ...
              { var: key, val: null },
              {
                var: mangleMetaVariable(
                  ...
                  makeFragment(
                    LOCATION,
                    /** @type {__unique} */ ("super_right_effect"),
                  ),
                ),
                val: makeUpdateValueExpression(
                  operator,
