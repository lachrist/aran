...
const duplicateUpdate = ({ path, meta }, update) => {
  switch (update.type) {
    ...
    case "member": {
      return bindSequence(
        duplicateObject({ path, meta }, update.object),
        ([object1, object2]) =>
          bindSequence(
            duplicateKey({ path, meta }, update.key),
            ([key1, key2]) =>
              zeroSequence([
...
const makeIncrementExpression = ({ path, meta }, { operator, left, right }) => {
  if (right === null) {
    return bindSequence(
      cacheConstant(forkMeta((meta = nextMeta(meta))), left, path),
      (left) =>
        makeConditionalExpression(
          makeBinaryExpression(
            "===",
...
const makeUpdateEffect = (
  ...
  bindSequence(
    duplicateUpdate({ path, meta: forkMeta((meta = nextMeta(meta))) }, update),
    ([update1, update2]) => {
      if (hasNarrowKey(LOGICAL_OPERATOR_RECORD, operator)) {
        ...
        return makeUpdateSaveEffect({ path, meta }, scope, {
          update: update2,
        ...
        switch (result) {
          case "new": {
            return bindSequence(
              ...
              (result) =>
                makeSequenceExpression(
                  makeUpdateSaveEffect({ path, meta }, scope, {
                    update: update2,
    ...
    return bindSequence(
      ...
      (object) =>
        bindSequence(
          ...
          (key) =>
            makeUpdateExpression({ path, meta }, scope, {
              result,
    ...
    return bindSequence(
      ...
      (object) =>
        bindSequence(
          ...
          (key) =>
            makeUpdateEffect({ path, meta }, scope, {
              update: { type: "member", object, key },
              operator,
              increment,
            }),
