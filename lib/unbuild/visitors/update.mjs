 ...
 */
export const unbuildUpdateExpression = wrapOrigin(
  ({ node, path }, context, { update, prefix, operator }) => {
    switch (node.type) {
        ...
        if (isNotSuperMemberExpression(node)) {
          ...
          const value = {
            ...
            val: makeGetExpression(
              makeReadExpression(object.var),
              makeReadExpression(key.var),
            ),
          };
          if (prefix) {
            return makeSequenceExpression(
              makeWriteEffect(object.var, object.val, true),
              makeSequenceExpression(
                makeWriteEffect(key.var, key.val, true),
                makeSetExpression(
                  context.strict,
                  makeReadExpression(object.var),
                  makeReadExpression(key.var),
                  makeBinaryExpression(operator, value.val, update),
                ),
              ),
            );
          } else {
            return makeSequenceExpression(
              ...
              makeSequenceExpression(
                makeWriteEffect(key.var, key.val, true),
                makeSequenceExpression(
                  makeWriteEffect(value.var, value.val, true),
                  makeSequenceExpression(
                    makeExpressionEffect(
                      makeSetExpression(
                        context.strict,
 ...
 */
export const unbuildUpdateEffect = wrapOriginArray(
  ({ node, path }, context, { update, operator }) => {
    switch (node.type) {
      // > console.log("foo") += console.log("bar");
      // foo
      // Uncaught ReferenceError: Invalid left-hand side in assignment
