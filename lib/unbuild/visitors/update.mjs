 ...
 * >}
 */
const makeUpdateLoadExpression = (
  { path, meta },
  { scope, deadzone, assignee },
) => {
  switch (assignee.type) {
    case "variable": {
      return makeScopeLoadExpression({ path, meta }, scope, {
        ...
        status: lookupDeadzone(deadzone, assignee.variable),
      });
    }
    case "member": {
      return makeGetMemberExpression(
        { path, meta },
        {
          scope,
 ...
 * }}
 */
const makeUpdateSaveContext = (scope, deadzone, assignee, value) => ({
  scope,
  deadzone,
  assignee,
  value,
 ...
 * >}
 */
const makeUpdateSaveEffect = (
  { path, meta },
  { scope, deadzone, assignee, value },
) => {
  switch (assignee.type) {
    case "variable": {
...
const makeUpdateEffect = (
  { path, meta },
  { scope, deadzone, assignee, assigner: { operator, increment } },
) =>
  incorporateEffect(
    bindSequence(
      duplicateAssignee(
        ...
        if (hasNarrowKey(LOGICAL_OPERATOR_RECORD, operator)) {
          return liftSequence__XX__(
            ...
            makeUpdateLoadExpression(
              { path, meta: forkMeta((meta = nextMeta(meta))) },
              { scope, deadzone, assignee: assignee1 },
            ),
            makeUpdateSaveEffect(
              { path, meta: forkMeta((meta = nextMeta(meta))) },
              {
                ...
                deadzone,
                assignee: assignee2,
                value: increment ?? makeIntrinsicExpression("undefined", path),
              },
            ),
            EMPTY,
            path,
          ...
          return callSequence_X(
            ...
            liftSequence___X(
              ...
              callSequence__X_(
                ...
                makeUpdateLoadExpression(
                  { path, meta: forkMeta((meta = nextMeta(meta))) },
                  { scope, deadzone, assignee: assignee2 },
                ),
                increment,
              ),
            ),
...
export const makeUpdateExpression = (
  { path, meta },
  { scope, deadzone, assignee, assigner: { result, operator, increment } },
) =>
  incorporateExpression(
    bindSequence(
      duplicateAssignee(
 ...
 * }}
 */
export const makeUpdateEffectContext = (scope, deadzone, assigner) => ({
  scope,
  deadzone,
  assigner,
});
 ...
 * }}
 */
export const makeUpdateExpressionContext = (scope, deadzone, assigner) => ({
  scope,
  deadzone,
  assigner,
});
