 ...
 */
const unbuildExportDeclaration = wrapOriginArray(({ node }, _context) => {
  switch (node.type) {
 ...
 * ) => aran.Link<unbuild.Atom>}
 */
const unbuildExportSpecifier = wrapOrigin(({ node }, _context) =>
 ...
 * ) => aran.Link<unbuild.Atom>}
 */
const unbuildImportSpecifier = wrapOrigin(({ node }, _context, { source }) => {
  switch (node.type) {
 ...
 * ) => aran.Link<unbuild.Atom>[]}
 */
export const unbuildModuleDeclaration = wrapOriginArray(
  ({ node, path }, context) => {
    switch (node.type) {
