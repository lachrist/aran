...
import { unbuildInit, unbuildLeft } from "./left.mjs";
                    ...
                    (right) =>
                      bindSequence(
                        cacheConstant(
                          forkMeta((meta = nextMeta(meta))),
                          makeApplyExpression(
                            makeIntrinsicExpression("aran.listForInKey", hash),
                            makeIntrinsicExpression("undefined", hash),
  ...
  switch (node.type) {
    case "ClassDeclaration": {
      return mapSequence(
        ...
        ({ 0: part, 1: scope }) =>
          tuple2(
            map(part, (part) => makeEffectStatement(part, hash)),
            scope,
          ),
      );
    }
    case "VariableDeclaration": {
      if (node.kind === "let" || node.kind === "const") {
        return mapSequence(
          mapReduceSequence(
            node.declarations,
            (node, scope) => unbuildInitializeDeclarator(node, meta, scope),
            scope,
          ),
          ({ 0: part, 1: scope }) =>
            tuple2(
              map(flat(part), (part) => makeEffectStatement(part, hash)),
              scope,
            ),
        );
      } else {
...
export const unbuildBody = (nodes, meta, scope, labeling) =>
  bindSequence(
    flatSequence(
      map(nodes, (node) =>
        unbuildHoistedStatement(node, forkMeta((meta = nextMeta(meta))), scope),
      ),
    ),
    (part1) =>
      mapSequence(
        ...
        ({ 0: part2, 1: _scope }) => concatXX(flat(part1), flat(part2)),
      ),
  );
