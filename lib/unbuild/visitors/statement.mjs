...
const LOCATION = /** @type {__location} */ (
  "lib/unbuild/visitors/statement.mjs"
);
  ...
  switch (node.type) {
            ...
            hasArgument(node)
              ? {
                  var: mangleMetaVariable(
                    path,
                    makeFragment(LOCATION, /** @type {__unique} */ ("return")),
                  ),
      ...
      return node.id == null
        ...
        : listScopeInitializeStatement(
            ...
            /** @type {estree.Variable} */ (node.id.name),
            {
              var: mangleMetaVariable(
                path,
                makeFragment(LOCATION, /** @type {__unique} */ ("class_right")),
              ),
    ...
    case "WithStatement": {
      const frame = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("with")),
      );
      return [
    ...
    case "DoWhileStatement": {
      ...
      const initial = mangleMetaVariable(
        path,
        makeFragment(LOCATION, /** @type {__unique} */ ("initial_do")),
      );
      return [
        ...makeUndefinedCompletion(path, completion),
        ...wrapBlock(
        ...
        ...wrapBlock(
          ...
          (context) => {
            const right = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("for_in_right")),
            );
            ...
            const index = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("for_in_index")),
            );
            const keys = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("for_in_keys")),
            );
            return [
        ...
        ...wrapBlock(
          ...
          (context) => {
            const generator = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("generator")),
            );
            const iterator = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("iterator")),
            );
            const next = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("next")),
            );
            return [
              ...unbuildLeftInit(drill({ node, path }, "left"), context, null),
              makeEffectStatement(
        ...
        ...wrapBlock(
          ...
          (context) => {
            const discriminant = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("discrimant")),
            );
            const matched = mangleMetaVariable(
              path,
              makeFragment(LOCATION, /** @type {__unique} */ ("matched")),
            );
            return [
              makeEffectStatement(
                makeWriteEffect(
