...
export const unbuildStatement = (
  ...
  switch (node.type) {
    ...
    case "WithStatement": {
      const metas = splitMeta(meta, ["object_cache", "object", "body"]);
      return [
        ...makeUndefinedCompletion(path, completion),
        makeEffectStatement(
          makeWriteEffect(
            mangleMetaVariable(metas.object_cache),
    ...
    case "DoWhileStatement": {
      const metas = splitMeta(meta, ["initial", "test", "body"]);
      const loop = {
        break: mangleEmptyBreakLabel(path),
        continue: mangleEmptyContinueLabel(path),
      };
      const initial = mangleMetaVariable(metas.initial);
      return [
        ...makeUndefinedCompletion(path, completion),
        ...wrapBlock(
          context,
    ...
    case "ForInStatement": {
      ...
      return [
        ...
        ...wrapBlock(
          ...
          (context) => {
            const right = mangleMetaVariable(metas.right_cache);
            const accumulation = mangleMetaVariable(metas.accumulation_cache);
            const prototype = mangleMetaVariable(metas.prototype_cache);
            const index = mangleMetaVariable(metas.index_cache);
            const keys = mangleMetaVariable(metas.keys_cache);
            return [
                  ...
                  makeApplyExpression(
                    ...
                    [
                      unbuildExpression(
                        ...
                        context,
                        { meta: metas.right, name: ANONYMOUS },
                      ),
                    ],
                    path,
                  ),
    ...
    case "ForOfStatement": {
      ...
      return [
        ...
        ...wrapBlock(
          ...
          (context) => {
            const generator = mangleMetaVariable(metas.generator_cache);
            const iterator = mangleMetaVariable(metas.iterator_cache);
            const next = mangleMetaVariable(metas.next_cache);
            return [
              ...unbuildLeftInit(drill({ node, path }, "left"), context, {
                meta: metas.left_init,
              }),
    ...
    case "SwitchStatement": {
      ...
      return [
        ...
        ...wrapBlock(
          ...
          (context) => {
            const discriminant = mangleMetaVariable(metas.discriminant_cache);
            const matched = mangleMetaVariable(metas.matched_cache);
            return [
              makeEffectStatement(
                makeWriteEffect(
                  discriminant,
