...
import { unbuildKeyExpression, unbuildKeySetup } from "./key.mjs";
...
const unbuildCommonProperty = ({ node, path, meta }, context, { self }) => {
  ...
  if (isNameSite(sites.value)) {
    ...
    const { setup, value: key } = unbuildKeySetup(sites.key, context, {
      computed,
    });
    const name = makeNameKeyExpression(key, node.kind, path);
    return [
      makeLongSequenceExpression(setup, makeReadKeyExpression(key, path), path),
