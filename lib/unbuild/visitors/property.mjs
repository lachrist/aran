 ...
 * >}
 */
export const unbuildProtoProperty = (node, meta, context) => {
 ...
 * >}
 */
export const unbuildInitProperty = (node, meta, context) => {
  const { _hash: hash } = node;
  if (isProtoProperty(node)) {
    ...
    return liftSequenceXX(
      ...
      unbuildExpression(node.value, forkMeta((meta = nextMeta(meta))), context),
    );
  }
};
 ...
 * >}
 */
export const unbuildProperty = (node, meta, context, self) => {
  const { _hash: hash } = node;
  switch (node.type) {
    case "SpreadElement": {
          ...
          liftSequenceX_(
            ...
            liftSequence__X_(
              ...
              liftSequence_X(
                concat__,
                makeReadCacheExpression(self, hash),
                unbuildProtoProperty(node, meta, context),
              ),
              hash,
            ),
            hash,
