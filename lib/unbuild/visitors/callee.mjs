...
const finalizeOptionalCall = (callee, this_, arguments_, { path, meta }) =>
  makeCacheExpression(callee, path, mangleMetaVariable(meta), (callee) =>
    makeConditionalExpression(
      makeBinaryExpression(
        "==",
        callee,
  ...
  switch (node.type) {
    ...
    case "MemberExpression": {
      if (isNotSuperMemberExpression(node)) {
        const metas = splitMeta(meta, ["object", "key", "finalize"]);
        return makeCacheExpression(
          ...
          (object) =>
            finalizeCall(
              ...
              object,
              arguments_,
              { optional, path, meta: forkMeta(metas.finalize) },
            ),
        );
      } else {
