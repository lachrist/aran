...
/**
 * @type {(
 *   site: import("../site").Site<import("../../estree").MemberExpression>,
 *   context: {
 *     scope: import("../scope").Scope,
 ...
 * >}
 */
const unbuildMemberCallee = ({ node, path, meta }, { scope, deadzone }) =>
  bindTwoSequence(
    bindSequence(
      unbuildObject(
    ...
    ([object1, object2], key) =>
      liftSequenceXX(
        ...
        makeGetMemberExpression(
          { path, meta: forkMeta((meta = nextMeta(meta))) },
          { scope, object: object1, key: convertKey({ path }, key) },
        ),
        makeThisExpression(
          { path, meta: forkMeta((meta = nextMeta(meta))) },
          scope,
 ...
 * >}
 */
const unbuildChainMemberCallee = ({ node, path, meta }, { scope, deadzone }) =>
  bindTwoSequence(
    bindSequence(
      guard(
        node.optional,
...
/**
 * @type {(
 *   site: import("../site").Site<import("../../estree").Super | import("../../estree").Expression>,
 *   context: {
 *     scope: import("../scope").Scope,
 *     deadzone: import("../deadzone").Deadzone,
 *   },
...
export const unbuildChainCallee = (
  ...
  if (node.type === "ChainExpression") {
    return unbuildChainCallee(drillSite(node, path, meta, "expression"), {
      scope,
      deadzone,
    });
    ...
    return unbuildChainMemberCallee({ node, path, meta }, { scope, deadzone });
  } else {
    return liftSequenceX_(
      makeRegularCallee,
      unbuildChainElement({ node, path, meta }, { scope, deadzone }),
 ...
 * >}
 */
export const unbuildCallee = ({ node, path, meta }, { scope, deadzone }) => {
  if (node.type === "Super") {
    return zeroSequence(SUPER_CALLEE);
  } else if (node.type === "Identifier") {
    if (node.name === "eval") {
    ...
    return unbuildMemberCallee({ node, path, meta }, { scope, deadzone });
  } else if (
