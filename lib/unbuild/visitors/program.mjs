import { recordCompletion, hoist, listBinding } from "../query/index.mjs";
...
/**
 * @type {(
 *   duplicate: import("../query/hoist-public").Duplicate,
 * ) => import("../prelude").EarlyErrorPrelude}
 */
const toEarlyErrorPrelude = ({ origin, variable }) =>
...
const setupScope = (
  ...
  if (scope === null) {
    if (sort === "eval.local.deep") {
      ...
      return mapSequence(
        prependSequence(
          map(report, toEarlyErrorPrelude),
          liftSequenceXX(
            extendScope,
            mapSequence(
              setupRootFrame(
            ...
            setupProgramFrame({ node, path }, listBinding(hoisting, path)),
          ),
        ),
 ...
 * >}
 */
const unbuildCompletion = ({ path }, scope, { completion, hoisting }) => {
  switch (completion.type) {
    ...
    case "direct": {
      return unbuildExpression(completion.site, scope, { hoisting });
    }
    default: {
