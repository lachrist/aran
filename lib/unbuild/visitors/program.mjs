...
import { scopeClosureBlock, makeScopePseudoBlock } from "../scope/index.mjs";
...
import {
  ...
  initSequence,
  passSequence,
  unwrapSequenceClosureBlock,
} from "../sequence.mjs";
import { listSetupClosureEffect } from "../param/index.mjs";
import { setupWritableCache } from "../cache_new.mjs";
    ...
    case "module": {
      ...
      return makeModuleProgram(
        ...
        unwrapSequenceClosureBlock(
          { path },
          bindSequence(
            scopeClosureBlock({ path }, context, {
      ...
      if (tail === null) {
        return makeScriptProgram(
          makeScopePseudoBlock(
            context,
            { situ: "global", link: null, kinds: hoisting.external },
            (context) =>
              makeScopePseudoBlock(
                context,
                { situ: "local", link: null, kinds: hoisting.internal },
                (context) =>
                  initCache(
                    "writable",
                    makePrimitiveExpression({ undefined: null }, path),
