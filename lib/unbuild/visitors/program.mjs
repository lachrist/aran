...
const toLateDeclareOperation = (binding, mode) => {
  ...
  if (binding.initial !== "undefined") {
    throw new AranExecError(
      "only undefined-initialized binding can escape eval",
      {
        binding,
      },
    );
  ...
  if (binding.write !== "perform") {
    throw new AranExecError("only writable binding can escape eval", {
      binding,
    });
  }
  if (binding.sloppy_function_near > 0) {
    throw new AranExecError("near sloppy function cannot escape eval", {
      binding,
    });
  }
  return {
    variable: binding.variable,
    conflict: binding.sloppy_function_away > 0 ? "report" : "ignore",
  };
};
