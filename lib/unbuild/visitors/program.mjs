...
import { unprefixClosureBody } from "../prefix.mjs";
...
const setupRootScope = (
  ...
  const hoisting = {
    ...
    block: flatMap(getProgramBody(node), (node) => hoistBlock(sort.mode, node)),
  };
...
const setupNodeScope = ({ node, path, meta }, scope, _sort) => {
  ...
  const hoisting = {
    closure: flatMap(getProgramBody(node), (node) => hoistClosure(mode, node)),
      ...
      return makeProgram(
        ...
        makeClosureBlock(
          setupEarlyError(
            ...
            {
              type: "Program",
              sourceType: sort.kind === "module" ? "module" : "script",
              body: [],
            },
          ),
          path,
