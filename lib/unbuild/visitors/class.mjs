...
const LOCATION = /** @type {__location} */ ("lib/unbuild/visitors/class.mjs");
  ...
  node.key.type === "PrivateIdentifier"
    ? [
        [
          ...
          mangleMetaVariable(
            path,
            makeFragment(LOCATION, /** @type {__unique} */ ("private")),
          ),
        ],
      ]
    : [];
  ...
  switch (node.type) {
    ...
    case "PropertyDefinition": {
      if (isPropertyDefinitionNotComputed(node)) {
        ...
        const variable = mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("property_key")),
        );
        return {
          head: [
            makeWriteEffect(
    ...
    case "MethodDefinition": {
      if (isMethodDefinitionNotComputed(node)) {
        ...
        const variable = mangleMetaVariable(
          path,
          makeFragment(LOCATION, /** @type {__unique} */ ("method_key")),
        );
...
export const unbuildClassBody = (
  ...
  const self = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("self")),
  );
  const field = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("field")),
  );
  const prototype = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("prototype")),
  );
...
export const unbuildClass = (
  ...
  const super_ = mangleMetaVariable(
    path,
    makeFragment(LOCATION, /** @type {__unique} */ ("super")),
  );
