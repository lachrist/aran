 ...
 * >}
 */
const unbuildKeyPass = ({ node, path, meta }, { scope, deadzone }) =>
  mapSequence(
    flatSequence(
      map(
        filterNarrow(
...
const unbuildConstructorPass = (
  { node, path, meta },
  { scope, deadzone, derived, name, field },
) => {
  ...
  if (index === -1) {
    ...
    return initErrorExpression("Multiple constructor definitions", path);
  }
};
...
export const unbuildMethod = (
  { node, path, meta },
  { scope, deadzone, key, konstructor, prototype },
) => {
...
export const unbuildMethodPass = (
  { node, path, meta },
  { scope, deadzone, keys, konstructor, prototype },
) =>
  liftSequenceX(
    flat,
    flatSequence(
 ...
 * >}
 */
const unbuildProperty = ({ node, path, meta }, { scope, deadzone, key }) => {
  const value = hasPropertyValue(node)
    ? unbuildNameExpression(
        drillSite(node, path, forkMeta((meta = nextMeta(meta))), "value"),
        { scope, deadzone, name: { type: "property", kind: "init", key } },
      )
 ...
 * >}
 */
const unbuildStatic = ({ node, path, meta }, { scope, deadzone, keys }) => {
...
const unbuildClassPropertyPass = (
  { node, path, meta },
  { scope, deadzone, keys, konstructor },
) =>
  liftSequence__X_(
    makeClosureExpression,
    "function",
            ...
            liftSequenceX(
              ...
              flatSequence(
                map(
                  ...
                  (site) => unbuildStatic(site, { scope, deadzone, keys }),
                ),
              ),
            ),
            makeIntrinsicExpression("undefined", path),
...
const unbuildInstancePropertyPass = (
  { node, path, meta },
  { scope, deadzone, keys, prototype }, // await|yield forbidden here
) =>
  liftSequence__X_(
    makeClosureExpression,
    "function",
...
const unbuildClassInner = (
  { node, path, meta },
  { scope, deadzone, zuper, keys, field, konstructor, prototype },
) =>
  liftSequenceX(
    flat,
    flatSequence([
...
const unbuildClassBody = (
  { node, path, meta },
  { scope, deadzone, name, self, zuper },
) =>
  incorporateExpression(
    bindSequence(
      mapSequence(
                ...
                bindSequence(
                  callSequence_X_(
                    ...
                    unbuildConstructorPass(
                      ...
                      { scope, deadzone, name, derived: zuper !== null, field },
                    ),
                    path,
                  ),
                  (konstructor) =>
...
export const unbuildClass = (
  { node, path, meta },
  { scope: parent_scope, deadzone, name },
) => {
      ...
      ({ self, scope }) =>
        hasClassSuper(node)
          ...
          : unbuildClassBody(
              ...
              { scope, deadzone, name, self, zuper: null },
            ),
    ),
    path,
  );
