 ...
 * ) => aran.Effect<unbuild.Atom<S>>[]}
 */
const listNonComputedFieldEffect = (
  { node, path },
  context,
  { target, serial },
) => {
 ...
 * ) => aran.Effect<unbuild.Atom<S>>[]}
 */
const listFieldEffect = ({ node, path }, context, { key, target, serial }) => [
  makeExpressionEffect(
    makeApplyExpression(
      makeIntrinsicExpression("Reflect.defineProperty", serial),
      makePrimitiveExpression({ undefined: null }, serial),
 ...
 * ) => aran.Expression<unbuild.Atom<S>>}
 */
const unbuildConstructor = (pairs, context, { name, hash, serial }) => {
  switch (pairs.length) {
    case 0:
      return makeDefaultConstructor(context, { name, hash, serial });
    case 1:
    ...
    default:
      throw new SyntaxAranError("multiple constructor", TODO);
  }
};
