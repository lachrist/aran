 ...
 * ) => aran.Statement<unbuild.Atom>[]}
 */
const unbuildClassPropertyPass = ({ node, path, meta }, context, { keys }) =>
  flatMap(
    filterNarrow(
      drillArray(drill({ node, path, meta }, ["body"]).body),
      isStaticSite,
...
export const unbuildClass = ({ node, path, meta }, context, { name }) => {
  ...
  if (self === null) {
    return kontinue({ ...context, mode: "strict" });
  } else {
    return makeSetupExpression(
      mapSetup(
        extendFakeScope(
