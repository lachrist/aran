 ...
 */
const makeKeyExpression = (key, path) => {
  switch (key.kind) {
    case "dynamic": {
    ...
    default: {
      throw new AranTypeError("invalid key kind", key);
    }
  }
};
 ...
 * ) => aran.Expression<unbuild.Atom>}
 */
const makeNameExpression = (key, prefix, path) => {
  switch (key.kind) {
    case "dynamic": {
      return guard(
        prefix !== "",
        (node) =>
          makeBinaryExpression(
            "+",
            makePrimitiveExpression(prefix, path),
            node,
            path,
          ),
        makeConditionalExpression(
          ...
          makeBinaryExpression(
            ...
            makeBinaryExpression(
              ...
              makeApplyExpression(
                makeIntrinsicExpression("String.prototype.slice", path),
                makeApplyExpression(
                ...
                [
                  makePrimitiveExpression(7, path),
    ...
    case "static": {
      switch (key.access) {
        case "public": {
          return makePrimitiveExpression(`${prefix}${key.value}`, path);
        }
        case "private": {
          return makePrimitiveExpression(`${prefix}#${key.value}`, path);
        }
 ...
 *     ztatic: boolean,
 *     computed: boolean,
 *     kontinue: (key: Key | null) => aran.Expression<unbuild.Atom>
 *   },
 * ) => aran.Expression<unbuild.Atom>}
 */
const unbuildKey = (
  ...
  if (computed) {
    ...
    return makeInitCacheExpression(
      ...
      makeApplyExpression(
        ...
        makePrimitiveExpression({ undefined: null }, path),
        [
          unbuildKeyExpression({ node, path, meta: metas.unbuild }, context, {
            convert: true,
            computed,
          }),
        ],
    ...
    switch (node.type) {
      ...
      case "Literal": {
        if (isRegExpLiteral(node)) {
          return makeSyntaxErrorExpression(
            "Illegal non-computed key: regexp literal",
            path,
          );
          ...
          if (ztatic && node.value === "prototype") {
            return makeSyntaxErrorExpression(
              "Illegal static key: 'prototype'",
              path,
            );
...
const unbuildMethod = (
  ...
  const method = unbuildFunction(
    ...
    {
      type: "method",
      name: makeNameExpression(key, PREFIXES[node.kind], path),
  ...
  const value = isNotNullishSite(site)
    ? isNameSite(site)
      ? unbuildNameExpression(site, context, {
          name: makeNameExpression(key, "", path),
        })
