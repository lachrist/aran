...
const {
  ...
  undefined,
  Object,
  Object: { create, entries, fromEntries },
  Reflect: { ownKeys },
} = globalThis;

/** @type {<K extends PropertyKey, V>(record: Record<K, V>) => [K, V][]} */
export const listEntry = entries;

/** @type {<K extends PropertyKey, V>(entries: [K, V][]) => Record<K, V>} */
export const reduceEntry = fromEntries;

export const listKey =
  /** @type {<K extends PropertyKey, V>(record: Record<K, V>) => K[]} */ (
    ownKeys
  );

/**
 * @type {<K extends PropertyKey, V>() => Record<K, V>}
 */
export const createRecord = () => create(null);
