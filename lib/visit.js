...
ArrayLite.forEach(["statement", "expression"], (type) => {
  closures[type] = (node, depth, options) => {
    ...
    if (options.visitors["_" + constructor + "_"]) {
      if (length === 0) {
        return options.visitors["_" + constructor + "_"](mapped);
      }
      if (length === 1) {
        return options.visitors["_" + constructor + "_"](fields[0], mapped);
      }
      if (length === 2) {
        return options.visitors["_" + constructor + "_"](fields[0], fields[1], mapped);
      }
      if (length === 3) {
        return options.visitors["_" + constructor + "_"](fields[0], fields[1], fields[2], mapped);
      }
