...
export const instrument = (raw_program, raw_options) => {
  ...
  for (const clash of clashes) {
    throw new AranClashError(clash, options.escape);
  }
  const logs = [...unbuild_log_array, ...weave_log_array];
  ...
  for (const log of logs) {
    if (log.name === "SyntaxError") {
      switch (options.error) {
        ...
        default: {
          throw new AranTypeError(options.error);
        }
      }
    }
  }
  if (options.warning === "silent") {
    ...
    throw new AranTypeError(options.warning);
  }
