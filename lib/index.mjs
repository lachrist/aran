import { cookWarning, unbuild } from "./unbuild/index.mjs";
...
import { AranTypeError, AranInputError, AranSyntaxError } from "./error.mjs";
...
export const instrument = (raw_file, raw_conf) => {
  ...
  const { root: root2, warnings } = unbuild(source, config);
  const { root: root3 } = weave(
    /** @type {import("./weave/atom").ArgProgram} */ (
      /** @type {import("./lang/syntax").Program<any>} */ (root2)
    ),
    config,
  );
  const { root: root4 } = rebuild(
    {
      root: /** @type {import("./lang/syntax").Program<import("./rebuild/atom").Atom>} */ (
        /** @type {import("./lang/syntax").Program<any>} */ (root3)
      ),
    },
    config,
  );
  return {
    ...root4,
    _aran_warning_array: map(warnings, cookWarning),
  };
};
