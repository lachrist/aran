...
import { hasNarrowObject, listAlias } from "./util/index.mjs";
...
export const instrument = (raw_program, raw_config) => {
  ...
  if (config.debug_alias) {
    for (const alias of listAlias(root2)) {
      if (hasNarrowObject(alias, "type")) {
        throw new AranError("Unexpected alias", { root: root2, alias });
      }
    }
  }
