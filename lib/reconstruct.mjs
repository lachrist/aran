...
import { unpackPrimitive } from "./deconstruct/syntax.mjs";
...
/** @type {(node: EstreeVariableDeclaration) => EstreeStatement[]} */
const cleanVariableDeclaration = (node) =>
  node.declarations.length === 0 ? [] : [node];

/** @type {(node: EstreeStatement) => EstreeBlockStatement} */
...
/** @type {(node: ControlBlock<unknown>, context: Context) => EstreeStatement} */
...
/** @type {(node: PseudoBlock<unknown>, context: Context) => EstreeStatement[]} */
...
/** @type {(link: Link, context: Context) => EstreeProgramStatement[]} */
  ...
  switch (node.type) {
    ...
    case "ApplyExpression":
      ...
      if (
        ...
        return {
          type: "ObjectExpression",
          properties: /** @type {EstreeProperty[]} */ ([
