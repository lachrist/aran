...
import { isParameter, unpackPrimitive } from "./node.mjs";
...
/** @type {<V, T>(node1: aran.Expression<V, T>, node2: aran.Expression<V, T>) => boolean} */
...
/** @type {<X extends estree.Variable | estree.Label>(print: string) => X} */
const escape = (string) => apply(replace, string, ESCAPE);

/** @type {(prefix: estree.Variable, source: estree.Source, specifier: estree.Specifier | null) => estree.Variable} */
const encodeImport = (prefix, source, specifier) =>
  encodeInternal(
    ...
    specifier === null
      ? escape(source)
      : append(escape(source), escape(specifier)),
  );
...
/** @type {<V extends string>(prefix: estree.Variable, variable: V) => string} */
const encodeRegular = (prefix, variable) =>
  encodeInternal(prefix, "reg", escape(variable));
...
/** @type {(prefix: estree.Variable, argument: "src" | "key" | "val" | "arg" | "mtd") => estree.Variable} */
...
const accumulateLabel = (node, label) => ({
  ...
  label: {
    type: "Identifier",
    name: /** @type {estree.Label} */ (escape(label)),
  },
  body: node,
});
...
/** @type {<V, T>(nodes: aran.Node<V, T>[], context: Context) => estree.Statement[]} */
...
/** @type {<V extends string, T>(node: aran.Program<V, T>, context: Context) => estree.Program} */
...
/** @type {<V extends string, T>(node: aran.ControlBlock<V, T>, context: Context) => estree.Statement} */
...
/**
 * @type {<V extends string, T>(
 *   node: aran.ClosureBlock<V, T>,
 *   context: Context,
...
/** @type {<V extends string, T>(node: aran.PseudoBlock<V, T>, context: Context) => estree.Statement[]} */
...
/** @type {<T>(link: aran.Link<T>, context: Context) => estree.ProgramStatement[]} */
...
/** @type {<V extends string, T>(node: aran.Statement<V, T>, context: Context) => estree.Statement} */
...
/** @type {<V extends string, T>(node: aran.Effect<V, T>, context: Context) => estree.Expression} */
...
/** @type {<V extends string, T>(node: aran.Expression<V, T>, context: Context) => estree.Expression} */
