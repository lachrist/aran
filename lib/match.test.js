...
///////////////
{
  const success = (code1, code2, mapping1) => Assert.deepEqual(
    Match._statement(
      Lang.ParseStatement(code1),
      Lang.ParseStatement(code2),
      (success, mapping2) => (
        Assert.ok(success),
        Assert.deepEqual(mapping1, mapping2),
        "foo")),
...
////////////////
{
  ...
  // eval //
  failure(`eval(§y1, §z1, x1)`, `eval(§y2, x1)`, /^Structural mismatch .* 2 and 1\n/);
  success(`eval(§y1, §z1, x1)`, `eval(§y2, §z2, x2)`, {__proto__:null, x1:"x2", y1:"y2", z1:"z2"});
  // Combiners //
  // apply //
