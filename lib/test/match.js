...
exports._match_expression = (expression1, expression2) => Lang._allign_expression(expression_callback_object, "expression", expression1, expression2);

exports._match_statement = (statement1, statement2) => Lang._allign_expression(statement_callback_object, "statement", statement1, statement2);

exports._match_block = (block1, block2) => Lang._allign_expression(block_callback_object, "block", block1, block2);
  ...
  for (let index = 0; index < mappings.length; index++) {
    for (let identifier in mappings[index]) {
      if (identifier in mapping && mapping[identifier] !== mappings[index][identifier]) {
        throw new global_Error(`Combination mismatch at ${path} between ${mapping[identifier]} and ${mappings[index][identifier]} for ${identifier}`);
      }
...
const bind = (path, identifiers1, identifiers2, mapping) => {
  ...
  for (let identifier in mapping) {
    ...
    if (bounded !== ArrayLite.has(identifiers2, mapping[identifier])) {
      throw new global_Error(`Binding mismatch at ${path} between ${identifier} and ${mapping[identifier]}`);
    }
    if (bounded) {
...
const check = (path, primitive1, primitive2) => {
  if (primitive1 !== primitive2) {
    throw new global_Error(`Structural missmatch at ${path} between ${print(primitive1)} and ${print(primitive2)}`);
  }
  return {__proto__:null}
};

const type_mismatch = (path, node1, node2, type1, type2) => { throw new global_Error(`Type mismatch at ${path} between ${type1} and ${type2}`)};

const block_callback_object = {
  __type_mismatch__: type_mismatch,
  BLOCK: (path, block1, block2, identifiers1, statements1, identifiers2, statements2) => combine(
    path,
    [
...
const expression_callback_object = {
    ...
    ArrayLite.concat(
      ...
      ArrayLite.map(
        ...
        (_, index) => Lang._allign_expression(
          ...
          expressions1[index],
          expressions2[index])))),
  unary: (path, expression1, expression2, operator1, expression11, operator2, expression21) => combine(
    path,
    [
  ...
  object: (path, expression1, expression2, expression11, properties1, expression21, properties2) => combine(
    ...
    ArrayLite.concat(
      [
        check(path + ".construct-length", properties1.length, properties2.length),
        Lang._allign_expression(
          expression_callback_object,
