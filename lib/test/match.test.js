...
///////////
{
  const success = (code1, code2, mapping) => Assert.deepEqual(
    Match._match_block(
      ...
      Parser.PARSE(code2)),
    mapping);
  const failure = (code1, code2, regexp) => Assert.match(
    Match._match_block(
      Parser.PARSE(code1),
      Parser.PARSE(code2)),
    regexp);
...
////////////////
{
  ...
  failure(`(x1, x1)`, `(x2, y2)`, /^Combination mismatch .* x2 and y2 for x1$/);
  // Producers //
  // primitive //
  ...
  // eval //
  failure(`eval(x1, §y1, §z1)`, `eval(x1, §y2)`, /^Structural mismatch .* 2 and 1$/);
