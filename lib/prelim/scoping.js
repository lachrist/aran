...
const setHoisting = (node, variables) => Query.setHoisting(query, node, variables);
  ...
  globals = Source.updateGlobals(
    ...
    Hoisting.hoistProgram(
      ...
      Source.getVariablePredicate(
        source,
        Query.hasUseStrictDirective(query, node)))),
  source = Source.toEval(source),
  source = (
    Query.hasUseStrictDirective(query, node) ?
    Source.extendStrict(source) :
    source),
  Query.setDirectEvalCall(
    query,
    node,
    ArrayLite.reduce(
      ArrayLite.map(
...
const visitClosure = (node, source, mode, _strict) => (
  Throw.assert(
    (
  ...
  source = (
    ...
    Source.extendScope(
      source,
      Query.getHoisting(query, node.body))),
  Query.setDirectEvalCall(
    query,
    node.body,
  ...
  CallExpression: (node, source) => or(
    ...
    ArrayLite.reduce(
      ...
      (
        ...
        (
          Query.setSource(query, node, source),
          true) :
        false))),
  // Other //
