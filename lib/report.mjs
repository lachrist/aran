...
export const AranSyntaxError = class extends SyntaxError {
  constructor(
    /** @type {string} */ message,
    /** @type {import("./report").SyntaxErrorCause} */ cause,
  ) {
    super(message);
...
/**
 * @type {(
 *   cause: import("./report").InputErrorCause,
 * ) => string}
 */
...
export const AranInputError = class extends Error {
  constructor(/** @type {import("./report").InputErrorCause} */ cause) {
...
/**
 * @type {(
 *   cause: import("./report").PointcutErrorCause,
 * ) => string}
 */
export const formatPointcutErrorMessage = (cause) => {
  switch (cause.type) {
    case "DuplicateCut": {
      return `${cause.point} can only be triggered once but was triggered at #${cause.hash} by: ${join(
        cause.conflict,
        ", ",
      )}`;
    }
    case "MissingCut": {
      return `${cause.point} must be triggered once at #${cause.hash}`;
    }
    default: {
...
export const AranPointcutError = class extends Error {
  constructor(/** @type {import("./report").PointcutErrorCause} */ cause) {
...
/**
 * @type {(
 *  clash: import("./report").ClashErrorCause,
 * ) => string}
 */
export const formatClashErrorMessage = ({ name, base, meta }) =>
  `External variable '${base}' clashes with config.${name} ('${meta}')`;

export const AranClashError = class extends Error {
  constructor(/** @type {import("./report").ClashErrorCause} */ cause) {
