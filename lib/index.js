...
const unmangle_instrument_options = {
  ...
  label: (label) => ({
    __proto__: null,
    raw: label,
    type: Label._is_break(label) ? "break" : "continue",
    name: Label._is_full(label) ? Label._get_body(label) : null}),
  identifier: (identifier, _type) => (
    _type = (
      Identifier._is_meta(identifier) ?
      // console.assert(Identifier._is_base(Identifier._get_body(identifier)))
      "deadzone" :
      (
        Identifier._is_base(identifier) ?
        (
          (
            Identifier._is_meta(
              Identifier._get_body(identifier))) ?
          "meta" :
          "base") :
        "input")),
    {
