...
const generate_options_generate = {
  ...
  apply: (expression1, expression2, expressions, result, nullable_result, results, _name) => Tree._match(expression1, array_of_intrinsic_matcher) && Tree._match(expression2, undefined_primitive_matcher) ? typeof result === 'string' ? `[${ ArrayLite.join(results) }]` : {
    type: 'ArrayExpression',
    elements: results
  } : (Tree._match(expression1, get_advice_matcher), Tree._match(expression2, advice_matcher)) ? (_name = Tree._extract(null, expression1, 'apply', get_advice_extractor), typeof result === 'string' ? `(${ nullable_result }.${ _name }(${ ArrayLite.join(_results, ',') }))` : {
    type: 'CallExpression',
    optional: false,
    callee: {
