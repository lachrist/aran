...
const diff_estree = (node1, node2) => {
  if (global.isArray(node1)) {
    Assert.ok(global.isArray(node2));
    Assert.deepEqual(node1.length, node2.length);
    for (let index = 0; index < node1.length; index++) {
      diff_estree(node1[index], node2[index]);
    }
  } else if (typeof node1 === "object" && node1 !== null) {
    Assert.ok(typeof node2 === "object" && node2 !== null);
    for (let key of global.Reflect.ownKeys(node1)) {
      Assert.ok(global.Reflect.getOwnPropertyDescriptor(node1, key) !== void 0);
      diff_estree(node1[key], node2[key]);
    }
  } else {
...
test(
  {
    apply: (expression1, expression2, expressions, code1, code2, codes) => `(${ArrayLite.join(ArrayLite.concat([code1, code2], codes), ",")})`,
    construct: (expression, expressions, code, codes) => `(${ArrayLite.join(ArrayLite.concat([code], codes), ",")})`},
  Tree._program(
    [],
    Tree.BLOCK(
      [],
