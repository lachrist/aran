...
State._run_session({nodes:[], serials:new Map(), scopes:{__proto__:null}}, () => {
  ...
  //   Meta.get(
  //     Meta._make_root(),
  //     Meta._box(Meta._make_root(), "foo", false, Tree.intrinsic("global"))),
  ...
  //     Assert.throws(() => Meta.set(scope, box, Tree.primitive(123)), new global.Error("Cannot set a constant meta box"));
  ...
  Lang._match(Meta.EXTEND_STATIC(Meta._make_root(), [], [], (scope) => {
    return Tree.Bundle([
      Tree.Lift(Meta.box(scope, false, "x", Tree.intrinsic("eval"), (box) => {
  ...
  Lang._match(Meta.EXTEND_STATIC(Meta._make_root(), [], ["kind"], (scope) => {
    ...
    return Tree.Bundle([
      Tree.Lift(Meta._initialize(scope, "kind", "y").initialize(Tree.primitive(123))),
      Tree.Lift(Meta.box(scope, false, "x", Meta.lookup(scope, "y", {
        on_miss: () => Assert.fail(),
        on_static_dead_hit: () => Assert.fail(),
        on_static_live_hit: (variable, read, write) => write(Tree.primitive(456)),
