...
State.runSession({nodes:[], serials:new Map(), scopes}, () => {
  ...
  Lang.match(Core.makeBlock(Core.RootScope(), ["kind"], (scope) => {
    ...
    return Tree.BundleStatement([
      Tree.BlockStatement(Tree.LabelBlock([], Core.makeBlock(Core.BindingScope(Core.ClosureScope(scope), "foo", "bar"), ["kind"], (scope) => {
        Core.declare(scope, {kind:"kind", ghost:false, data:null, name:"x"});
      ...
      }, (scope) => Tree.PrimitiveExpression("completion1")))),
  ...
  }, (scope) => Tree.PrimitiveExpression("completion2")), Lang.parseBlock(`{
    let $x, $y, _y;
    _y = false;
    {
      let $x, $z;
