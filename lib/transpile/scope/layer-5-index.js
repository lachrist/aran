...
const populate = (scope, variables, callback) => Tree.Bundle(
  [
    Tree.Bundle(
      ArrayLite.map(
        variables,
        (variable) => Base.Declare(scope, variable))),
    callback(scope)]);
...
exports._make_root = () => exports._make_test_root(null);
...
const typeof_callback_prototype = {
  ...
  on_static_dead_hit: (variable) => (
    variable.kind === "import" ?
    Tree.unary(
      "typeof",
      Tree.import(variable.specifier, variable.source)) :
    Builtin.throw_reference_error(
  ...
  on_static_live_hit: (variable, read, write) => Tree.unary(
    "typeof",
    read()),
  on_dynamic_dead_hit: function () { return Builtin.throw_reference_error(
    `Cannot read type from non-initialized dynamic variable named ${this.identifier}`); },
  on_dynamic_live_hit: function (tag, box) { return Tree.unary(
    "typeof",
    Builtin.get(
      Base.get(this.scope, box),
      Tree.primitive(this.identifier),
...
const read_callback_prototype = {
  on_miss: function () { return Builtin.throw_reference_error(
    `Cannot read from missing variable ${this.identifier}`); },
  on_static_dead_hit: (variable) => (
    variable.kind === "import" ?
    Tree.import(variable.specifier, variable.source) :
    Builtin.throw_reference_error(`Cannot read from non-initialized static variable named ${variable.name}`)),
  on_static_live_hit: (variable, read, write) => read(),
  on_dynamic_dead_hit: function () { return Builtin.throw_reference_error(
    `Cannot read from non-initialized dynamic variable named ${this.identifier}`); },
  on_dynamic_live_hit: function (tag, box) { return Builtin.get(
    Base.get(this.scope, box),
    Tree.primitive(this.identifier),
    null); }};
...
const optimistic_write_callback_prototype = {
  ...
  on_static_dead_hit: function (variable) { return Tree.sequence(
    this.right,
    Builtin.throw_reference_error(`Cannot write to non-initialized variable named ${variable.name}`)); },
  on_static_live_hit: function (variable, read, write) { return (
    is_writable_kind(variable.kind) ?
    write(this.right) :
    Tree.sequence(
      this.right,
      Builtin.throw_type_error(`Cannot write to constant variable ${variable.name}`))); },
  on_dynamic_dead_hit: Throw.deadcode,
  on_dynamic_live_hit: (tag, box) => { throw MARKER; }};
...
const pessimistic_write_callback_prototype = {
  on_miss: function () { return (
    ...
    Builtin.define_property(
      ...
      false,
      Builtin._success_result)); },
  on_static_dead_hit: (variable) => Builtin.throw_reference_error(
    `Cannot write to non-initialized static variable named ${variable.name}`),
  on_static_live_hit: function (variable, read, write) { return (
    is_writable_kind(variable.kind) ?
    write(
      Base.get(this.scope, this.right)) :
    Builtin.throw_type_error(`Cannot write to static constant variable named ${variable.name}`)); },
  ...
  on_dynamic_live_hit: function (tag, box) { return Builtin.set(
    Base.get(this.scope, box),
    Tree.primitive(this.identifier),
    Base.get(this.scope, this.right),
    null,
