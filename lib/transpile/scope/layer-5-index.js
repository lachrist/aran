...
exports.eval = (scope, expression, _closure_context) => (
  expression = Base.eval(scope, expression),
  _closure_context = Base._get_binding(scope, CLOSURE_KEY),
  State._register_scope(
    {
      data: global_JSON_stringify(scope),
      info: {
        "strict-mode": Base._get_binding(scope, STRICT_KEY),
        "super-call": _closure_context.super !== null,
        "super-property": _closure_context.self !== null,
        "new-target": _closure_context.newtarget}}),
  expression);
...
const delete_callback_prototype = {
  ...
  // console.assert(kind === "import")
  on_fake_hit: (kind, tag) => Tree.primitive(false),
  on_dead_hit: (kind, tag) => Tree.primitive(false),
  on_live_hit: (kind, tag, access) => Tree.primitive(false),
...
const typeof_callback_prototype = {
  ...
  // console.assert(kind === "import")
  on_fake_hit: (kind, data) => Tree.unary(
    "typeof",
    Tree.import(data.source, data.specifier)),
  on_dead_hit: function (kind, data) { return Builtin.throw_reference_error(
    `Cannot read type from deadzone variable ${this.identifier}`) },
  on_live_hit: (kind, data, access) => Tree.unary(
    "typeof",
    access(null)),
...
const read_callback_prototype = {
  ...
  // console.assert(kind === "import")
  on_fake_hit: (kind, data) => Tree.import(data.source, data.specifier),
  on_dead_hit: function (kind, data) { return Builtin.throw_reference_error(`Cannot read from deadzone variable ${this.identifier}`) },
  on_live_hit: (kind, data, access) => access(null),
...
const optimistic_write_callback_prototype = {
  ...
  // console.assert(kind === "import")
  on_fake_hit: (kind, data) => { return Tree.sequence(
    this.right,
    Builtin.throw_reference_error(`Cannot write to import variable ${this.identifier}`)); },
  on_dead_hit: function (kind, data) { return Tree.sequence(
    this.right,
    Builtin.throw_reference_error(`Cannot write to deadzone variable ${this.identifier}`)); },
  on_live_hit: function (kind, tag, access) { return (
    writable ?
    ArrayLite.reduce(
      data.specifiers,
      (expression, specifier) => ArrayLite.sequence(
        expression,
        Tree.export(
          specifier,
          access(null))),
...
const pessimistic_write_callback_prototype = {
  on_miss: function () { return (
    ...
    Builtin.define_property(
      ...
      Builtin._success_result)) },
  // console.assert(kind === "import")
  on_fake_hit: function (kind, tag) { return Builtin.throw_reference_error(
    `Cannot write to import variable ${this.identifier}`); },
  on_dead_hit: function (kind, tag) { return Builtin.throw_reference_error(
    `Cannot write to deadzone variable ${this.identifier}`) },
  on_live_hit: function (kind, tag, access) { return (
    writable ?
    access(
