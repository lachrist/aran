...
exports._make_root = (strict) => exports._make_test_root({strict});
...
const typeof_callback_prototype = {
  ...
  on_static_live_hit: (variable, read, write) => Tree.unary(
    "typeof",
    read()),
  on_dynamic_dead_hit: function () { return Intrinsic.throw_reference_error(
...
const read_callback_prototype = {
  on_miss: function () { return Intrinsic.throw_reference_error(
  ...
  on_dynamic_live_hit: function (tag, box) { return Intrinsic.get(
    Base.get(this.scope, box),
    Tree.primitive(this.identifier),
    null); }};
...
const optimistic_write_callback_prototype = {
  ...
  on_static_dead_hit: function (variable) { return Tree.sequence(
    this.right,
    Intrinsic.throw_reference_error(`Cannot write to non-initialized variable named ${variable.name}`)); },
  on_static_live_hit: function (variable, read, write) { return (
    is_writable_kind(variable.kind) ?
    write(this.right) :
    Tree.sequence(
      this.right,
      Intrinsic.throw_type_error(`Cannot write to constant variable ${variable.name}`))); },
...
const pessimistic_write_callback_prototype = {
  on_miss: function () { return (
    Base._get_binding(this.scope, STRICT_KEY) ?
    Intrinsic.throw_reference_error(`Cannot write to missing variable ${this.identifier}`) :
    Intrinsic.define_property(
      ...
      false,
      Intrinsic._success_result)); },
  on_static_dead_hit: (variable) => Intrinsic.throw_reference_error(
    `Cannot write to non-initialized static variable named ${variable.name}`),
  on_static_live_hit: function (variable, read, write) { return (
    is_writable_kind(variable.kind) ?
    write(
      Base.get(this.scope, this.right)) :
    Intrinsic.throw_type_error(`Cannot write to static constant variable named ${variable.name}`)); },
  ...
  on_dynamic_live_hit: function (tag, box) { return Intrinsic.set(
    Base.get(this.scope, box),
    Tree.primitive(this.identifier),
    Base.get(this.scope, this.right),
    null,
