...
const CLOSURE_KEY = "closure"; // {arrow:boolean, function: "function" | "method" | "constructor" | "derived-constructor"}
...
exports.fetchClosure = (scope, _closure) => (
  _closure = Base.fetchBinding(scope, CLOSURE_KEY),
  _closure.arrow ? "arrow" : _closure.function);

exports.getParseOptions = (scope, _) => ({
  strict: Base.fetchBInding(scope, STRICT_KEY),
  function: Base.fetchBinding(scope, CLOSURE_KEY).function});

// exports.validate = (scope, result) => ({});
// 
// exports.getValidateOptions = (scope, runtime) => ({
// 
//   variables: 123,
// 
// });

exports.fetchFunction = (scope, _closure) => Base.fetchBinding(scope, CLOSURE_KEY).function;
...
exports.RootScope = (options, _scope) => (
  ...
  _scope = Base.BindingScope(_scope, SORT_LKEY, options.sort),
  _scope);
...
exports.makeEvalExpression = (scope, expression, _identifiers) => (
  _identifiers = Base.lookupAll(scope),
  State.registerEval(
    {
      identifiers: _identifiers,
      context: {
        strict: Base.fetchBinding(scope, STRICT_KEY),
        sort: Base.fetchBinding(scope, CLOSURE_KEY).function },
      scope: scope}),
  Tree.__EvalExpression__(_identifiers, expression));
...
const special_read_callback_prototype = {
  ...
  on_static_live_hit (variable, read, write) { return (
    (
      ...
      Base.fetchBinding(this.scope, CLOSURE_KEY).function === "derived-constructor") ?
...
const failure = (optimistic, right, expression) => (
  optimistic ?
...
const regular_write_callback_prototype = {
  on_miss () { return (
    ...
    (
      ...
      Intrinsic.makeDefinePropertyExpression(
        ...
        false,
        Intrinsic.SUCCESS_RESULT))); },
  on_static_dead_hit (variable) { return failure(
    this.optimistic,
    this.right,
    Intrinsic.makeThrowReferenceErrorExpression(
  ...
  on_dynamic_dead_hit () { return failure(
    this.optimistic,
    this.right,
    Intrinsic.makeThrowReferenceErrorExpression(
...
const super_member_callback_prototype = {
  on_miss () { return (
    ...
    Tree.__MemberSuperEnclaveExpression__(this.key)); },
  on_static_dead_hit (variable) { return (
    Throw.abort(
      null,
...
const super_call_callback_prototype = {
  on_miss () { return (
    ...
    Tree.__CallSuperEnclaveExpression__(this.arguments)); },
  on_static_dead_hit (variable) { return (
    Throw.abort(
      null,
