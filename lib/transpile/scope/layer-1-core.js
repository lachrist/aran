...
exports.EXTEND_STATIC = (parent, labels, kinds, callback) /* -> Block */ => {
  ...
  for (let index = 0; index < static_frame.length; index++) {
    ...
    if (!binding.variable.ghost) {
      ...
      stratified_identifier_array[stratified_identifier_array.length] = Identifier._base(identifier);
      if (binding.has_dynamic_deadzone) {
        stratified_identifier_array[stratified_identifier_array.length] = Identifier._meta(identifier);
        statement_array[statement_array.length] = Tree.Lift(Tree.__write__(Identifier._meta(identifier), Tree.primitive(false)));
      }
    }
  }
      ...
      for (let index = 0; index < scope.frame.length; index++) {
        if (scope.frame[index].variable.name === identifier) {
          ...
          return {
            ...
            read: () => Tree.__read__(Identifier._base(identifier)),
            initialize: (expression) => {
              expression = Tree.__write__(Identifier._base(identifier), expression);
              if (binding.has_dynamic_deadzone) {
                expression = Tree.sequence(expression, Tree.__write__(Identifier._meta(identifier), Tree.primitive(true)));
              }
              return expression
            }
          };
  ...
  const loop = (scope, escaped) => {
    ...
    for (let index = 0; index < scope.frame.length; index++) {
      if (scope.frame[index].variable.name === identifier) {
        ...
        const read = () => Tree.__read__(Identifier._base(identifier));
        const write = (expression) => Tree.__write__(Identifier._base(identifier), expression);
        ...
        return Tree.conditional(
          Tree.__read__(Identifier._meta(identifier)),
