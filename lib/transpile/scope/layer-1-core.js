...
exports.makeBlock = (parent, kinds, callback1, callback2) /* -> Block */ => {
  const static_frame = [];
  const scope = {
    parent,
  ...
  const statement = callback1(scope);
  const expression = callback2(scope)
  ...
  for (let index = 0; index < static_frame.length; index++) {
    ...
    if (!binding.variable.ghost) {
      ...
      stratified_identifier_array[stratified_identifier_array.length] = Identifier.Base(identifier);
      if (binding.has_dynamic_deadzone) {
        stratified_identifier_array[stratified_identifier_array.length] = Identifier.Meta(identifier);
        statement_array[statement_array.length] = Tree.ExpressionStatement(Tree.__WriteExpression__(Identifier.Meta(identifier), Tree.PrimitiveExpression(false)));
      }
    }
  }
  statement_array[statement_array.length] = statement;
  return Tree.__Block__(stratified_identifier_array, statement_array, expression);
};
      ...
      for (let index = 0; index < scope.frame.length; index++) {
        if (scope.frame[index].variable.name === identifier) {
          ...
          return {
            ...
            read: () => Tree.__ReadExpression__(Identifier.Base(identifier)),
            initialize: (expression) => {
              expression = Tree.__WriteExpression__(Identifier.Base(identifier), expression);
              if (binding.has_dynamic_deadzone) {
                expression = Tree.SequenceExpression(expression, Tree.__WriteExpression__(Identifier.Meta(identifier), Tree.PrimitiveExpression(true)));
              }
              return expression
            }
          };
  ...
  const loop = (scope, escaped) => {
    ...
    for (let index = 0; index < scope.frame.length; index++) {
      if (scope.frame[index].variable.name === identifier) {
        ...
        const read = () => Tree.__ReadExpression__(Identifier.Base(identifier));
        const write = (expression) => Tree.__WriteExpression__(Identifier.Base(identifier), expression);
        ...
        return Tree.ConditionalExpression(
          Tree.__ReadExpression__(Identifier.Meta(identifier)),
...
exports.makeEvalExpression = (scope, expression) => {
  ...
  return Tree.__EvalExpression__(expression);
};
