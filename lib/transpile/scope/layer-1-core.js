...
exports.EXTEND_STATIC = (parent, labels, kinds, callback) /* -> Block */ => {
  const static_frame = [];
  ...
  for (let index = 0; index < static_frame.length; index++) {
    ...
    if (!binding.variable.ghost) {
      ...
      if (binding.has_dynamic_deadzone) {
        ...
        statement_array[statement_array.length] = Tree.ExpressionStatement(Tree.__write__(Identifier.Meta(identifier), Tree.PrimitiveExpression(false)));
      }
    }
  }
  statement_array[statement_array.length] = statement;
  return Tree.__Block__(labels, stratified_identifier_array, statement_array);
};
...
exports._fetch_binding = (scope, key) => {
...
exports._is_available = (scope, kind, identifier, callbacks) => {
  ...
  // enclaves might not have binding frame
      ...
      for (let index = 0; index < scope.frame.length; index++) {
        if (scope.frame[index].variable.name === identifier) {
          ...
          return {
            ...
            ReadExpression: () => Tree.__read__(Identifier.Base(identifier)),
            initialize: (expression) => {
              expression = Tree.__write__(Identifier.Base(identifier), expression);
              if (binding.has_dynamic_deadzone) {
                expression = Tree.SequenceExpression(expression, Tree.__write__(Identifier.Meta(identifier), Tree.PrimitiveExpression(true)));
              }
              return expression
            }
          };
  ...
  const loop = (scope, escaped) => {
    ...
    for (let index = 0; index < scope.frame.length; index++) {
      if (scope.frame[index].variable.name === identifier) {
        ...
        const read = () => Tree.__read__(Identifier.Base(identifier));
        const write = (expression) => Tree.__write__(Identifier.Base(identifier), expression);
        ...
        return Tree.ConditionalExpression(
          Tree.__read__(Identifier.Meta(identifier)),
