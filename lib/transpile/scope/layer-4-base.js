  ...
  (
    ...
    (
      ...
      updateExport(
        ...
        (expression) => Variable.makeStaticInitializeExpression(
          scope,
          Variable.getName(variable),
...
exports.makeInitializeStatement = (scope, kind, identifier, box, write, _frame) => (
  ...
  (
    ...
    (
      ...
      (
        _frame = Meta.getDynamicFrame(scope),
        Throw.assert(_frame.deadzone, null, `Invalid dynamic frame for rigid variable (initialization)`),
        Tree.ExpressionStatement(
          Tree.ConditionalExpression(
...
const lookup_callback_prototype = {
  ...
  onStaticLiveHit (data, read, write) { return this.callbacks.on_static_live_hit(
    read,
    (box) => (
      data.writable ?
      // console.assert(data.import === null)
      updateExportLink(this.scope, data.exports, box, write) :
