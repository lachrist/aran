...
const state = {nodes:[], serials:new Map(), evals:{__proto__:null}};
...
State.runSession(state, () => {
  ...
  Lang.match(
    ...
    Assert);
  // makeEvalExpression //
  Assert.deepEqual(global.Reflect.ownKeys(state.evals).length, 0);
  Lang.match(
    ...
    Assert);
  Assert.deepEqual(global.Reflect.ownKeys(state.evals).length, 1);
  Assert.deepEqual(state.evals[global.Reflect.ownKeys(state.evals)[0]].context, {strict:true, sort:"function"});
