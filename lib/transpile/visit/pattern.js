...
const propagate = (dropped, sequenced) => (dropped === null ? null : sequenced);
...
const lift = (dropped, aran_node) => (
  dropped === null ?
  Tree.ExpressionStatement(aran_node) :
  aran_node);

const sequence = (dropped, aran_node_1, aran_node_2) => (
  dropped === null ?
  // console.assert(typeof node1 === "statement" && typeof node2 === "statement")
  Tree.ListStatement([aran_node_1, aran_node_2]) :
  // console.assert(typeof node1 === "expression" && typeof node2 === "expression")
  Tree.SequenceExpression(aran_node_1, aran_node_2));

const boxify = (dropped, scope, writable, identifier, expression, callback) => Scope[dropped === null ? "makeBoxStatement" : "makeBoxExpression"](
  scope,
  writable,
  identifier,
  expression,
...
visitors.CallExpression = (scope, node, context) => sequence(
  context.dropped,
  lift(context.dropped, context.expression),
  sequence(
    context.dropped,
    lift(
      context.dropped,
  ...
  (
    ...
    (
      ...
      Tree.ExpressionStatement(
        Scope.makeWriteExpression(scope, node.name, context.expression)))) :
  (
    Throw.assert(context.dropped === null, null, `Cannot initialize variable in an expression context`),
...
visitors.AssignmentPattern = (scope, node, context) => boxify(
  ...
  (box) => Visit.visitPattern(
    ...
    {
      kind: context.kind,
      dropped: propagate(context.dropped, false),
        ...
        (next, property) => boxify(
          ...
          (key_box) => sequence(
            ...
            Visit.visitPattern(
              ...
              {
                kind: context.kind,
                dropped: propagate(context.dropped, true),
