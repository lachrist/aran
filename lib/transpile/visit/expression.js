      ...
      Scope.CLOSURE_HEAD(
        ...
        (scope) => Scope.Box(
          ...
          Intrinsic.construct_object(
            ...
            [
              [
                Intrinsic.grab("Symbol.isConcatSpreadable"),
                Tree.primitive(true)]]),
          (result_box) => Scope.Box(
            scope,
            true,
            "length",
            Tree.primitive(0),
            (length_box) => Scope.Box(
              scope,
              false,
              "iterable",
              Intrinsic.get(
                ...
                Tree.primitive(0),
                null),
              (iterable_box) => Scope.Box(
                scope,
                false,
                "iterator",
                Tree.apply(
                  Intrinsic.get(
                    Scope.get(scope, iterable_box),
...
//   (
//     context.key === null ?
//     Scope.super_call(scope, context.arguments) :
//     Scope.super_member(scope, context.key)));
...
visitors.YieldExpression = (scope, node, context) => Tree.yield(
  node.delegate,
  (
    node.argument === null ?
    Tree.primitive(void 0) :
...
visitors.AwaitExpression = (scope, node, context) => Tree.await(
  Visit.expression(
    scope,
    node.argument,
    null));
...
visitors.UpdateExpression = (scope, node, context, _closure) => (
  _closure = (old_value_box, closure, _expression) => (
    _expression = Tree.binary(
      node.operator[0],
      Scope.get(scope, old_value_box),
      Tree.conditional(
        Tree.binary(
          "===",
          Tree.unary(
    ...
    (
      ...
      (
        ...
        Scope.box(
          ...
          _expression,
          (new_value_box) => Tree.sequence(
            closure(
              Scope.get(scope, new_value_box)),
            Scope.get(scope, new_value_box))) :
        Tree.sequence(
          closure(_expression),
          Scope.get(scope, old_value_box))))),
...
//       scope,
//       kontinuation,
//       Tree.binary(expression1, expression2))));
// 
  ...
  (
    ...
    (
      ...
      Scope.super_call(
        scope,
        _closure2(
          (expressions) => Intrinsic.construct_array(expressions),
          identity)) :
      (
