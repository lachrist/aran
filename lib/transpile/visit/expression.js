...
exports.visitExpression = (scope, node, context) => (
  context = global_Object_assign(
    {dropped:false},
    context),
  visitors[node.type](scope, node, context));

exports.visitSpreadable = (scope, node, context) => (
...
exports.visitMember = (scope, node, context) => (
...
exports.visitQuasi = (scope, node, context) => (
...
visitors.TemplateLiteral = (scope, node, context) => (
  ...
  Visit.visitQuasi(scope, node.quasis[0], null) :
  Tree.BinaryExpression(
    "+",
    ArrayLite.reduce(
      ...
      (expression, _, index) => Tree.BinaryExpression(
        ...
        Tree.BinaryExpression(
          "+",
          Visit.visitQuasi(scope, node.quasis[index + 1], null),
          Visit.visitExpression(scope, node.expressions[index + 1], null))),
      Tree.BinaryExpression(
        "+",
        Visit.visitQuasi(scope, node.quasis[0], null),
        Visit.visitExpression(scope, node.expressions[0], null))),
    Visit.visitQuasi(scope, node.quasis[node.quasis.length - 1], null)));
...
visitors.ArrowFunctionExpression = (scope, node, context) => Visit.visitClosure(scope, node, null);

visitors.FunctionExpression = (scope, node, context) => Visit.visitClosure(scope, node, null);

visitors.ClassExpression = (scope, node, context) => Visit.visitClass(scope, node, null);
...
visitors.AssignmentExpression = (scope, node, context, _expression) => (
  ...
  (
    ...
    Intrinsic.makeSetExpression(
      Visit.visitExpression(scope, node.left.object, null),
      (
        node.left.computed ?
        Visit.visitExpression(scope, node.left.property, null) :
    ...
    (
      ...
      Visit.visitPattern(
        ...
        node.left,
        {
          kind: null,
          surrounding: "expression",
          expression: Visit.visitExpression(scope, node.right, null)}) :
      Scope.makeBoxExpression(
        scope,
        false,
...
visitors.UpdateExpression = (scope, node, context, _closure) => (
  ...
  (
    ...
    Scope.makeBoxExpression(
      ...
      Visit.visitExpression(scope, node.argument.object, null),
...
// > o.x.name
// ''
visitors.SequenceExpression = (scope, node, context) => ArrayLite.mapReduce(
  ArrayLite.slice(node.expressions, 0, node.expressions.length - 1),
  (next, node) => Tree.SequenceExpression(
    Visit.visitExpression(
      scope,
      node,
      {dropped: true}),
    next(null)),
  (array) => Visit.visitExpression(
    scope,
...
visitors.LogicalExpression = (scope, node, context) => (
  (
    ...
    context.dropped) ?
  Tree.ConditionalExpression(
    Visit.visitExpression(scope, node.left, null),
    (
      node.operator === "&&" ?
      Visit.visitExpression(
        scope,
...
visitors.ArrayExpression = (scope, node, context) => (
  ...
  Intrinsic.makeArrayExpression(
    ArrayLite.map(
      node.elements,
      (element) => Visit.visitExpression(scope, element, null))) :
        ...
        Intrinsic.makeObjectExpression(
          ...
          [
            ...
            [
              Intrinsic.makeGrabExpression("Symbol.isConcatSpreadable"),
              Tree.PrimitiveExpression(true)]]) :
        Visit.visitSpreadable(scope, node, null)))));

visitors.ImportExpression = (scope, node, context) => Tree.RequireExpression(
  Visit.visitExpression(scope, node.source, null));
      ...
      Intrinsic.makeDeletePropertyExpression(
        Scope.makeBoxExpression(
          ...
          (box) => Intrinsic.makeNullishExpression(
            ...
            null,
            null)),
        Visit.visitKey(scope, node.argument.property, {computed:node.argument.computed}),
        Scope.isStrict(scope),
        Intrinsic.SUCCESS_RESULT) :
      (
        node.argument.type === "Identifier" ?
...
// visitors.AwaitExpression = (scope, node, context, kontinuation) => Visit.visitExpression(
//   scope,
//   node.argument,
//   null,
...
//   node.left,
//   null,
//   (scope, expression1) => Visit.visitExpression(
//     scope,
//     node.right,
//     null,
...
visitors.MemberExpression = (scope, node, context) => Visit.visitMember(
  scope,
  node,
  {
    // We can safely drop the second expression because it comes from
    // Scope.makeOpenExpression(scope, box) which should not have any side effect (on the scope)
    kontinuation: identity});

visitors.ChainExpression = (scope, node, context) => Visit.visitExpression(scope, node.expression, null);
...
visitors.NewExpression = (scope, node, context) => (
  ...
  Intrinsic.makeConstructExpression(
    Visit.visitExpression(scope, node.callee, null),
    Intrinsic.makeConcatExpression(
      ArrayLite.map(
        node.arguments,
        (node) => Visit.visitSpreadable(scope, node, null))),
    null) :
  Tree.ConstructExpression(
    Visit.visitExpression(scope, node.callee, null),
    ArrayLite.map(
      node.arguments,
      (argument) => Visit.visitExpression(scope, argument, null))));
    ...
    (box, _expression) => (
      _expression = ArrayLite.mapReduce(
        ...
        (next, argument) => Scope.makeBoxExpression( // console.assert(argument.type !== "SpreadElement")
          ...
          (
            ArrayLite.some(node.arguments, is_spread_element_node) ?
            Visit.visitSpreadable(scope, argument, null) :
            Visit.visitExpression(scope, argument, null)),
          next),
  ...
  (
    ...
    _closure2 = (callback1, callback2) => (
      ...
      callback1(
        ArrayLite.map(
          node.arguments,
          (node) => Visit.visitExpression(scope, node, null)))),
    (
