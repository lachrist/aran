  ...
  require("./pattern.js"),
  {
    visitExpression: (scope, expression, context) => {
      ...
      if (expression.type === "CallExpression" && expression.callee.type === "Literal" && expression.arguments.length === 0) {
        return Tree.PrimitiveExpression(expression.callee.value);
      }
...
State.runSession({nodes: [], serials: new Map(), scopes: {__proto__:null}}, () => {
  const test = (strict, variables, kind, code1, code2, code3) => Lang.match(
      ...
      (scope) => (
        ...
        (
          Visit.visitPattern(
            ...
            {
              kind: kind,
              expression: Lang.parseExpression(code2)})))),
    Lang.parseBlock(code3),
    Assert);
  // CallExpression //
  test(
    true,
