...
visitors.ExpressionStatement = (scope, node, context) => (
  ...
  (
    Throw.assert(Scope.hasCompletion(scope), null, "Last valued-statement without completion in scope"),
    Tree.ExpressionStatement(
      Scope.makeCloseCompletionExpression(
        scope,
        Visit.visitExpression(scope, node.expression, null)))) :
  Tree.ExpressionStatement(
    Visit.visitExpression(scope, node.expression, {dropped: true})));
...
exports.visitDeclarator = (scope, node, context) => (
    ...
    Tree.ExpressionStatement(
      Scope.makeBoxExpression(
        ...
        (box) => Tree.ExportExpression(
          "default",
          Visit[node.declaration.type === "ClassDeclaration" ? "visitClass" : "visitClosure"](
            scope,
            node.declaration,
            {name:box})))) :
...
visitors.BlockStatement = (scope, node, context) => Tree.BranchStatement(
  Tree.Branch(
    ArrayLite.map(context.labels, Label.FullBreak),
    Visit.visitBlock(scope, node, {completion:context.completion})));

visitors.LabeledStatement = (scope, node, context) => Visit.visitStatement(
  scope,
  node.body,
  {
...
visitors.IfStatement = (scope, node, context) => reset_completion(
  ...
  Tree.IfStatement(
    ...
    Tree.Branch(
      ArrayLite.map(context.labels, Label.FullBreak),
      Visit.visitBlock(scope, node.consequent, {completion: context.completion})),
    Tree.Branch(
      ArrayLite.map(context.labels, Label.FullBreak),
      (
        node.alternate === null ?
        Scope.makeNormalBlock(
          ...
          (scope) => Tree.CompletionStatement(
            Tree.PrimitiveExpression(void 0))) :
        Visit.visitBlock(scope, node.alternate, {completion:context.completion})))));
...
visitors.TryStatement = (scope, node, context) => reset_completion(
  ...
  Tree.TryStatement(
    Tree.Branch(
      ArrayLite.map(context.labels, Label.FullBreak),
      Visit.visitBlock(scope, node.block, {completion:context.completion})),
    ...
    Tree.Branch(
      ...
      (
        ...
        Visit.visitCatch(scope, node.handler, {completion:context.completion}))),
