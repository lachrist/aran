...
exports.hasUseStrictDirective = (statements) => {
  for (let index = 0; index < statements.length; index++) {
    ...
    // we cannot use statement[index].expression.value because it should  compare the raw
    // string (and not the cooked one with utf8 escaped sequence and escaped newline)
    if (statements[index].directive === "use strict") {
      return true;
    }
  }
  return false;
...
exports.getValuation = (statement) => get_valuation_loop(statement, null);
