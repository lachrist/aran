...
exports.ExpressionStatement = (node) => Flaten(
  Build.Statement(
  ...
  node.handler ?
    Flaten(
      Helpers.assignment(
        ...
        Build.read("error")),
      Flaten.apply(
        node.handler.body.map(Visit.Statement))) :
    [],
  node.finalizer ?
    Flaten.apply(
      node.finalizer.body.map(Visit.Statement)) :
    []);
...
exports.DoWhileStatement = (node) => Flaten(
  Build.Statement(
    Build.write(
      Hide("dowhile"),
...
exports.VariableDeclaration = (node) => Helpers.Declaration(node.kind, node.declarators);
